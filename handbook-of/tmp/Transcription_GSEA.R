...
plotVolcano <- function(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, legend, legends, cols, cols2, fold=1, ymax=0) {
   ...
   plot(de$LOG2_FC, de$log10P, pch=16, xlim=c(1, 4), ylim=c(2, ymax), xlab=xlab.text, ylab=ylab.text, col="lightgray", main=file.main[1], cex=1.7, cex.axis=1.6, cex.lab=1.7, cex.main=1.8)
   ...
   if (nrow(genes) != 0) {
      for (g in 1:nrow(genes)) {
         ...
         if (nrow(gene) > 0) {
            ...
            if (!is.na(gene$ADJ_1))
               if (is.na(gene$ADJ_2))
                  text(gene$LOG2_FC, gene$log10P, gene$external_gene_name, col="black", adj=gene$ADJ_1, cex=1.7, srt=45)
               else
                  text(gene$LOG2_FC, gene$log10P, gene$external_gene_name, col="black", adj=c(gene$ADJ_1, gene$ADJ_2), cex=1.7, srt=45)
            else
               if (gene$LOG2_FC > 0)
                  text(gene$LOG2_FC, gene$log10P, gene$external_gene_name, col="black", adj=c(0, -0.5), cex=1.7, srt=45)
               else
                  text(gene$LOG2_FC, gene$log10P, gene$external_gene_name, col="black", adj=c(1, -0.3), cex=1.7, srt=45)
         } else
            print(genes[g,])
      }
   }
...
plotCannoli<- function(de1, de2, pvalue, genes, file.de, file.main, xlab.text, ylab.text, legend, legends, cols, cols2, fold=1, ymax=0) {
   ...
   pdf(file.de, height=6, width=6)
   par(mar=c(5.1, 4.7, 4.1, 1.4))
   plot(de$Effect1, de$Effect2, pch=16, xlab=xlab.text, ylab=ylab.text, col="lightgray", main=file.main[1], cex=1.7, cex.axis=1.6, cex.lab=1.7, cex.main=1.8)
   #abline(h=c(-log10(pvalue)), lty=5, lwd=2)

   de.negative <- subset(subset(de, Effect1 < 0), Effect2 < 0)
   de.negative.sig <- subset(subset(de.negative, P1 <= pvalue), P2 <= pvalue)
   points(de.negative.sig$Effect1, de.negative.sig$Effect2, pch=16, col=cols[2], cex=1.7)
   
   de.positive <- subset(subset(de, Effect1 > 0), Effect2 > 0)
   de.positive.sig <- subset(subset(de.positive, P1 <= pvalue), P2 <= pvalue)
   points(de.positive.sig$Effect1, de.positive.sig$Effect2, pch=16, col=cols[1], cex=1.7)
   ...
   legends[1] <- paste0(nrow(de.positive.sig), " ", legends[1])
   legends[2] <- paste0(nrow(de.negative.sig), " ", legends[2])
   #axis(side=1, at=seq(-8, 8, by=1), labels=c(-3, -2, -1, 0, 1, 2, 3), cex.axis=1.2)
   legend(legend, legend=legends, col=cols2, pch=19, pt.cex=2, cex=1.6)
   dev.off()
}
