...
plotSRC <- function(wd.plots, gene, cn, snr, pch, pos="bottomright", col, size=6) {
   ...
   xlab.text <- "Proliferation rate"
   ylab.text <- expression("Log" * ""[2] * "(TPM + 1)")
   #ylab.text <- "Expression"
   id <- subset(ensGene, external_gene_name == gene)$ensembl_gene_id
   file.name <- file.path(wd.plots, paste0("TPM-vs-SORTING_", genes[g], "_S"))
 
   pdf(paste0(file.name, ".pdf"), height=size, width=size)
   par(mar=c(5.1, 4.7, 4.1, 1.4))
   plot(cn ~ snr, ylim=ylim, xlim=xlim, ylab=ylab.text, xaxt="n", xlab=xlab.text, main=paste0(gene, " (n=29)"), col="black", pch=pch, cex=2, cex.axis=1.7, cex.lab=1.8, cex.main=1.9)
