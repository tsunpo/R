...
# -----------------------------------------------------------------------------
getCannoli <- function(wd.de.data, BASE, n, gene.list=NULL) {
   load(file=file.path(wd.de.data, paste0("SRC_", BASE, "_tpm-gene_CNA-vs-TPM_q_n", n, ".RData")))
   de1 <- src.tpm.gene
   load(file=file.path(wd.de.data, paste0("SRC_", BASE, "_tpm-gene_SORTING-vs-TPM_q_n", n, ".RData")))
   de2 <- src.tpm.gene
...
plotCannoliNegative <- function(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, legend, legends, cols, cols2, fold=1, ymax=0, col="black", pos="bottomright") {
   pdf(file.de, height=6, width=6)
   par(mar=c(5.1, 4.7, 4.1, 1.4))
...
writeRNKformatCNA <- function(de.tpm.gene, wd.de.data, file.name) {
   de.tpm.gene$WEIGHT <- abs(de.tpm.gene$Effect1) * de.tpm.gene$Effect2
   de.sorted <- de.tpm.gene[order(de.tpm.gene$WEIGHT, decreasing=T),]
