...
###
##
initMergedReportSNV <- function() {
   colnames <- c("SAMPLE", "CHROM", "POS", "REF", "ALT", "QUAL")
 
   return(toTable("", length(colnames), 0, colnames))
}

initTableS6SNV <- function() {
   tx.snv.s6 <- list(list(), list(), list(), list(), list(), list())
   for (i in 1:6)
      for (j in 1:2)
         tx.snv.s6[[i]][[j]] <- initMergedReportSNV()
   
   return(tx.snv.s6)
}

getTableS6SNV <- function(snvs) {
   snvs.s6 <- initTableS6SNV()

   for (i in 1:length(idxs)) {
      idx <- idxs[i]
  
      watson <- subset(subset(snvs, REF == REFS[idx]), ALT == ALTS[idx])
      snvs.s6[[i]][[1]] <- rbind(snvs.s6[[i]][[1]], watson)

      crick  <- subset(subset(snvs, REF == REFS[idx+1]), ALT == ALTS[idx+1])
      snvs.s6[[i]][[2]] <- rbind(snvs.s6[[i]][[2]], crick)
   }
