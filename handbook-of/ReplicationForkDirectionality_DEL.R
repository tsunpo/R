...
plotReportNRFD <- function(report.rfds, names, file.name, main.text) {
   ...
   ##
   #par(mar=c(6, 4.6, 0, 0))
   #plot(NULL, xlim=c(0.5, 8.4), ylim=c(0, 26), ylab="", xlab="", col=cols[2], xaxt="n", bty="n", pch=rfds$pch, cex.axis=1.7, cex.lab=1.8, cex.main=1.9)
   #points(rfds$X, rfds$CTR_L, col=cols[3], pch=rfds$pch, cex=rfds$cex)
   #lines(rfds$X[1:4], y=rfds$CTR_L[1:4], type="l", lwd=3, col=cols[3])
   #lines(rfds$X[5:6], y=rfds$CTR_L[5:6], type="l", lwd=3, col=cols[3])
...
plotVio20 <- function(wd.de.plots, file.name, tpm.1, tpm.2, main, names, cols, cols2, ylim, height=4.5, width=3.5, ylab="") {
	  trait <- rep(0, length(tpm.1))
	  trait <- c(trait, rep(1, length(tpm.2)))
	  trait <- as.factor(trait)
	  expr <- c(tpm.1, tpm.2)
