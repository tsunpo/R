...
plotG4RS <- function(rts, g4s, file.name, main.text, xlab.text, ylab.text, cols, pos) {
   ...
   idx <- which(rts < rts[2])
   points(rts[idx] ~ g4s[idx], col=cols[2], pch=19, cex=1.2)
   ...
   text(g4s[4], rts[4], "Chr4", col=cols[2], pos=1, cex=1.2)
   text(g4s[13], rts[13], "Chr13", col=cols[2], pos=1, cex=1.2)
   if (length(na) != 0)
      text(g4s[na], rts[na], paste0("Chr", na), col=cols[3], pos=1, cex=1.2) 
...
plotBoxG4R <- function(wd.de.plots, file.name, tpm.1, tpm.2, main.txt, names, cols, ylim, height=6, width=4.2) {
   ...
   boxplot(expr ~ trait, outline=T, xaxt="n", ylab="", main=main.txt[1], col=cols, ylim=ylim, cex.axis=1.5, cex.lab=1.6, cex.main=1.7)
   ...
   ##
   axis(side=1, at=seq(1, 2, by=1), labels=names, font=2, cex.axis=1.6)
