...
plotRT <- function(file.name, main.text, chr, xmin, xmax, nrds.chr, bed.gc.chr, colours, legends, colours2, legends2, ext, width, peaks, ylim=NULL, lcl.rt.chr=NULL, nrds.lcl.chr=NULL) {
   ...
   if (is.null(ylim)) {
      ...
      plot(NULL, xlim=c(xmin/1E6, xmax/1E6), ylim=c(min(rds), max(rds)), xlab="", ylab=ylab.text, main=main.text, xaxt="n", cex.axis=1.1, cex.lab=1.2, cex.main=1.25)
   } else
      plot(NULL, xlim=c(xmin/1E6, xmax/1E6), ylim=ylim, xlab="", ylab=ylab.text, main=main.text, xaxt="n", cex.axis=1.1, cex.lab=1.2, cex.main=1.25)
...
plotRTvsRT2 <- function(cors, file.name, main.text, ymin, ymax, cols, legends) {
   ...
   #png(paste0(file.name, ".png"), height=5, width=5, units="in", res=300)
   pdf(paste0(file.name, ".pdf"), height=6, width=6)
   plot(cors$cor ~ cors$chr, ylim=c(ymin, ymax), ylab="", xlab=xlab.text, main=main.text, col=cols[1], xaxt="n", yaxt="n", pch=19, cex.axis=1.75, cex.lab=1.75, cex.main=1.85)
   lines(cors$cor, y=NULL, type="l", lwd=3, col=cols[1])
   points(cors$chr, cors$cor1, col=cols[2], pch=19)
   lines(cors$cor1, y=NULL, type="l", lwd=3, col=cols[2])
