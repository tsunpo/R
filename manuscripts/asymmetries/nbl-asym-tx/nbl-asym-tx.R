...
for (s in 1:length(samples)) {
   ...
   ens.tx <- intersect(unique(vcf$ensembl_gene_id), rownames(tpm.gene.nbl))   ## All SNVs on "expressed" genes, regardless protein coding or not
...
# -----------------------------------------------------------------------------
# Step 4.1: SNV asymetrey (Keep only SNVs on protein coding genes)
...
getTxQ4 <- function(tpm.gene.nbl.log2) {
   q <- quantile(tpm.gene.nbl.log2$MEDIAN)
 
   tx.q4 <- list()
   tx.q4[[4]] <- rownames(subset(tpm.gene.nbl.log2, MEDIAN > as.numeric(q[4])))
   tx.q4[[3]] <- rownames(subset(subset(tpm.gene.nbl.log2, MEDIAN > as.numeric(q[3])), MEDIAN <= as.numeric(q[4])))
   tx.q4[[2]] <- rownames(subset(subset(tpm.gene.nbl.log2, MEDIAN > as.numeric(q[2])), MEDIAN <= as.numeric(q[3])))
   tx.q4[[1]] <- rownames(subset(tpm.gene.nbl.log2, MEDIAN <= as.numeric(q[2])))
...
###
## Use only protein coding genes (with at least a SNV)
tpm.gene.nbl.log2     <- getLog2andMedian(tpm.gene.nbl)            ## ADD 07/02/18
tpm.gene.nbl.pcg.log2 <- getLog2andMedian(tpm.gene.nbl.pcg)
...
# -----------------------------------------------------------------------------
# Step 4.2: SNV asymetrey (Figure S1)
# Last Modified: 25/01/18
# -----------------------------------------------------------------------------
asyms <- list()
pdf(paste0(wd.asym.plots, "nbl_asym_tx_pcg_snv_s6_asyms.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
...
for (i in 1:length(idxs)) {
   ...
   for (j in 1:2)
      for (k in 1:2)
         asym[j, k] <- getMutPerMb(tx.snv.s6[[i]][[j]][[k]], tpm.gene.nbl.pcg)   ## Only look at SNVs on protein coding genes
...
dev.off()
save(tx.snv.s6, asyms, file=paste0(wd.asym.data, "nbl_asym_tx_pcg_snv_s6_asyms.RData"))

###
## Refining the plot
pdf(paste0(wd.asym.plots, "nbl_asym_tx_pcg_snv_s6_asyms_ylim10.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   asym <- asyms[[i]]
   idx <- idxs[i]
   
   barplot(asym, ylab="SNVs/Mb", main=getMain(rownames(asym)), beside=TRUE, width=.3, col=c("#E6E6E6", "#4D4D4D", "#4D4D4D", "#E6E6E6"), ylim=c(0, 10))
   mtext(paste(c(paste0(REFS[idx], ":", REFS[idx+1]), paste0(REFS[idx+1], ":", REFS[idx])), collapse=" vs "), cex=0.55, font=3, line=0.5)
}
dev.off()

# -----------------------------------------------------------------------------
# Step 4.3: Transcription-associated SNV asymmetry (Figure S2)
# Last Modified: 26/01/18
# -----------------------------------------------------------------------------
q4s <- list()
pdf(paste0(wd.asym.plots, "nbl_asym_tx_pcg_snv_s6_asyms_q4s.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   idx <- idxs[i]
...
# -----------------------------------------------------------------------------
# Step 4.4: Log2 transcription-associated SNV asymmetry (Figure S3)
# Last Modified: 26/01/18
# -----------------------------------------------------------------------------
pdf(paste0(wd.asym.plots, "nbl_asym_tx_pcg_snv_s6_asyms_q4s_log2_ylim0.25.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   q4 <- q4s[[i]]
   colnames(q4) <- c("", "           Tx", "", "")
   ...
   barplot(log2(q4[1,]/q4[2,]), ylab="Asymmetry", ylim=c(-0.25, 0.25), main=getMain(rownames(asyms[[i]])), beside=TRUE, width=.3, col=cols)   ## ADD 08/03/18
