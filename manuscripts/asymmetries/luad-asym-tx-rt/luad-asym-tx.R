...
# > nrow(tpm.gene.input)   ## All SNVs on expressed, "non-redundant" protein coding genes
# [1] 10352
...
max(asyms[[1]])
# [1] 283.1005
pdf(file.path(wd.asym.plots, paste0(base, "_asym_tx_snv_s6_ylim284.pdf")), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   asym <- asyms[[i]]
   idx <- idxs[i]
   
   barplot(asym, ylab="SNVs/Mb", main=getMain(rownames(asym)), beside=TRUE, width=.3, col=c("lightskyblue", "sandybrown", "sandybrown", "lightskyblue"), ylim=c(0, 283.1005))
   mtext(paste(c(paste0(REFS[idx], ":", REFS[idx+1]), paste0(REFS[idx+1], ":", REFS[idx])), collapse=" vs "), cex=0.55, font=3, line=0.5)
}
dev.off()
