...
wd.rt       <- file.path(wd.anlys, "replication", paste0(base, "-wgs-rt-m2"))   ## CHANGED from cm2
...
for (s in 1:6) {
   ...
   file.name <- file.path(wd.asym.plots, paste0(time, "_RDS-vs-MUT-", gsub("/", "_", s6[s]), "_SCLC"))
   main.text <- c(paste0(s6[s], ""), "")
   ymax <- 0.00695
   unit <- (max(cors$cor) - min(cors$cor))/20
   xlim <- c(min(cors$cor) - unit, max(cors$cor) + unit)
   plotRDCorsRDS(cors, skews, file.name, main.text, ymax, xlim=xlim, c(1, 4, 5, 13, 17, 19, 22), "SNVs/Mb")
   
   file.name <- file.path(wd.asym.plots, paste0(time, "_LENGTH-vs-MUT-", gsub("/", "_", s6[s]), "_SCLC"))
   main.text <- c(paste0(s6[s], ""), "")
   plotMUTLength(cors, muts, file.name, main.text, c(1, 4, 5, 13, 17, 19, 22), "SNVs/Mb")
   
   file.name <- file.path(wd.asym.plots, paste0(time, "_LENGTH-vs-SNV-", gsub("/", "_", s6[s]), "_SCLC"))
   main.text <- c(paste0(s6[s], ""), "")
