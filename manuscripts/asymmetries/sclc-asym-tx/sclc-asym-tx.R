...
tpm.gene.sclc.pcg <- getEnsGeneFiltered(tpm.gene.sclc, ensGene, autosomeOnly=T, proteinCodingOnly=T, withHLA=T)   ## CHANGE 05/02/18
...
for (s in 1:length(samples)) {
   ...
   ens.tx <- intersect(unique(vcf$ensembl_gene_id), rownames(tpm.gene.sclc))   ## All SNVs on "expressed" genes, regardless protein coding or not
...
###
##
ens.tx.pcg.snv <- intersect(unique(tx.snv$ensembl_gene_id), rownames(tpm.gene.sclc.pcg))   ## Needed in Step 6.0; ADD 02/02/18
...
tpm.gene.sclc.log2     <- getLog2andMedian(tpm.gene.sclc)            ## ADD 07/02/18
tpm.gene.sclc.pcg.log2 <- getLog2andMedian(tpm.gene.sclc.pcg)
#tpm.gene.sclc.pcg.log2 <- tpm.gene.sclc.pcg.log2[ens.tx.pcg.snv,]   ## ADD 02/02/18; REMOVE 06/02/18
...
# -----------------------------------------------------------------------------
asyms <- list()
pdf(paste0(wd.asym.plots, "sclc_asym_tx_pcg_snv_s6_asyms.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
...
for (i in 1:length(idxs)) {
   ...
   for (j in 1:2)
      for (k in 1:2)
         asym[j, k] <- getMutPerMb(tx.snv.s6[[i]][[j]][[k]], tpm.gene.sclc.pcg)   ## Only look at SNVs on protein coding genes
...
# Last Modified: 26/01/18
# -----------------------------------------------------------------------------
pdf(paste0(wd.asym.plots, "sclc_asym_tx_pcg_snv_s6_asyms_q4s_log2_ylim1.5.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   q4 <- q4s[[i]]
   colnames(q4) <- c("", "           Tx", "", "")
   
   barplot(log2(q4[1,]/q4[2,]), ylab="Asymmetry", ylim=c(-1.5, 1.5), main=getMain(rownames(asyms[[i]])), beside=TRUE, width=.3, col=c("#E6E6E6", "#E6E6E6", "#E6E6E6", "#E6E6E6"))
