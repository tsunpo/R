...
# -----------------------------------------------------------------------------
# Step 6.1: Read in all the indels
# Last Modified: 30/01/18
# -----------------------------------------------------------------------------
...
# [1] 14145
ens.tx.pcg.snv.indel <- intersect(unique(tx.indel$ensembl_gene_id), ens.tx.pcg.snv)   ## From Step 3.0; ADD 02/02/18; CHANGE 05/02/18 (with HLAs)
...
# > nrow(tx.bp)   ## Without size < 10000
# [1] 10301
# > nrow(tx.bp)   ## With size < 10000
# [1] 11194
...
tx.input <- tx.pcg.snv.indel
headon <- T   ## Replication–transcription Head-on (T) / Co-directional (F) collisions
prefix <- "ho"
ens.tx.pcg.snv.indel.rt <- intersect(unique(tx.input$ensembl_gene_id), unique(ensGene.tx.rt.nona.sign$ensembl_gene_id))
ensGene.tx.rt.input <- ensGene.tx.rt.nona.sign[ens.tx.pcg.snv.indel.rt,]
...
q4s <- list()
pdf(paste0(wd.asym.plots, "sclc_asym_tx_rt_snv_indel_q4s_", prefix, ".pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   ...
   q4s[[i]] <- q4
   barplot(q4, ylab="INVs/Mb", main=paste(rownames(q4), collapse="/"), beside=TRUE, width=.3, col=c("#E6E6E6", "#4D4D4D"))
}
dev.off()
save(q4s, file=paste0(wd.asym.data, "sclc_asym_tx_rt_snv_indel_q4s_", prefix, ".RData"))
...
for (i in 1:length(idxs)) {
   ...
   barplot(q4, ylab="INVs/Mb", main=getMain(rownames(asyms[[i]])), beside=TRUE, width=.3, col=c("#E6E6E6", "#4D4D4D"))   #, xlab="Number of genes")
...
# -----------------------------------------------------------------------------
pdf(paste0(wd.asym.plots, "sclc_asym_tx_rt_snv_indel_q4s_", prefix, "_log2_ylim0.2.pdf"), height=4.5, width=7)
par(mfrow=c(2, 3))
for (i in 1:length(idxs)) {
   q4 <- q4s[[i]]
   if (headon)
