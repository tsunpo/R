...
# -----------------------------------------------------------------------------
# Step 8.2: Log2 transcription-associated SNV asymmetry (Figure 3)
...
# > t <- as.data.frame(table(tx.indel$SAMPLE))
...
## Remove S00501   ## ADD 07/02/18
tx.indel <- subset(tx.indel, SAMPLE != "S00501")
# > nrow(tx.indel)
# [1] 263649
# > tx.indel <- subset(tx.indel, SAMPLE != "S00501")
# > nrow(tx.indel)
...
## ADD 07/02/18 (after removed S00501)
...
save(ensGene.tx.rt, tx.indel, tx.pcg.snv.indel, file=paste0(wd.asym.data, "sclc_asym_tx_rt_indel.RData"))   ## ADD 07/02/18
##TO-DO in Step 7

for (q in 1:4)
   print(length(intersect(ens.tx.pcg.snv.indel, tx.q4[[q]])))
