...
sv.del.rt <- rbind(sv.del.1, sv.del.2) 

FILE <- "cpg_islands_pos"
sv.del.1 <- sv.del[0,]
sv.del.2 <- sv.del[0,]

for (c in 1:23) {
	  load(file.path(wd.rt.data, paste0("icgc_wgs_sv_del_", FILE, "_chr", c, ".RData")))
	  colnames(sv.del.chr.1)[38] <- "RT"
	  colnames(sv.del.chr.2)[38] <- "RT"
...
file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_RT_FS_", FILE, "_1+2.pdf"))
main.text <- "PCAWG Del"
#xlab.text <- expression("log" * ""[10] * "(Gene density)")
xlab.text <- "CpG island"
plotDensity2(sv.del.fs.na$VALUE, sv.del.fs$VALUE, file.name, c("black", red), c("Not in FS", "In FS"), main.text, xlab.text)
...
file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_RT_FS_", FILE, "_1+2_>10kb.pdf"))
main.text <- "PCAWG Del > 10 kb"
xlab.text <- "CpG island"
plotDensity2(subset(sv.del.fs.na, SIZE >= 10000)$VALUE, subset(sv.del.fs, SIZE >= 10000)$VALUE, file.name, c("black", red), c("Not in FS", "In FS"), main.text, xlab.text)

file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_RT_FS_", FILE, "_1+2_<10kb.pdf"))
main.text <- "PCAWG Del < 10 kb"
xlab.text <- "CpG island"
plotDensity2(subset(sv.del.fs.na, SIZE < 10000)$VALUE, NA, file.name, c("black", red), c("Not in FS", "In FS"), main.text, xlab.text)
