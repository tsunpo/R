...
fs <- readTable(file.path(wd.nr3, "intermediate/plain_text", paste0(FILE, ".txt")), header=T, rownames=F, sep="")
...
## Breakpoint 1
sv.del.chr.1$VALUE <- mapply(x = 1:nrow(sv.del.chr.1), function(x) getGenomicPropertyCount(sv.del.chr.1$CHR[x], sv.del.chr.1$START[x], fs))

## Breakpoint 2
sv.del.chr.2$VALUE <- mapply(x = 1:nrow(sv.del.chr.2), function(x) getGenomicPropertyCount(sv.del.chr.2$CHR[x], sv.del.chr.2$START[x], fs))
