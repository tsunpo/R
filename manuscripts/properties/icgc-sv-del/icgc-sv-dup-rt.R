...
wd.icgc     <- file.path(wd, "dev/nr3/data/genome_properties")
...
sv.dup <- subset(sv, class1 == "Tandem Dup")
sv.dup$BED <- NA
sv.dup$BED <- mapply(x = 1:nrow(sv.dup), function(x) getGenomicProperty(sv.dup$CHR[x], sv.dup$START[x], nrds.RT))
sv.dup.nona <- sv.dup[!is.na(sv.dup$BED), ]
sv.dup.nona$RT2 <- nrds.RT[sv.dup.nona$BED,]$RT

file.name <- file.path(wd.rt.plots, paste0("Density_svpos_with_hg19_props_TD_Yang.pdf"))
main.text <- "PCAWG TD (Yang)"
xlab.text <- ""
plotDensity(sv.dup.nona$RT2, file.name, "black", main.text, xlab.text, showMedian=F, max=2)

file.name <- file.path(wd.rt.plots, paste0("Density_svpos_with_hg19_props_TD_Yang_MC.pdf"))
main.text <- "PCAWG TD (Yang)"
xlab.text <- ""
plotDensityMonteCarlo(sv.dup.nona$RT2, nrds.RT, file.name, "black", main.text, xlab.text, showMedian=F, max=2)

save(sv.dup, sv.dup.nona, file=file.path(wd.rt.data, paste0("Density_svpos_with_hg19_props_TD_Yang.RData")))
...
xlab.text <- ""
plotPropertyDensity0(sv.del.nona$RT2, file.name, c(red, blue), main.text, xlab.text)

sv.del.nona.encode1 <- sv.del.nona
...
sv.del.nona.encode2 <- sv.del.nona
...
sv.del$BED <- NA
sv.del$BED <- mapply(x = 1:nrow(sv.del), function(x) getGenomicProperty(sv.del$CHR[x], sv.del$START[x], rt.in))
sv.del.nona <- sv.del[!is.na(sv.del$BED), ]
sv.del.nona$RT2 <- rt[sv.del.nona$BED,]$RT

file.name <- file.path(wd.rt.plots, paste0("Density_svpos_with_hg19_props_Del_LCL_Koren.pdf"))
main.text <- "PCAWG DEL (LCL S/G1 Koren)"
xlab.text <- ""
plotPropertyDensity0(sv.del.nona$RT2, file.name, c(red, blue), main.text, xlab.text)

save(sv.del.nona, file=file.path(wd.rt.data, paste0("Density_svpos_with_hg19_props_Del_LCL_Koren.RData")))
