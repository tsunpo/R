...
for (h in 1:nrow(machines)) {
	  ...
	  #for (rt in 0:3) {	  
	     ...
	     ylim <- c(-1, 1.1)
	     file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RT_IZ_", kb, "kb")
	     plotVio20(wd.rt.plots, file.name, dels.9kb.ctr.iz$SPLINE, dels.10kb.ctr.iz$SPLINE, main=hist, names=c(paste0("Del <", kb, "kb"), paste0("Del >", kb, "kb")), cols=c(red, red), ylim, ylab="SCLC-NL IZ RT")
...
file.name <- "heatmap_table.sv.del.20.result.20.rt.iz"
plotHeatmap(wd.rt.plots, file.name, pvals, c("white", red), height=3, width=10, text="", ylab="IZ RT")
save(kbs, machines, pvals, sizes, file=file.path(wd.rt.data, "table.sv.del.20.result.20.rt.iz.RData"), version=2)
