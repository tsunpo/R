...
plotHeatmap <- function(wd.rt.plots, file.name, data0, cols, height=3, width=10, text="", ylab="") {
	  ...
	  pdf(file.path(wd.rt.plots, paste0(file.name, "_ggplot_clust_RT_SIZE.pdf")), height=3, width=10)
...
pvals <- toTable(0, 4, nrow(table.sv.del.20.result.20), c("RT_RT0", "RT_RT1", "RT_RT2", "RT_RT3"))
rownames(pvals) <- table.sv.del.20.result.20$Histology

sizes <- toTable(0, 4, nrow(table.sv.del.20.result.20), c("RT_RT0", "RT_RT1", "RT_RT2", "RT_RT3"))
...
for (h in 1:nrow(machines)) {
	  ...
	  for (rt in 0:3) {	  
	  	  ...
	  	  for (k in 1:length(kbs)) {
		       ...
		       dels.9kb.ctr <- subset(dels.nona.ctr, size < kb*1000)
		       dels.9kb.ctr.iz <- subset(dels.9kb.ctr, NRFD < 0)
		
		       dels.10kb.ctr <- subset(dels.nona.ctr, size >= kb*1000)
		       dels.10kb.ctr.iz <- subset(dels.10kb.ctr, NRFD < 0)
		
		       ##
		       machines[h, k] <- testU(dels.9kb.ctr.iz$SPLINE, dels.10kb.ctr.iz$SPLINE)
	     }
	     ...
	     #file.name <- file.path(wd.rt.plots, paste0("correlation_LENGTH_P_", hist, "_RT_IZ"))
	     ...
	     #dels.10kb <- subset(dels.nona.ctr, size >= kb*1000)
	     #dels.10kb.ctr.iz <- subset(dels.10kb.ctr, NRFD < 0)
	     
	     #ylim <- c(-1, 1.1)
	     #file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RT_TZ_", kb, "kb")
	     #plotVio20(wd.rt.plots, file.name, dels.9kb.ctr.iz$SPLINE, dels.10kb.ctr.iz$SPLINE, main=hist, names=c(paste0("Del <", kb, "kb"), paste0("Del >", kb, "kb")), cols=c(blue, blue), ylim, ylab="SCLC-NL IZ RT")
...
file.name <- "heatmap_table.sv.del.20.result.20.rt.tz"
plotHeatmap(wd.rt.plots, file.name, pvals, c("white", red), height=3, width=10, text="", ylab="TZ RT")
save(kbs, machines, pvals, sizes, file=file.path(wd.rt.data, "table.sv.del.20.result.20.rt.tz.RData"), version=2)
