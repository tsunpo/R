...
for (p in 1:2) {
	  ...
	  file.name <- file.path(wd.rt.plots, paste0("Density_Recombination_RT_", FILE[p], ".pdf"))
...
nrds.ttr <- getBootstrapTTR(nrds.RT.NRFD[overlaps, ], 0.9)
nrds.ctr <- getBootstrapCTR(nrds.RT.NRFD[overlaps, ], 0.9)
nrds.ctr.iz <- subset(nrds.ctr, NRFD >= 0)
nrds.ctr.tz <- subset(nrds.ctr, NRFD < 0)

overlaps <- intersect(rownames(nrds.ttr), map.nona$BED)
nrds.ttr <- cbind(map.nona[overlaps, ], nrds.ttr[overlaps, ])
overlaps <- intersect(rownames(nrds.ctr.tz), map.nona$BED)
nrds.ctr.tz <- cbind(map.nona[overlaps, ], nrds.ctr.tz[overlaps, ])
overlaps <- intersect(rownames(nrds.ctr.iz), map.nona$BED)
nrds.ctr.iz <- cbind(map.nona[overlaps, ], nrds.ctr.iz[overlaps, ])
...
for (p in 1:2) {
	  main.text <- "Recombination (HapMap II)"
	  file.name <- file.path(wd.rt.plots, paste0("Density_Recombination_RFD_", FILE[p], ".pdf"))
	  plotDensity2(nrds.ttr[, FILE[p]], nrds.ctr.tz[, FILE[p]], nrds.ctr.iz[, FILE[p]], file.name, cols, legends, main.text, xlab.text[p], min=min[p], max=max[p])
	  ...
	  plotCorrelation5(file.name, main.text, FILE[p], "RT (TZ)", nrds.ctr.iz[, FILE[p]], nrds.ctr.iz$RT, pos="bottomright", cols=c(red.lighter, red))
}
...
plotDensity(subset(sv.del.nona, SIZE < 10000)$VALUE, file.name, col="black", main.text, xlab.text[p], min=min[p], max=max[p], rev=rev[p])
   ...
   ## Total dels
   main.text <- ""
   file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_FS_", FILE[p], ".pdf"))
   plotDensity2(sv.del.fs.na$VALUE, sv.del.fs$VALUE, file.name, cols, legends, main.text, xlab.text[p], min=min[p], max=max[p], rev=rev[p])

   file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_", FILE[p], ".pdf"))
   plotDensity(sv.del.nona$VALUE, file.name, col="black", main.text, xlab.text[p], min=min[p], max=max[p], rev=rev[p])

   ## Del <10 kb
   main.text <- ""
   file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_FS_", FILE[p], "_>10kb.pdf"))
   plotDensity2(subset(sv.del.fs.na, SIZE >= 10000)$VALUE, subset(sv.del.fs, SIZE >= 10000)$VALUE, file.name, cols, legends, main.text, xlab.text[p], min=min[p], max=max[p], rev=rev[p])
...
##
file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_FS_", FILE[3], "_>10kb.pdf"))
main.text <- "PCAWG Del > 10 kb"
xlab.text <- "Gene density"
plotDensity2(subset(sv.del.fs.na, SIZE >= 10000)$VALUE, subset(sv.del.fs, SIZE >= 10000)$VALUE, file.name, c("black", red), c("Not in FS", "In FS"), main.text, xlab.text, rev=F)

file.name <- file.path(wd.rt.plots, paste0("Density_SV_Del_", FILE[3], "_>10kb.pdf"))
plotDensity(subset(sv.del, SIZE >= 10000)$VALUE, file.name, col="black", main.text, xlab.text)
...
# -----------------------------------------------------------------------------
file.name <- file.path(wd.rt.plots, paste0("Cor_SV_Del_RT_FS_1+2_Yang.pdf"))
xlab.text <- expression("log" * ""[10] * "(Size)")
plotCorrelation5(file.name, "PCAWG Del", xlab.text, "RT (Yang et al.)", log10(sv.del.nona$SIZE), sv.del.nona$RT2, pos="bottomright", cols=c("dimgray", "black"))

file.name <- file.path(wd.rt.plots, paste0("Cor_SV_Del_RT_FS_1+2_Yang_>10kb.pdf"))
xlab.text <- expression("log" * ""[10] * "(Size)")
plotCorrelation5(file.name, "PCAWG Del > 10 kb", xlab.text, "RT (Yang et al.)", log10(subset(sv.del.nona, SIZE >= 10000)$SIZE), subset(sv.del.nona, SIZE >= 10000)$RT2, pos="bottomright", cols=c("dimgray", "black"))
...
colnames(nrds.RT) <- c("SCLC-NL", "SCLC", "NB", "CLL")
nrds.RT$RT <- mapply(x = 1:nrow(nrds.RT), function(x) median(as.numeric(nrds.RT[x, 1:4])))
...
file.name <- file.path(wd.rt.plots, paste0("QS_RT_TOTAL.pdf"))
main.text <- "RT"
xlab.text <- ""
plotPropertyDensity0(as.numeric(unlist(nrds.RT[, 1:4])), file.name, c(red, blue), main.text, xlab.text)

file.name <- file.path(wd.rt.plots, paste0("QS_RT_TOTAL_-1.pdf"))
main.text <- "RT"
xlab.text <- ""
plotPropertyDensity0(as.numeric(unlist(nrds.RT.1[, 1:4])), file.name, c(red, blue), main.text, xlab.text)

file.name <- file.path(wd.rt.plots, paste0("QS_RT_TOTAL_-2.pdf"))
main.text <- "RT"
xlab.text <- ""
plotPropertyDensity0(as.numeric(unlist(nrds.RT.2[, 1:4])), file.name, c(red, blue), main.text, xlab.text)
...
nrds.RT.NRFD.nona.2 <- subset(subset(nrds.RT.NRFD, RT >= -2), RT <= 2)
nrds.RT.NRFD.nona.1 <- subset(subset(nrds.RT.NRFD, RT >= -1), RT <= 1)
nrow(nrds.RT.NRFD)
# [1] 2651861
nrow(nrds.RT.NRFD.nona.2)
# [1] 2540339
(2651861 - 2540339) / 2651861
# [1] 0.04205424
nrow(nrds.RT.NRFD.nona.1)
# [1] 1934794

nrds.RT.NRFD.nona <- nrds.RT.NRFD.nona.2

file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_-2.pdf"))
...
#dels.all.auto <- subset(subset(dels.all, chrom2 != "X"), chrom2 != "Y")
dels.all.nona <- dels.all[!is.na(dels.all$BED),]
dels.all.nona$RT <- nrds.RT.NRFD.nona[dels.all.nona$BED,]$RT
dels.all.nona <- dels.all.nona[!is.na(dels.all.nona$RT),]

## PCAWG deletions (ALL)
tests <- dels.all.nona$RT
randoms <- replicate(1000, getMonteCarloSimulations(nrds.RT.NRFD.nona, length(tests)))

file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_PCAWG.pdf"))
main.text <- "PCAWG deletions"
xlab.text <- ""
plotMonteCarloSimulation(tests, randoms, file.name, c(red, blue), main.text, xlab.text)

file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_PCAWG_P.pdf"))
plotPropertyDensity(tests, randoms, file.name, c(red, blue), main.text, xlab.text)
...
## PCAWG deletions (< 10 kb)
tests <- subset(dels.all.nona, size < 10000)$SCALE
randoms <- replicate(1000, getMonteCarloSimulations(nrds.RT.NRFD.nona, length(tests)))

file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_PCAWG_<10kb.pdf"))
main.text <- "PCAWG deletions < 10 kb"
xlab.text <- ""
plotMonteCarloSimulation(tests, randoms, file.name, c(red, blue), main.text, xlab.text)

file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_PCAWG_<10kn_P.pdf"))
plotPropertyDensity(tests, randoms, file.name, c(red, blue), main.text, xlab.text)
...
file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_SCALE_PCAWG_>10kb.pdf"))
main.text <- "PCAWG deletions > 10 kb"
xlab.text <- ""
plotMonteCarloSimulation(tests, randoms, file.name, c(red, blue), main.text, xlab.text)

file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_SCALE_PCAWG_>10kn_P.pdf"))
plotPropertyDensity(tests, randoms, file.name, c(red, blue), main.text, xlab.text)




## PCAWG deletions (Test)
icgc.idx <- sample(nrow(dels.all.auto), 1436, replace=F)
icgc.del <- dels.all.auto[icgc.idx, ]

icgc.del$BED <- NA
icgc.del$BED <- mapply(x = 1:nrow(icgc.del), function(x) getRandomBreakpointBED(icgc.del[x,], nrds.RT.NRFD.nona, bed.gc))
icgc.del.nona <- icgc.del[!is.na(icgc.del$BED), ]

tests <- nrds.RT.NRFD.nona[icgc.del.nona$BED, "SPLINE"]
randoms <- replicate(1000, getMonteCarloSimulations(nrds.RT.NRFD.nona, length(tests)))
...
file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_SPLINE_ggplot2.pdf"))
pdf(file.name, height=6, width=6)
p <- ggplot(nrds.RT.NRFD, aes(x=SPLINE)) + geom_density()
p
dev.off()

p
p + geom_vline(aes(xintercept=mean(weight)),	color="black", linetype="dashed", size=1)

nrds.RT.NRFD.nona <- subset(subset(nrds.RT.NRFD, RT >= -1 ), RT <= 1)


max <- max(nrds.RT.NRFD.nona$RT)
nrds.RT.NRFD.nona$SCALE <- 0
idx <- which(nrds.RT.NRFD.nona$RT >= 0)
nrds.RT.NRFD.nona$SCALE[idx] <- nrds.RT.NRFD.nona$RT[idx]/max
idx <- which(nrds.RT.NRFD.nona$RT < 0)
nrds.RT.NRFD.nona$SCALE[idx] <- nrds.RT.NRFD.nona$RT[idx]/max

##
file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_SCALE_-1.pdf"))
main.text <- "SCLC-NL SCALE"
xlab.text <- ""
plotPropertyDensity0(nrds.RT.NRFD.nona$SCALE, file.name, c(red, blue), main.text, xlab.text)




nrds.RT.NRFD.nona <- subset(subset(nrds.RT.NRFD, SPLINE >= -2 ), SPLINE <= 2)
nrds.RT.NRFD.nona <- nrds.RT.NRFD[!is.na(nrds.RT.NRFD$RT),]
nrds.RT.NRFD.nona$SCALE <- rescale(nrds.RT.NRFD.nona$RT)
...
file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_SPLINE_-2.pdf"))
main.text <- "SCLC-NL SPLINE"
xlab.text <- ""
plotPropertyDensity0(nrds.RT.NRFD.nona$SPLINE, file.name, c(red, blue), main.text, xlab.text)
...
file.name <- file.path(wd.rt.plots, paste0("QS_SCLC-NL_RT_-2_1437_1000.pdf"))
main.text <- "Simulated 1,437 pos 1,000 times"
xlab.text <- ""
...
#dels.all.auto <- subset(subset(dels.all, chrom2 != "X"), chrom2 != "Y")
dels.all.nona <- dels.all[!is.na(dels.all$BED),]
dels.all.nona$SCALE <- nrds.RT.NRFD.nona[dels.all.nona$BED, ]$SCALE
...
list1 <- readTable(file.path(wd.meta, paste0("icgc.sv.list")), header=F, rownames=F, sep="")
list2 <- readTable(file.path(wd.meta, paste0("tcga.sv.list")), header=F, rownames=F, sep="")
list <- c(list1, list2)
length(list)
# [1] 2748
...
length(overlaps)
# [1] 2567
release.sv <- release[overlaps,]
#rownames(release.sv) <- release.sv$tumor_wgs_icgc_specimen_id

overlaps <- intersect(rownames(totals), rownames(release.sv))
length(overlaps)
# [1] 2565
...
table.sv.del <- toTable(0, 3, nrow(table.sv), c("Histology", "Sample", "Deletion"))
table.sv.del$Histology <- table.sv$Var1
for (h in 1:nrow(table.sv.del)) {
	  hist <- as.vector(table.sv.del$Histology[h])
	  totals.sv.hist <- subset(totals.sv, histology_abbreviation == hist)
	  ...
	  for (s in 1:nrow(totals.sv.hist)) {
	  	  ...
	  	  sv.del <- subset(sv, svclass == "DEL")
	  	  if (nrow(sv.del) > 0) {
	  	  	  n <- n + 1
	  	  	  d <- d + nrow(sv.del)
		  	 }
	  }
	  table.sv.del$Sample[h] <- n
...
for (h in 1:nrow(table.sv.del)) {
	  hist <- as.vector(table.sv.del$Histology[h])
	  dir.create(file.path(wd.icgc.sv, "del", "data", hist))
	  totals.sv.hist <- subset(totals.sv, histology_abbreviation == hist)
	  ...
	  for (s in 1:nrow(totals.sv.hist)) {
		    ...
		    sv <- readTable(file.path(wd.icgc.sv, "final", paste0(id, ".pcawg_consensus_1.6.161116.somatic.sv.bedpe.gz")), header=T, rownames=F, sep="\t")
		    sv.del <- subset(sv, svclass == "DEL")
		    writeTable(sv.del, gzfile(file.path(wd.icgc.sv, "del", "data", hist, paste0(id2, ".somatic.sv.del.bedpe.gz"))), colnames=T, rownames=F, sep="\t")
	  }
}
...
for (h in 1:nrow(table.sv.del.20)) {
	  ...
	  for (d in 1:nrow(dels)) {
	  	  overlaps <- getDELRTNRFD(dels[d,], nrds.RT.NRFD.sclc.nl, bed.gc)
	  	  
	  	  dels$RT[d]   <- median(nrds.RT.NRFD.sclc.nl[overlaps,]$RT)
	  	  dels$RFD[d]  <- median(nrds.RT.NRFD.sclc.nl[overlaps,]$RFD)
	  	  dels$NRFD[d] <- median(nrds.RT.NRFD.sclc.nl[overlaps,]$NRFD)
	  }
	  
	  save(dels, file=file.path(wd.rt.data, paste0(hist, ".sv.del.RData")), version=2)
}
...
# -----------------------------------------------------------------------------
rt <- 0
table.sv.del.20.result.rt0 <- toTable(0, 6, nrow(table.sv.del.20), c("TTR_9kb", "TZ_9kb", "IZ_9kb", "TTR_10kb", "TZ_10kb", "IZ_10kb"))
rownames(table.sv.del.20.result.rt0) <- table.sv.del.20$Histology
...
for (h in 1:nrow(table.sv.del.20)) {
	  ...
	  dels.9kb <- subset(dels.nona, size < 10000)
	  dels.9kb.ttr <- getBootstrapTTR(dels.9kb, 0.9)
	  dels.9kb.ctr <- getBootstrapCTR(dels.9kb, 0.9)
	  dels.9kb.ctr.iz <- subset(dels.9kb.ctr, NRFD >= 0)
	  dels.9kb.ctr.tz <- subset(dels.9kb.ctr, NRFD < 0)
	  
	  dels.10kb <- subset(dels.nona, size >= 10000)
	  dels.10kb.ttr <- getBootstrapTTR(dels.10kb, 0.9)
	  dels.10kb.ctr <- getBootstrapCTR(dels.10kb, 0.9)
	  dels.10kb.ctr.iz <- subset(dels.10kb.ctr, NRFD >= 0)
	  dels.10kb.ctr.tz <- subset(dels.10kb.ctr, NRFD < 0)
	  
	  table.sv.del.20.result.rt0[h, ]$TTR_9kb <- nrow(dels.9kb.ttr)
	  table.sv.del.20.result.rt0[h, ]$TZ_9kb  <- nrow(dels.9kb.ctr.tz)
	  table.sv.del.20.result.rt0[h, ]$IZ_9kb  <- nrow(dels.9kb.ctr.iz)
	  
	  table.sv.del.20.result.rt0[h, ]$TTR_10kb <- nrow(dels.10kb.ttr)
	  table.sv.del.20.result.rt0[h, ]$TZ_10kb  <- nrow(dels.10kb.ctr.tz)
	  table.sv.del.20.result.rt0[h, ]$IZ_10kb  <- nrow(dels.10kb.ctr.iz)
}

writeTable(table.sv.del.20.result.rt0, file.path(wd.icgc.sv, "del", "table.sv.del.20.result.rt0.txt"), colnames=T, rownames=T, sep="\t")
save(table.sv.del.20.result.rt0, table.sv.del.20.result.rt1, table.sv.del.20.result.rt2, table.sv.del.20.result.rt3, file=file.path(wd.rt.data, "table.sv.del.20.result.rts.RData"), version=2)
...
for (h in 1:nrow(table.sv.del.20.result.10)) {
	  hist <- table.sv.del.20.result.10$Histology[h]
  	...
  	## Violin plot
	  #ylim <- c(-1, 1.1)
	  #file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RT")
	  #plotVio20(wd.rt.plots, file.name, dels.9kb$SPLINE, dels.10kb$SPLINE, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="RT")
	
	  #ylim <- c(-1, 1.1)
	  #file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RFD")
	  #plotVio20(wd.rt.plots, file.name, dels.9kb$RFD, dels.10kb$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="RFD")
	
	  #ylim <- c(-1, 1.1)
	  #file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RFD_TTR")
	  #plotVio20(wd.rt.plots, file.name, dels.9kb.ttr$RFD, dels.10kb.ttr$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="SCLC-NL TTR RFD")
	
	  #ylim <- c(-1, 1.1)
	  #file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RFD_CTR_TZ")
	  #plotVio20(wd.rt.plots, file.name, dels.9kb.ctr.tz$RFD, dels.10kb.ctr.tz$RFD, dels.9kb.ctr.tz$NRFD, dels.10kb.ctr.tz$NRFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c(blue, blue), ylim, ylab="SCLC-NL TZ RFD")
	
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RFD_CTR_IZ")
	  plotVio20(wd.rt.plots, file.name, dels.9kb.ctr.iz$RFD, dels.10kb.ctr.iz$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c(red, red), ylim, ylab="SCLC-NL IZ RFD")

	  ##
	  #file.name <- paste0("corr_", hist, "_rt", rt, "_SIZE-VS-IZ_10kb")
	  #plotCorrelation5(file.name, hist, "Size (Del >10kb) [kb]", "SCLC-NL RFD", dels.10kb.ctr.iz$size/1000, dels.10kb.ctr.iz$RFD, pos="bottomright", cols=c(red, lightblue))

	  #file.name <- paste0("corr_", hist, "_rt", rt, "_SIZE-VS-IZ_ALL")
	  #plotCorrelation5(file.name, hist, "Size [kb]", "SCLC-NL RFD", dels$size/1000, dels$RFD, pos="bottomright", cols=c("dimgray", "black"))
}
...
# Last Modified: 25/04/23
# -----------------------------------------------------------------------------
table.sv.del.20.result.iz <- toTable(0, 4, nrow(table.sv.del.20), c("IZ_RT0", "IZ_RT1", "IZ_RT2", "IZ_RT3"))
rownames(table.sv.del.20.result.iz) <- table.sv.del.20$Histology

table.sv.del.20.result.tz <- toTable(0, 4, nrow(table.sv.del.20), c("TZ_RT0", "TZ_RT1", "TZ_RT2", "TZ_RT3"))
rownames(table.sv.del.20.result.tz) <- table.sv.del.20$Histology

table.sv.del.20.result.ttr <- toTable(0, 4, nrow(table.sv.del.20), c("TTR_RT0", "TTR_RT1", "TTR_RT2", "TTR_RT3"))
rownames(table.sv.del.20.result.ttr) <- table.sv.del.20$Histology

table.sv.del.20.result.rt <- toTable(0, 4, nrow(table.sv.del.20), c("RT_RT0", "RT_RT1", "RT_RT2", "RT_RT3"))
rownames(table.sv.del.20.result.rt) <- table.sv.del.20$Histology

table.sv.del.20.result.rfd <- toTable(0, 4, nrow(table.sv.del.20), c("RT_RT0", "RT_RT1", "RT_RT2", "RT_RT3"))
rownames(table.sv.del.20.result.rfd) <- table.sv.del.20$Histology
...
for (h in 1:nrow(table.sv.del.20)) {
	  ...
	  for (rt in 0:3) {	  
	     ...
	     ##
	     #table.sv.del.20.result.ttr[h, rt+1] <- testU(dels.9kb.ttr$RFD, dels.10kb.ttr$RFD)
	     #table.sv.del.20.result.tz[h, rt+1] <- testU(dels.9kb.ctr.tz$RFD, dels.10kb.ctr.tz$RFD)
	     #table.sv.del.20.result.iz[h, rt+1] <- testU(dels.9kb.ctr.iz$RFD, dels.10kb.ctr.iz$RFD)

	     table.sv.del.20.result.rfd[h, rt+1] <- testU(dels.9kb$RFD, dels.10kb$RFD)	     
	     #table.sv.del.20.result.rt[h, rt+1] <- testU(dels.9kb$SPLINE, dels.10kb$SPLINE)
	  }
}

save(table.sv.del.20.result.rt, table.sv.del.20.result.rfd, table.sv.del.20.result.iz, table.sv.del.20.result.tz, table.sv.del.20.result.ttr, file=file.path(wd.rt.data, "table.sv.del.20.results.SPLINE.FDR.RData"), version=2)

# -----------------------------------------------------------------------------
# 
# Last Modified: 02/05/23
# -----------------------------------------------------------------------------
table.sv.del.20.result.20 <- table.sv.del.20[-c(9,10,19),]
table.sv.del.20.result.20 <- table.sv.del.20.result.20[order(table.sv.del.20.result.20$Deletion, decreasing=T),]

table.sv.del.20.result.rt0.20 <- table.sv.del.20.result.rt0[-c(9,10,19),]
table.sv.del.20.result.rt1.20 <- table.sv.del.20.result.rt1[-c(9,10,19),]
table.sv.del.20.result.rt2.20 <- table.sv.del.20.result.rt2[-c(9,10,19),]
table.sv.del.20.result.rt3.20 <- table.sv.del.20.result.rt3[-c(9,10,19),]

table.sv.del.20.result.iz.20  <- table.sv.del.20.result.iz[-c(9,10,19),]
table.sv.del.20.result.tz.20  <- table.sv.del.20.result.tz[-c(9,10,19),]
table.sv.del.20.result.ttr.20 <- table.sv.del.20.result.ttr[-c(9,10,19),]

table.sv.del.20.result.rt.20  <- table.sv.del.20.result.rt[-c(9,10,19),]
table.sv.del.20.result.rfd.20 <- table.sv.del.20.result.rfd[-c(9,10,19),]

#heatmap(data, Colv = NA, Rowv = NA, scale="column")

writeTable(table.sv.del.20.result.iz.20, file.path(wd.rt.data, "sv.del.20.result.iz.20.txt"), colnames=T, rownames=T, sep="\t")
writeTable(table.sv.del.20.result.tz.20, file.path(wd.rt.data, "sv.del.20.result.tz.20.txt"), colnames=T, rownames=T, sep="\t")
writeTable(table.sv.del.20.result.ttr.20, file.path(wd.rt.data, "sv.del.20.result.ttr.20.txt"), colnames=T, rownames=T, sep="\t")

# -----------------------------------------------------------------------------
# Heatmap (IZ)
# Last Modified: 25/04/23
# -----------------------------------------------------------------------------
plotHeatmap <- function(wd.rt.plots, file.name, data0, cols, height=10, width=5, text="", ylab="") {
	  data <- as.matrix(data0[order(data0[,1], decreasing=F),])
	  X <- rownames(data)
	  Y <- c("SCLC-NL", "SCLC", "NB", "CLL")
	  
	  colnames(data) <- Y
	  pdf(file.path(wd.rt.plots, paste0(file.name, "_RT.pdf")), height=10, width=10)
	  par(mar=c(3.4, 4.5, 2.1, 6.5))
	  
	  heatmap(-log10(data),	col=c("white", red),	main="Del <10kb vs. Del >10kb in IZ domains", xlab="", ylab="", scale="row")
	  #margins = c(5,10)
	  
	  #text(labels=X, x=1:19, y=par("usr")[3] - 0.15, srt=45, adj=0.965, xpd=NA, cex=1.4)
	  dev.off()
}

plotHeatmap <- function(wd.rt.plots, file.name, data0, cols, height=3, width=10, text="", ylab="") {
	  #data0 <- data0[order(data0[, 1], decreasing=F),]
	  X <- rownames(data0)
	  Y <- c("SCLC-NL", "SCLC", "NB", "CLL")
	  
	  data <- toTable(0, 3, nrow(data0), c("X", "Y", "P"))
	  data$X <- X
	  data$Y <- Y[4]
	  data$P <- -log10(data0[, 4])
	  for (rt in 3:1) {
	  	  data1 <- toTable(0, 3, nrow(data0), c("X", "Y", "P"))
	    	data1$X <- X
	  	  data1$Y <- Y[rt]
	  	  data1$P <- -log10(data0[, rt]) 
	  	  
	  	  data <- rbind(data, data1)
	  }
	
	  # Cluster based on euclidean distance
	  m <- tidyr::pivot_wider(data, names_from="X", values_from="P")
	  ...
	  plt <- ggplot(data, aes(x = X, y = Y, fill = P)) + geom_tile() + coord_fixed()
	  ...
	  #plt <- plt + theme(axis.title = element_text(size = 11))
	  
	  # setting the title and subtitles using title and subtitle
	  #plt <- plt + labs(title = "Heatmap")
	  #plt <- plt + labs(subtitle = "A simple heatmap using geom_tile()")
	
	  # setting x and y labels using labs
	  plt <- plt + labs(x = "", y = ylab) + guides(fill = guide_colourbar(title = text.Log10.P))
...
file.name <- file.path(wd.rt.plots, paste0("correlation_LENGTH_P_", "ALL", "_IZ"))
plotOS(file.name, "Deletions in IZ", "Size [kb]", text.Log10.P, x, y, machines[h, min], kb, lwd=3)
...
ylim <- c(-1, 1.1)
file.name <- paste0("vioplot_", "ALL", "_DEL_RFD_CTR_IZ_", kb, "kb")
plotVio20(wd.rt.plots, file.name, dels.9kb.ctr.iz.all$RFD, dels.10kb.ctr.iz.all$RFD, main="Deletions in IZ", names=c(paste0("Del <", kb, "kb"), paste0("Del >", kb, "kb")), cols=c(red, red), cols2=c(orange, lightblue), ylim, ylab="RFD")
...
save(kbs, machines, pvals, sizes, file=file.path(wd.rt.data, "table.sv.del.20.result.20.rfd.iz.RData"), version=2)
	  ...
	  ## Boxplot
	  ylim <- c(-2, 2.15)
	  file.name <- paste0("boxplot_", hist, "_DEL_RT")
	  plotBox20(wd.icgc.sv.plots, file.name, dels.9kb$RT, dels.10kb$RT, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="RT")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("boxplot_", hist, "_DEL_RFD")
	  plotBox20(wd.icgc.sv.plots, file.name, dels.9kb$RFD, dels.10kb$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="RFD")

	  ylim <- c(-1, 1.1)
	  file.name <- paste0("boxplot_", hist, "_DEL_RFD_TTR")
	  plotBox20(wd.icgc.sv.plots, file.name, dels.9kb.ttr$RFD, dels.10kb.ttr$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="TTR RFD")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("boxplot_", hist, "_DEL_RFD_CTR_IZ")
	  plotBox20(wd.icgc.sv.plots, file.name, dels.9kb.ctr.iz$RFD, dels.10kb.ctr.iz$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="IZ RFD")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("boxplot_", hist, "_DEL_RFD_CTR_TZ")
	  plotBox20(wd.icgc.sv.plots, file.name, dels.9kb.ctr.tz$RFD, dels.10kb.ctr.tz$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="TZ RFD")

	  ## Violin plot
	  ylim <- c(-2, 2.15)
	  file.name <- paste0("vioplot_", hist, "_DEL_RT")
	  plotVio20(wd.icgc.sv.plots, file.name, dels.9kb$RT, dels.10kb$RT, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="RT")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("vioplot_", hist, "_DEL_RFD")
	  plotVio20(wd.icgc.sv.plots, file.name, dels.9kb$RFD, dels.10kb$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="RFD")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("vioplot_", hist, "_DEL_RFD_TTR")
	  plotVio20(wd.icgc.sv.plots, file.name, dels.9kb.ttr$RFD, dels.10kb.ttr$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="TTR RFD")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("vioplot_", hist, "_DEL_RFD_CTR_IZ")
	  plotVio20(wd.icgc.sv.plots, file.name, dels.9kb.ctr.iz$RFD, dels.10kb.ctr.iz$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="IZ RFD")
	  
	  ylim <- c(-1, 1.1)
	  file.name <- paste0("vioplot_", hist, "_DEL_RFD_CTR_TZ")
	  plotVio20(wd.icgc.sv.plots, file.name, dels.9kb.ctr.tz$RFD, dels.10kb.ctr.tz$RFD, main=hist, names=c("Del <10kb", "Del >10kb"), cols=c("black", "black"), ylim, ylab="TZ RFD")


chr <- del$chrom1
bed.gc.chr <- subset(bed.gc, CHR == paste0("chr", chr))
start_position <- del$start1
end_position   <- del$end2

size  <- 1000000
start <- start_position + size
end   <- start_position - size

file.name <- file.path(wd.icgc.sv.plots, paste0("NRFD_", paste0("SCLC\u2212NL-", hist), "_", method, ".d.rt.log2s_", chr, "_", PAIR1, "-", PAIR0, "_n", n1, "-", n0))
plotBootstrapRFD(file.name, paste0("SCLC\u2212NL"), chr, end, start, nrds.RT.NRFD.sclc.nl, bed.gc.chr, boundary.upper, boundary.lower, "png", width=6, kb, F)

file.name <- file.path(wd.icgc.sv.plots, paste0("NRFD_", paste0("SCLC-", hist), "_", method, ".d.rt.log2s_", chr, "_", PAIR1, "-", PAIR0, "_n", n1, "-", n0))
plotBootstrapRFD(file.name, paste0("SCLC"), chr, end, start, nrds.RT.NRFD.sclc, bed.gc.chr, boundary.upper, boundary.lower, "png", width=6, kb, F)

file.name <- file.path(wd.icgc.sv.plots, paste0("NRFD_", paste0("NB-", hist), "_", method, ".d.rt.log2s_", chr, "_", PAIR1, "-", PAIR0, "_n", n1, "-", n0))
plotBootstrapRFD(file.name, paste0("NB"), chr, end, start, nrds.RT.NRFD.nbl, bed.gc.chr, boundary.upper, boundary.lower, "png", width=6, kb, F)
...
file.name <- paste0("de_", base, "_cna-gene_SG1_wilcox_q_n2443")
...
plotVolcano <- function(de, pvalue, genes, file.de, file.main, xlab.text, fold=1, ymax=0, line=0) {
   ...
   legend("topright", legend=c(paste0(nrow(de.up), " Proliferating"), paste0("    ", nrow(de.down), " Resting")), col=c(red, blue), pch=19, cex=1.2)
   dev.off()
}

## ICGC expressed genes
xlab.text <- "Proliferating/Resting difference"
plot.de <- file.path(wd.driver.plots, "volcanoplot_icgc_cna-gene_cn.seg_DE_p1e-19")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("Differential CNA", "Sclust (cn.seg)")
plotVolcano(de.cna.gene, 1E-19, genes, file.de, file.main, xlab.text, line=2.85, fold=0.02)
