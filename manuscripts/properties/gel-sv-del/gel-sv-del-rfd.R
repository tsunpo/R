...
wd.rt.data  <- file.path(wd.pt, "data")
wd.rt.plots <- file.path(wd.pt, "plots")
...
# -----------------------------------------------------------------------------
getMonteCarloSimulations <- function(nrds.RT.NRFD.nona, size) {
	  random.beds <- sort(sample(1:nrow(nrds.RT.NRFD.nona), size, replace=T))
	
	  return(median(nrds.RT.NRFD.nona[random.beds, "SPLINE"]))
}
...
reals <- nrds.RT.NRFD.nona[gel.del.nona$BED, "SPLINE"]
...
reals.10k <- nrds.RT.NRFD.nona[gel.del.10kb$BED, "SPLINE"]
...
reals.9k <- nrds.RT.NRFD.nona[gel.del.9kb$BED, "SPLINE"]
