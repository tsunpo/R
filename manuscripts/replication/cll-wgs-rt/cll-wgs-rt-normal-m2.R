...
samples1 <- readTable(file.path(wd.ngs, "cll_wgs_n96_N.txt"), header=T, rownames=T, sep="")   ## M2/M1
samples1 <- subset(samples1, M2 == 1)[,1]
samples0 <- readTable(file.path(wd.ngs, "cll_wgs_n96_N.txt"), header=T, rownames=T, sep="")
...
for (c in 1:22) {
   ...
   main.text <- c(paste0("CLL (WB) read depth correlation (", "Chr", c, ")"), paste0("rho = ", round0(sprs$cor[c], digits=2), " (Q3 vs. Q1)"))
   xlab.text <- "CLL (WB) M2/M1"
   ylab.text <- "CLL (WB) read depth [RPKM]"
...
main.text <- paste0("CLL (WB) M2/M1 vs. LCL S/G1")
ymin <- 0.25
ymax <- 1.05
