...
for (c in 1:22) {
   ...
   main.text <- c(paste0("CLL read depth correlation (", "Chr", c, ")"), paste0("rho = ", round0(cor, digits=2), " (Q3 vs. Q1)"))
   xlab.text <- "CLL Q3/Q1"
   ylab.text <- "CLL read depth [RPKM]"
   file.name <- file.path(wd.rt.plots, "chrs", paste0("RD-vs-RT_CLL-Q3-Q1_chr", c, "_spline_spearman"))
...
ylab.text <- "SPR"
file.name <- file.path(wd.rt.plots, "SPR_CLL-Q3-Q1_spline_spearman")
...
file.name <- file.path(wd.rt.plots, "SPR-RDC_CLL-Q3-Q1_spline_spearman")
...
file.name <- file.path(wd.rt.plots, "SPR-Woodfine_CLL-Q3-Q1_spline_spearman")
...
file.name <- file.path(wd.rt.plots, "RT-vs-RT_CLL-Q3-Q1-vs-LCL-S-G1_spline_spearman")
main.text <- paste0("CLL Q3/Q1 vs. LCL S/G1")
ymin <- 0.25
ymax <- 1.05
plotRTvsRTALL(cors, file.name, main.text, ylab.text, xlab.text, ymin, ymax, col="black", c=2, pos=1)

##
file.name <- file.path(wd.rt.plots, "RTD-vs-RT_CLL-Q3-Q1-vs-LCL-S-G1_spline_spearman")
ymin <- -1.1
ymax <- 1.1
