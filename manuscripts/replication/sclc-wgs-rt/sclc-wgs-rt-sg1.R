...
# Name         : manuscripts/replication/sclc-wgs-rt-sg1.R
...
samples1 <- readTable(file.path(wd.ngs, "sclc_wgs_n101.sg1"), header=T, rownames=T, sep="")
samples1 <- subset(samples1, M2 == 1)[,1]
samples0 <- readTable(file.path(wd.ngs, "sclc_wgs_n101.sg1"), header=T, rownames=T, sep="")
...
file.name <- file.path(wd.rt.plots, "plot_RT-vs-RT_SCLC-SL-G1L-vs-LCL-S-G1_spline_spearman")
main.text <- paste0("SCLC SL/G1L vs. LCL S/G1")
ymin <- 0.25
ymax <- 1.05
...
for (c in 1:22) {
   ...
   main.text <- paste0("SCLC read depths vs. SCLC SL/G1L (", "Chr", c, ")")
   xlab.text <- "SCLC SL/G1L"
...
main.text <- c(paste0("SCLC SL vs. SCLC G1L"), "Linear regression")
ymax <- 0.01505
xlim <- c(-0.18, 1)
