...
# Name         : manuscripts/replication/sclc-wgs-rt-q3.R
...
wd.rt       <- file.path(wd.anlys, "replication", paste0(base, "-wgs-rt-q3"))
wd.rt.data  <- file.path(wd.rt, "data")
wd.rt.plots <- file.path(wd.rt, "plots")

#samples1 <- readTable(file.path(wd.ngs, "sclc_wgs_n101.txt"), header=T, rownames=T, sep="")   ## M2/M1
samples1 <- readTable(file.path(wd.ngs, "sclc_wgs_q3_n51.txt"), header=T, rownames=T, sep="")    ## Q3/Q1
samples1 <- subset(samples1, M2 == 1)[,1]
#samples0 <- readTable(file.path(wd.ngs, "sclc_wgs_n101.txt"), header=T, rownames=T, sep="")
samples0 <- readTable(file.path(wd.ngs, "sclc_wgs_q3_n51.txt"), header=T, rownames=T, sep="")
...
for (c in 1:22) {
   ...
   ## Plot RT
   main.text <- paste0(BASE, " Q3/Q1 read depth ratio between tumour (n=", n1, ") and tumour (n=", n0, ") samples")  
   file.name <- file.path(wd.rt.plots, paste0("RT_", base, "_", method, ".d.rt_", chr, "_", PAIR1, "-", PAIR0, "_n", n1, "-", n0, ""))   
   plotRT(file.name, main.text, chr, NA, NA, nrds.chr, bed.gc.chr, c("red", "blue"), c("Q3 tumour", "Q1 tumour"), c("lightcoral", "lightskyblue3"), c("Q3", "Q1"), "png", width=10, peaks=c(), ylim=c(ymin, ymax), lcl.rt.chr)
}
...
for (c in 1:22) {
   ...
   main.text <- c(paste0("SCLC read depth correlation (", "Chr", c, ")"), paste0("rho = ", round0(cor, digits=2), " (Q3 vs. Q1)"))
   xlab.text <- "SCLC Q3/Q1"
   ylab.text <- "SCLC read depth [RPKM]"
   file.name <- file.path(wd.rt.plots, "chrs", paste0("RD-vs-RT_SCLC-Q3-Q1_chr", c, "_spline_spearman"))
   plotRD2vsRT(nrds.chr.T$SPLINE, nrds.chr.N$SPLINE, nrds.chr.RT$SPLINE, file.name, main.text, ylab.text, xlab.text, c("red", "blue"), c("Q3", "Q1"), method="spearman")
...
save(cors, file=file.path(wd.rt.data, paste0("rd-vs-rt_", base, "-q3-q1_spline_spearman.RData")))
writeTable(cors, file=file.path(wd.rt.data, paste0("rd-vs-rt_", base, "-q3-q1_spline_spearman.txt")), colnames=T, rownames=F, sep="\t")

## S-phase progression rate (SPR)
ylab.text <- "SPR"
file.name <- file.path(wd.rt.plots, "SPR_SCLC-Q3-Q1_spline_spearman")
main.text <- c(paste0(BASE, " Q3/Q1 S-phase progression rate"), "SPR = (E-L)/(E+L)")
plotSPR(cors, file.name, main.text, c(13, 17), digits=3, unit=5, ylab.text)

## SPR vs Read depth correlation
file.name <- file.path(wd.rt.plots, "SPR-RDC_SCLC-Q3-Q1_spline_spearman")
...
file.name <- file.path(wd.rt.plots, "SPR-Woodfine_SCLC-Q3-Q1_spline_spearman")
main.text <- c(paste0(BASE, " Q3/Q1 SPR vs. Woodfine 2004"), "Mean replication timing ratio")
...
file.name <- file.path(wd.rt.plots, "RT-vs-RT_SCLC-Q3-Q1-vs-LCL-S-G1_spline_spearman")
main.text <- paste0("SCLC Q3/Q1 vs. LCL S/G1")
ymin <- 0.25
ymax <- 1.05
plotRTvsRTALL(cors, file.name, main.text, ylab.text, xlab.text, ymin, ymax, col="black", c=2, pos=1)

##
file.name <- file.path(wd.rt.plots, "RTD-vs-RT_SCLC-Q3-Q1-vs-LCL-S-G1_spline_spearman")
ymin <- -1.1
ymax <- 1.1
