...
# Name         : manuscripts/replication/sclc-wgs-rt-q4.R
...
##
nrds.sclc.q4 <- setSpline(nrds.sclc.q4, bed.gc.chr, "RT", returnAll=T)
snr.q4$S[2] <- sd(nrds.sclc.q4$SPLINE)
snr.q4$N[2] <- sd(nrds.sclc.q4$RT - nrds.sclc.q4$SPLINE)
...
for (c in 1:22) {
   ...
   plotRT(file.name, main.text, chr, NA, NA, nrds.chr, bed.gc.chr, c("red", "blue", "#01DF01"), c("Q4 tumour", "Q1 tumour"), c("lightpink1", "lightskyblue2"), c("Q4", "Q1"), "png", width=10, peaks=c(), ylim=c(ymin, ymax), NULL, NULL)
}
...
for (c in 1:22) {
   ...
   main.text <- c(paste0("SCLC read depth correlation (", "Chr", c, ")"), paste0("rho = ", round0(sprs$cor[c], digits=2), " (Q4 vs. Q1)"))
