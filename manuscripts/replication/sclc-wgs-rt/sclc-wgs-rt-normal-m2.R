...
plotSNR2 <- function(n1, snr1, n2, snr2, file.name, main.text, xlab.text, ylab.text, col, pos) {
   ...
   legend(pos[1], c("M2/M1", "(rho = 0.8)"), text.col=col[1], pch=c(19, NA), cex=1.8)
   ...
   legend(pos[2], c("Q4/Q1", "(rho = 0.8)"), text.col=col[2], pch=c(19, NA), col=col[2], box.col=col[2], cex=1.8)
...
file.name <- file.path(wd.rt.plots, "STN2_ALL_SIZE_darkgray_lwd=4.5_bty_gray50")
main.text <- c("Signal-to-noise", "")
xlab.text <- "Signal-to-noise ratio"
ylab.text <- "Sample size"
plotSNR2(c(92, 101, 56, 96), snr$SNR, c(46, 51, 28, 48), snr.q4$SNR, file.name, main.text, xlab.text, ylab.text, c("black", "gray50"), c("topleft", "bottomright"))
