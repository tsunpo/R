...
ensGene.rt.start.sclc <- getEnsGeneRTTx(ensGene.rt.start, ensGene.rt.end, ensGene, tpm.gene.log2)   ## TSS information; see ReplicationTiming.R
ensGene.rt.end.sclc   <- ensGene.rt.end[rownames(ensGene.rt.start.sclc),]                           ## TES information
...
ylab.text <- "Expression (log2(TPM+1))"
...
right <- subset(ensGene.rt.start.sclc, RIGHT_LEADING > 505)$MEDIAN
left  <- subset(ensGene.rt.start.sclc, RIGHT_LEADING < 495)$MEDIAN
testW(right, left)
# [1] 0.1335412
...
pdf(file.name, height=6, width=6)
plot(MEDIAN ~ log10(LENGTH), data=subset(ensGene.rt.start.sclc, RIGHT_LEADING > 505), ylab=ylab.text, xlab=xlab.text, main=main.text, col="sandybrown")
abline(lm(MEDIAN ~ log10(LENGTH), data=subset(ensGene.rt.start.sclc, RIGHT_LEADING > 505)), col="orange")
dev.off()
...
pdf(file.name, height=6, width=6)
plot(MEDIAN ~ log10(LENGTH), data=subset(ensGene.rt.start.sclc, RIGHT_LEADING < 495), ylab=ylab.text, xlab=xlab.text, main=main.text, col="steelblue1")
abline(lm(MEDIAN ~ log10(LENGTH), data=subset(ensGene.rt.start.sclc, RIGHT_LEADING < 495)), col="steelblue2")
dev.off()
