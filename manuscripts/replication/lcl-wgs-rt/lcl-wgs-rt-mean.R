...
for (c in c(22:1)) {
   ...
   lcls.rt.chr <- subset(lcls.rt, CHR == chr)   ## Koren 2012
   ...
   plotRT(file.name, main.text, chr, NA, NA, nrds.chr, bed.gc.chr, c("red", "blue"), c("S phase", "G1 phase"), c(adjustcolor.red, adjustcolor.blue), c("S", "G1"), "png", width=10, peaks=c(), ylim=c(ymin, ymax), lcls.rt.chr)
}
...
main.text <- c(paste0(BASE, " S-phase progression rate"), "SPR = S/G1 mean")
...
main.text <- c(paste0(BASE, " SPR vs. Read depth correlation"), "SPR = S/G1 mean")
