...
# Last Modified: 26/11/20; 29/05/18
# =============================================================================
...
samples1 <- readTable(file.path(wd.ngs, "luad_wgs_n39-5.list"), header=F, rownames=F, sep="")
samples0 <- readTable(file.path(wd.ngs, "luad_wgs_n39-5.list"), header=F, rownames=F, sep="")
n1 <- length(samples1)
n0 <- length(samples0)
...
#load(file.path(wd.rt.data, paste0("samples-vs-rt_luad-vs-lcl_spline_spearman.RData")))
file.name <- file.path(wd.rt.plots, "SAMPLES-vs-RT_LUAD-vs-LCL_spline_spearman")
...
# -----------------------------------------------------------------------------
samples.luad <- setSamplesQ4(wd.rt.data, samples1)
writeTable(samples.luad, file.path(wd.ngs, "luad_wgs_n34.txt"), colnames=T, rownames=F, sep="\t")
#         0%        25%        50%        75%       100% 
# -0.7952126 -0.7638682 -0.7514207 -0.7399593  0.5381444 

writeTable(subset(samples.luad, Q4 %in% c(4,1)), file.path(wd.ngs, "luad_wgs_q4_n17.txt"), colnames=T, rownames=F, sep="\t")

# -----------------------------------------------------------------------------
# Beeswarm plots
# Last Modified: 21/04/19
# -----------------------------------------------------------------------------
n.luad  <- nrow(samples.luad)

samples <- toTable(0, 4, n.luad, c("CANCER", "COR", "Q4", "SAMPLE_ID"))
samples$CANCER[1:n.luad] <- 1
samples$COR <- samples.luad$COR
samples$Q4  <- samples.luad$Q4
samples$SAMPLE_ID <- samples.luad$SAMPLE_ID
rownames(samples) <- samples$SAMPLE_ID

pdf(file.path(wd.rt.plots, "boxplot_luad_google.red_nasa.blue_lwd=2_pt.cex=2.5.pdf"), height=6, width=4.2)
ymax <- 0.8392611
ymin <- -0.8773492
boxplot(COR ~ CANCER, data=samples, outline=F, names=c(""), ylim=c(ymin, ymax), ylab="", main="In silico sorting", yaxt="n", boxwex=0.75, cex.axis=1.5, cex.lab=1.6, cex.main=1.7)
...
axis(side=1, at=1, labels="LUAD", cex.axis=1.6)
#mtext(text=c(), side=1, cex=1.4, line=0.9, at=c(1,2,3))
mtext(text=c("n=34"), side=1, cex=1.6, line=2.3, at=c(1,2,3))
dev.off()
