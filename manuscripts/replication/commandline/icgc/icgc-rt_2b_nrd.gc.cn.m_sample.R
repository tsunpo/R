...
for (c in 1:22) {
   ...
   overlaps <- intersect(nrds.chr$BED, rownames(bed.gc.chr))
   bed.gc.chr <- bed.gc.chr[overlaps,]
   nrds.chr   <- nrds.chr[overlaps,]                      ## 26/02/22: Changed from intersect(rownames(nrds), rownames(bed.gc.chr))
   nrds.chr.RT <- setSpline(nrds.chr, bed.gc.chr, "RT")   ## Reference LCL S/G1 ratio
