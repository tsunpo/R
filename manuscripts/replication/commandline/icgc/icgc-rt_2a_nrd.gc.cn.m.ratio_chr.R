...
samples1 <- readTable(file.path(wd.ngs, LIST1), header=T, rownames=2, sep="")$specimen_id
samples0 <- readTable(file.path(wd.ngs, LIST0), header=T, rownames=2, sep="")$specimen_id
...
#for (c in 1:22) {
   ...
   writeTable(nrds.T.chr.m[, c("BED", samples1, "MEDIAN")], gzfile(file.path(wd.rt.data, paste0(base, "_", method, ".gc.cn.m_", chr, "_", PAIR1, "_n", n1, ".txt.gz"))), colnames=T, rownames=F, sep="\t")
   if (PAIR1 != PAIR0) {
      writeTable(nrds.N.chr.m[, c("BED", samples0, "MEDIAN")], gzfile(file.path(wd.rt.data, paste0(base, "_", method, ".gc.cn.m_", chr, "_", PAIR0, "_n", n0, ".txt.gz"))), colnames=T, rownames=F, sep="\t")
   }
   ...
   if (RATIO != "NA") {
      ...
      writeTable(outputRT(nrds.chr), gzfile(file.path(wd.rt.data, paste0(base, "_", method, ".gc.cn.m.ratio_", chr, "_", PAIR1, "-", PAIR0, "_n", n1, "-", n0, ".txt.gz"))), colnames=T, rownames=F, sep="\t")
   }
#}
