...
for (c in 1:22) {
   ...
   colnames(nrds.chr.m) <- gsub("\\.", "-", colnames(nrds.chr.m))   ## ADD 05/10/19
...
for (c in 1:22) {
   ...
   nrds.chr.RT <- setSpline2(nrds.chr, bed.gc.chr, "RT", returnAll=F)   ## Changed 29/11/19

   nrds.RT <- rbind(nrds.RT, nrds.chr.RT)
}
rm(nrds)   ## ADD 22/02/22
save(nrds.RT, file=file.path(wd.rt.data, paste0(base, "_", method, ".gc.cn.m.rt.RT.RData")))
