...
load(file=file.path("/projects/cangen/tyang2/SCLC/analysis/replication/sclc-wgs-rt/data", paste0("pca_sclc+nbl+cll+sclc.nl.RData")))
...
overlaps <- intersect(intersect(rownames(test.sclc), rownames(test.nbl.wb)), rownames(test.cll.wb))
test.sclc <- test.sclc[overlaps,]
test.nbl  <- test.nbl[overlaps,]
test.cll  <- test.cll[overlaps,]
test.sclc.nl <- test.sclc.nl[overlaps,]
