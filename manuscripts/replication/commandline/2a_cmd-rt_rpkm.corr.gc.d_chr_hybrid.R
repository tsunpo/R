...
wd1.rt.data.hybrid <- file.path(wd1.rt, "data/hybrid")
wd0.anlys  <- file.path(wd, BASE0, "analysis")
wd0.rt     <- file.path(wd0.anlys, "replication", paste0(base0, "-wgs-rt"))
...
#for (c in 1:22) {
   ...
   writeTable(outputRT(rpkms.chr), gzfile(file.path(wd1.rt.data.hybrid, paste0(base1, "_rpkm.corr.gc.d.rt_", chr, "_", BASE1, "-", BASE0, "_n", n1, "-", n0, ".txt.gz"))), colnames=T, rownames=F, sep="\t")
#}
