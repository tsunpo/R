...
for (c in 1:22) {
   ...
   ## Replication timing
   rpkms.T.chr.d <- readTable(file.path(wd.rt.data, paste0(base1, "_rpkm.corr.gc.d_", chr, "_", PAIR1, "_n", N, ".txt.gz")), header=T, rownames=T, sep="\t")
   rpkms.T.chr.d <- rpkms.T.chr.d[,c("BED", samples1)]
   rpkms.N.chr.d <- readTable(file.path(wd.rt.data, paste0(base0, "_rpkm.corr.gc.d_", chr, "_", PAIR0, "_n", N, ".txt.gz")), header=T, rownames=T, sep="\t")
