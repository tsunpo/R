...
load(file.path(wd.src.ref, "hg19.bed.gc.1kb.wes.RData"))
...
## LCL S/G1
load(file.path(wd, "LCL/analysis/replication/lcl-wgs-rt/data/lcl_rpkm.gc.cn.d.rt.log2s_s-g1_peiflyne_wes_new.RData"))
...
for (c in 1:22) {
   ...
   cors$cor[c] <- getCor(nrds.chr.RT[overlaps,]$SPLINE, nrds.T.chr.m.sample[overlaps,]$T, method="spearman")
...
cor <- getCor(nrds.chr.RT.all$SPLINE, nrds.T.chr.m.sample.all$T, method="spearman")
