...
#for (s in 1:length(samples$wgs_id)) {
   ...
   rd <- read.peiflyne.wes.cn.txt(file.path("/projects/cangen/tyang2/SCLC/ngs/WES_NEW", paste0(SAMPLE, "_ANALYSIS"), paste0(SAMPLE, "_cn.txt")))   ## See ReplicationTiming.R / Inner Class / PeifLyne File Reader
...
#for (s in 1:length(samples)) {
   ...
   if (CN)
      ...
      segs <- read.peiflyne.cn.seg(file.path("/projects/cangen/tyang2/SCLC/ngs/WES_NEW", paste0(SAMPLE, "_ANALYSIS"), paste0(SAMPLE, "_uncorr_cn.seg")))   ## See ReplicationTiming.R / Inner Class / PeifLyne File Reader
...
for (c in 1:22) {
   ...
   ## Replication timing
   load(file.path(wd, paste0("LCL/analysis/replication/lcl-wgs-rt/data/lcl_rpkm.gc.cn.d.rt.log2s_s-g1_peiflyne_wes_new_", chr, ".RData")))   ## 26/02/22: ADD
