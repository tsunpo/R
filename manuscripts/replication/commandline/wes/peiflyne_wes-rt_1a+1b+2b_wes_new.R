...
#for (s in 1:length(samples$wgs_id)) {
   ...
   rd <- read.peiflyne.wes.cn.txt(file.path("/projects/cangen/tyang2/SCLC/ngs/WES_NEW", SAMPLE, paste0(SAMPLE, "_ANALYSIS"), paste0(SAMPLE, "_cn.txt")))   ## See ReplicationTiming.R / Inner Class / PeifLyne File Reader
   nrd <- initNRD(rd, bed.gc, pair="T", method=METHOD)   ## See ReplicationTiming.R
   #nrd <- subset(nrd, RD > 20)   ## ADD 09/06/22
...
#for (s in 1:length(samples)) {
   ...
   if (CN)
      ...
      segs <- read.peiflyne.cn.seg(file.path("/projects/cangen/tyang2/SCLC/ngs/WES_NEW", SAMPLE, paste0(SAMPLE, "_ANALYSIS"), paste0(SAMPLE, "_cn.seg")))   ## See ReplicationTiming.R / Inner Class / PeifLyne File Reader
...
#wd.anlys <- file.path(wd, BASE, "analysis")
#wd.rt    <- file.path(wd.anlys, "replication", paste0(base, "-wes-rt"))
#wd.rt.data <- file.path(wd.rt, "data_1k")
...
## LCL S/G1
load(file.path(wd, "LCL/analysis/replication/lcl-wes-rt/data/lcl_rpkm.gc.cn.d.rt.log2s_s-g1_peiflyne_wes_new.RData"))
...
for (c in 1:22) {
   ...
   nrds.chr <- nrds[intersect(rownames(bed.gc.chr), nrds$BED),]   ## Reference LCL S/G1 ratio
   ...
   overlaps <- intersect(nrds.T.chr.m.sample$BED, nrds.chr.RT$BED)
   ...
   if (is.null(nrds.T.chr.m.sample.all)) {
      nrds.T.chr.m.sample.all <- nrds.T.chr.m.sample.T[overlaps,]
   } else {
      nrds.T.chr.m.sample.all <- rbind(nrds.T.chr.m.sample.all, nrds.T.chr.m.sample.T[overlaps,])
   }
...
overlaps <- intersect(rownames(nrds.chr.RT.all), rownames(nrds.T.chr.m.sample.all))
cor <- getCor(nrds.chr.RT.all[overlaps,]$SPLINE, nrds.T.chr.m.sample.all[overlaps,]$SPLINE, method="spearman")
save(cor, cors, file=file.path(wd.rt.data, "samples", paste0("rd-vs-rt_", SAMPLE, "-vs-lcl_spline_spearman.RData")))

cor <- getCor(nrds.chr.RT.all[overlaps,]$SPLINE, nrds.T.chr.m.sample.all[overlaps,]$T, method="spearman")
save(cor, cors, file=file.path(wd.rt.data, "samples", paste0("rd-vs-rt_", SAMPLE, "-vs-lcl_T_spearman.RData")))
