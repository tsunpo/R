...
# =============================================================================
# Name: cmd-rt_2c_nrd.gc.cn.d.ratio_chr_insilico.R (commandline mode)
# Author: Tsun-Po Yang (tyang2@uni-koeln.de)
# Last Modified: 31/01/21; 30/11/20; 23/04/19; 22/10/18
# =============================================================================
...
#for (c in 1:22) {
   ...
   ## Replication timing
   nrds.chr.m <- readTable(file.path(wd.rt.data, paste0(base, "_", method, ".gc.cn.d_", chr, "_", PAIR, "_n", N, ".txt.gz")), header=T, rownames=T, sep="\t")
   ...
   #nrds.chr.m.d <- nrds.chr.m[getDetectedRD(nrds.chr.m[, samples[,1]]),]   ## ADD 31/01/21; getDetectedRD()
   ...
   nrds.chr <- toTable(NA, 3, nrow(nrds.chr.m), c("T", "N", "RT"))
   rownames(nrds.chr) <- rownames(nrds.chr.m)
   nrds.chr$T  <- nrds.T.chr.m$MEDIAN
   nrds.chr$N  <- nrds.N.chr.m$MEDIAN
