...
#for (c in 1:length(chrs)) {
   ...
   for (s in 1:length(samples)) {
      ...
      nrd.chr <- nrd[bed.gc.chr, c("BED", "NRD_GC")]
      
      nrds.chr[nrd.chr$BED, s] <- nrd.chr$NRD_GC
   }

   nrds.chr$BED <- rownames(nrds.chr)
   writeTable(nrds.chr[,c("BED", samples)], gzfile(file.path(wd.ngs.data, paste0(tolower(BASE), "_", method, ".gc_", chr, "_", PAIR, ".txt.gz"))), colnames=T, rownames=F, sep="\t")
#}
