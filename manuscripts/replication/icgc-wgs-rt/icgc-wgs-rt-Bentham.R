...
tests <- readTable(file.path("/Users/ty2/Work/uni-koeln/tyang2/ICGC/metadata/Danaher 2017", "40425_2017_215_MOESM4_ESM.txt"), header=T, rownames=T, sep="")


# -----------------------------------------------------------------------------
# Bentham 2025
# Last Modified: 30/05/25
# -----------------------------------------------------------------------------
immunes <- readTable(file.path("/Users/ty2/Work/uni-koeln/tyang2/ICGC/metadata/Bentham 2025", "PCAWG_TCGA_samples_processed_20240306.tsv"), header=T, rownames=T, sep="\t")
nrow(immunes)
# [1] 819

tcgas <- intersect(rownames(immunes), mappings$icgc_donor_id)
length(tcgas)
# [1] 813

mappings.tcga <- subset(mappings, icgc_donor_id %in% tcgas)
...
rownames(mappings.tcga.rna) <- mappings.tcga.rna$icgc_donor_id
rownames(mappings.tcga.wgs) <- mappings.tcga.wgs$icgc_donor_id
overlaps.2 <- intersect(rownames(immunes), mappings.tcga.wgs$icgc_donor_id)
length(overlaps.2)
# [1] 813
...
features <- colnames(samples.mut.tcga)

# Find the index positions of the start and end columns
start_col <- which(features == "dcc_project_code")
end_col <- which(features == "pcawg_wgs_id")

# Extract column names between (inclusive)
features <- features[(start_col + 1):(end_col -1)]
features <- features[!grepl("\\.adj$", features)]
print(features)
...
for (feature in features) {
   ...
   if (sum(valid_rows) >= 3) {
      ...
      feature.title <- gsub("\\.", " ", feature)
      feature.title <- gsub("^shannon", "Shannon", feature.title)
      feature.title <- gsub("div tumour", "diversity", feature.title)
      feature.title <- gsub("fraction tumour", "fraction", feature.title)
      feature.title <- gsub("frac tumour", "fraction", feature.title)
      feature.title <- gsub("tcell", "T cell", feature.title)
      feature.title <- gsub("igMD", "IgM/IgD", feature.title) 
      feature.title <- gsub("bcell", "B cell", feature.title)
      ...
      file.name <- file.path(wd.rt.plots, "Bentham 2025_cor22", paste0("Cor_SCF-vs-", feature))
      main.text <- c("", "")
      xlab.text <- feature.title
      ylab.text <- "SCF index"
...
col <- "shannon.TRBV.div.tumour"
file.name <- file.path(wd.rt.plots, "Bentham 2025_cor22", paste0("Cor_SCF-vs-", col, "_"))
main.text <- c("", "")
xlab.text <- "TCR beta diversity"
ylab.text <- "SCF index"
plotCorrelation(file.name, main.text, xlab.text, ylab.text, x=samples.mut.tcga$shannon.TRBV.div.tumour, y=samples.mut.tcga$COR, pos="topright", cols=c("darkgray", "black"), size=5)

col <- "donor_age_at_diagnosis"
file.name <- file.path(wd.rt.plots, "Bentham 2025", paste0("Cor_SCF-vs-", col, "_TRBV"))
main.text <- c("", "")
xlab.text <- "Age"
ylab.text <- "TCR beta diversity"
plotCorrelation(file.name, main.text, xlab.text, ylab.text, x=samples.mut.tcga$donor_age_at_diagnosis, y=samples.mut.tcga$shannon.TRBV.div.tumour, pos="topright", cols=c("darkgray", "black"), size=5)
...
write.table(cor_results, file.path(wd.rt.plots, "Bentham 2025_cor22", "correlation_results.txt"),
...
ggplot(cor_sig, aes(x = SpearmanRho, y = reorder(Feature, SpearmanRho))) +
...
colnames(samples.mut.tcga) <- make.names(colnames(samples.mut.tcga), unique = TRUE)

# Initialize results data frame
cor_results <- data.frame(
 histology_abbreviation = character(),
...
# Get unique histology_abbreviation values
histologies <- unique(samples.mut.tcga$histology_abbreviation)

# Loop through each histology type
for (h in histologies) {
 # Subset data for current histology
 subset_data <- subset(samples.mut.tcga, histology_abbreviation == h)
 
 # Check for sufficient data
 valid_rows <- complete.cases(subset_data[, c("COR", "shannon.TRBV.div.tumour")])
 subset_data <- subset_data[valid_rows, ]
 
 if (nrow(subset_data) >= 3) {
  test <- cor.test(subset_data$COR, subset_data$shannon.TRBV.div.tumour, method = "spearman")
  cor_results <- rbind(cor_results, data.frame(
   histology_abbreviation = h,
   spearman_rho = as.numeric(test$estimate),
   p_value = test$p.value,
   n = nrow(subset_data)
  ))
  
  file.name <- file.path(wd.rt.plots, "Bentham 2025", paste0("SCF_", h, ""))
  main.text <- c(h, "")
  xlab.text <- "TCR beta diversity"
  ylab.text <- "SCF index"
  plotCorrelation(file.name, main.text, xlab.text, ylab.text, x=subset_data$shannon.TRBV.div.tumour, y=subset_data$COR, pos="topright", cols=c("darkgray", "black"), size=5)
 }
}

# Order results by absolute correlation
...
# Now safely select columns and compute Spearman by histology_abbreviation
library(dplyr)

df <- samples.mut.tcga %>%
   select(COR, shannon.TRBV.div.tumour, histology_abbreviation) %>%
   filter(!is.na(COR) & !is.na(shannon.TRBV.div.tumour))

# Group and compute Spearman correlation
cor_results <- df %>%
   group_by(histology_abbreviation) %>%
   filter(n() >= 3) %>%
   summarise(
      spearman_rho = cor(COR, shannon.TRBV.div.tumour, method = "spearman"),
      p_value = cor.test(COR, shannon.TRBV.div.tumour, method = "spearman")$p.value,
      n = n()
   ) %>%
