...
samples1 <- readTable(file.path(wd.ngs, "lusc-us_wgs_n48.list"), header=F, rownames=F, sep="")$V3
samples0 <- readTable(file.path(wd.ngs, "lusc-us_wgs_n48.list"), header=F, rownames=F, sep="")$V3
n1 <- length(samples1)
n0 <- length(samples0)
...
# -----------------------------------------------------------------------------
samples.lusc.us <- setSamplesQ4(wd.rt.data, samples1)
writeTable(samples.lusc.us, file.path(wd.ngs, "lusc-us_wgs_m2_n48.txt"), colnames=T, rownames=F, sep="\t")
#         0%        25%        50%        75%       100% 
# -0.7288829 -0.6789311 -0.6427159 -0.4721246  0.6915058

writeTable(subset(samples.lusc.us, Q4 %in% c(4,1)), file.path(wd.ngs, "lusc-us_wgs_q4_n24.txt"), colnames=T, rownames=F, sep="\t")
