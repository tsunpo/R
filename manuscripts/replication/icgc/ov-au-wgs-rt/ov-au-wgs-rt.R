...
samples1 <- readTable(file.path(wd.ngs, "ov-au_wgs_n70.list"), header=F, rownames=F, sep="")$V3
samples0 <- readTable(file.path(wd.ngs, "ov-au_wgs_n70.list"), header=F, rownames=F, sep="")$V3
n1 <- length(samples1)
n0 <- length(samples0)
...
# -----------------------------------------------------------------------------
samples.ov.au <- setSamplesQ4(wd.rt.data, samples1)
writeTable(samples.ov.au, file.path(wd.ngs, "ov-au_wgs_m2_n70.txt"), colnames=T, rownames=F, sep="\t")
#         0%        25%        50%        75%       100% 
# -0.7658037 -0.7395580 -0.7299205 -0.7041961 -0.1574432

writeTable(subset(samples.ov.au, Q4 %in% c(4,1)), file.path(wd.ngs, "ov-au_wgs_q4_n35.txt"), colnames=T, rownames=F, sep="\t")
