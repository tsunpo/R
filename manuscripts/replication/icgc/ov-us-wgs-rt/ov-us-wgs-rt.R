...
samples1 <- readTable(file.path(wd.ngs, "ov-us_wgs_n42.list"), header=F, rownames=F, sep="")$V3
samples0 <- readTable(file.path(wd.ngs, "ov-us_wgs_n42.list"), header=F, rownames=F, sep="")$V3
n1 <- length(samples1)
n0 <- length(samples0)
...
# -----------------------------------------------------------------------------
samples.ov.us <- setSamplesQ4(wd.rt.data, samples1)
writeTable(samples.ov.us, file.path(wd.ngs, "ov-us_wgs_m2_n42.txt"), colnames=T, rownames=F, sep="\t")
#         0%        25%        50%        75%       100% 
# -0.7328229 -0.7155357 -0.6988944 -0.6791143  0.6228575

writeTable(subset(samples.ov.us, Q4 %in% c(4,1)), file.path(wd.ngs, "ov-us_wgs_q4_n21.txt"), colnames=T, rownames=F, sep="\t")
