...
samples1 <- readTable(file.path(wd.ngs, "cll_wgs_q4_n47.txt"), header=T, rownames=T, sep="")    ## Q3/Q1
samples1 <- subset(samples1, M2 == 2)[,1]
#samples0 <- readTable(file.path(wd.ngs, "cll_wgs_n96.txt"), header=T, rownames=T, sep="")
samples0 <- readTable(file.path(wd.ngs, "cll_wgs_q4_n47.txt"), header=T, rownames=T, sep="")
...
for (c in 1:22) {
   ...
   ## Plot RT
   main.text <- paste0(BASE, " Q4 to Q1 ratio replication timing")  
