...
samples1 <- readTable(file.path(wd.ngs, "brca-uk_wgs_n44.list"), header=F, rownames=F, sep="")$V3
samples0 <- readTable(file.path(wd.ngs, "brca-uk_wgs_n44.list"), header=F, rownames=F, sep="")$V3
n1 <- length(samples1)
n0 <- length(samples0)
...
# -----------------------------------------------------------------------------
samples.brca.uk <- setSamplesQ4(wd.rt.data, samples1)
writeTable(samples.brca.uk, file.path(wd.ngs, "brca-uk_wgs_m2_n44.txt"), colnames=T, rownames=F, sep="\t")
#         0%        25%        50%        75%       100%
# -0.7571027 -0.7343835 -0.7184380 -0.6531273  0.6619123 

writeTable(subset(samples.brca.uk, Q4 %in% c(4,1)), file.path(wd.ngs, "brca-uk_wgs_q4_n22.txt"), colnames=T, rownames=F, sep="\t")
