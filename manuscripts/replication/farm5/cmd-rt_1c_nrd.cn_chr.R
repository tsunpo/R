...
#for (c in 1:length(chrs)) {
   ...
   for (s in 1:length(samples)) {
      ...
      nrd <- readTable(file.path(wd.ngs.data, sample, paste0(sample, "_", PAIR, ".", method, ".cn.txt.gz")), header=T, rownames=T, sep="")
   ...
   writeTable(nrds.chr[,c("BED", samples)], gzfile(file.path(wd.ngs.data, paste0(tolower(BASE), "_", method, ".cn_", chr, "_", PAIR, ".txt.gz"))), colnames=T, rownames=F, sep="\t")
#}
