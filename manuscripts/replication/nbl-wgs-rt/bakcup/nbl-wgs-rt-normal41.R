...
wd.rt       <- file.path(wd.anlys, "replication", paste0(base, "-wgs-rt-normal41"))
wd.rt.data  <- file.path(wd.rt, "data")
wd.rt.plots <- file.path(wd.rt, "plots")

wd.ngs.data <- file.path(wd.anlys, "replication", paste0(base, "-wgs-rt"))
samples1 <- readTable(file.path(wd.ngs, "nbl_wgs_n41_WB.list"), header=F, rownames=F, sep="")
n1 <- length(samples1)
...
nrds.T.chr.d.all <- readTable(file.path(wd.ngs.data, paste0(base, "_", method, ".gc.cn.d_", "chr1", "_", PAIR1, "_n", 56, ".txt.gz")), header=T, rownames=F, sep="\t")
for (c in 2:22) {
   chr <- chrs[c]
 
   ## Read depth
   #nrds.T.chr.d <- pipeGetDetectedRD(wd.ngs.data, BASE, chr, PAIR1, method)
   nrds.T.chr.d <- readTable(file.path(wd.ngs.data, paste0(base, "_", method, ".gc.cn.d_", chr, "_", PAIR1, "_n", 56, ".txt.gz")), header=T, rownames=F, sep="\t")
 
   nrds.T.chr.d.all <- rbind(nrds.T.chr.d.all, nrds.T.chr.d)
}

##
nrds.T.chr.d.all <- nrds.T.chr.d.all[, samples1]
test <- nrds.T.chr.d.all   ## BUG 2019/10/14: Remove column BED
pca.de <- getPCA(t(test))
save(pca.de, file=file.path(wd.rt.data, paste0("pca_nbl-wb41_chrs.RData")))

#load(file.path(wd.rt.data, paste0("pca_nbl-wb41_chrs.RData")))
