...
for (c in 1:22) {
   ...
   file.name <- file.path(wd.rt.plots, paste0("RT_", BASE, "_", method, ".d.rt.log2s_", chr, "_", PAIR1, "-", PAIR0, "_n", n1, "-", n0, "_lightpink1_lightskyblue2_0.05_1.8"))   
...
for (c in 1:22) {
   ...
   main.text <- c(paste0("NBL read depth correlation (", "Chr", c, ")"), paste0("rho = ", round0(sprs$cor[c], digits=2), " (M2 vs. M1)"))
   xlab.text <- "NBL M2/M1"
...
file.name <- file.path(wd.rt.plots, "RT-vs-RT2_NBL-M2-M1-vs-ALL_spline_spearman")
main.text <- paste0(BASE, " M2/M1")
ymin <- 0.85
ymax <- 1
plotRTvsRT2(cors, file.name, main.text, ymin, ymax, cols=c("goldenrod2", "black"), c("M2/M1 vs. Q4/Q1", "NBL vs. SCLC-NL"))
