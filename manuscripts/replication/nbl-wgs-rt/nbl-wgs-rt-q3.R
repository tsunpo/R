...
# Name         : manuscripts/replication/nbl-wgs-rt-q3.R
...
samples1 <- readTable(file.path(wd.ngs, "nbl_wgs_q3_n28.txt"), header=T, rownames=T, sep="")    ## Q3/Q1
samples1 <- subset(samples1, M2 == 1)[,1]
#samples0 <- readTable(file.path(wd.ngs, "nbl_wgs_n57-1.txt"), header=T, rownames=T, sep="")
samples0 <- readTable(file.path(wd.ngs, "nbl_wgs_q3_n28.txt"), header=T, rownames=T, sep="")
...
for (c in 1:22) {
   ...
   main.text <- c(paste0("NBL read depth correlation (", "Chr", c, ")"), paste0("rho = ", round0(cor, digits=2), " (Q3 vs. Q1)"))
   xlab.text <- "NBL Q3/Q1"
   ylab.text <- "NBL read depth [RPKM]"
   file.name <- file.path(wd.rt.plots, "chrs", paste0("RD-vs-RT_NBL-Q3-Q1_chr", c, "_spline_spearman"))
...
ylab.text <- "SPR"
file.name <- file.path(wd.rt.plots, "SPR_NBL-Q3-Q1_spline_spearman")
...
file.name <- file.path(wd.rt.plots, "SPR-Woodfine_NBL-Q3-Q1_spline_spearman")
...
file.name <- file.path(wd.rt.plots, "RT-vs-RT_NBL-Q3-Q1-vs-LCL-S-G1_spline_spearman")
main.text <- paste0("NBL Q3/Q1 vs. LCL S/G1")
ymin <- 0.25
ymax <- 1.05
plotRTvsRTALL(cors, file.name, main.text, ylab.text, xlab.text, ymin, ymax, col="black", c=2, pos=1)

##
file.name <- file.path(wd.rt.plots, "RTD-vs-RT_NBL-Q3-Q1-vs-LCL-S-G1_spline_spearman")
ymin <- -1.1
ymax <- 1.1
