...
overlaps <- intersect(samples.rna$V2, samples$SAMPLE_ID)
...
## Log2 fold change
de$M1 <- median00(tpm.gene.log2, subset(samples, M2 == 0)$V1)
de$M2 <- median00(tpm.gene.log2, subset(samples, M2 == 1)$V1)
...
save(de.tpm.gene, samples, file=file.path(wd.de.data, "de_nbl_tpm-gene-r5p47_cor_src_q_n53.RData"))
writeTable(de.tpm.gene, file.path(wd.de.data, "de_nbl_tpm-gene-r5p47_cor_src_q_n53.txt"), colnames=T, rownames=F, sep="\t")
