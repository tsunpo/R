...
count_matrix <- readTable("/lustre/scratch124/casm/team294/rs30/RNA_analyses_12Sept2024_Mouse_Shosei/featurecounts/GRCm38_FeatureCounts.txt", header=T, rownames=T, sep="\t")
count_matrix <- count_matrix[,-c(1:6)]
so <- CreateSeuratObject(counts = count_matrix)

metadata <- readTable("/lustre/scratch127/casm/team294rr/ty2/SSC/analysis/expression/shosei-de/GRCm38_FeatureCounts_metadata.txt", header=T, rownames=T, sep="\t")
...
read_counts <- data.frame(sample.id = so@meta.data$sample.id, nCount_RNA = so@meta.data$nCount_RNA)

# Generate a box plot for read counts grouped by sample ID
box_plot_reads <- ggplot(read_counts, aes(x = sample.id, y = nCount_RNA)) +
	  geom_boxplot() +
	  theme_minimal() +
	  labs(title = "Read counts per sample", x = "Sample ID", y = "Read counts")

# Part 2: Generate the Box Plot for Global Expression Levels
# Extract the total expression level (nFeature_RNA) and sample identities
expression_data <- data.frame(sample.id = so@meta.data$sample.id, nFeature_RNA = so@meta.data$nFeature_RNA)

# Generate a box plot for global expression levels grouped by sample ID
box_plot_expression <- ggplot(expression_data, aes(x = sample.id, y = nFeature_RNA)) +
	  geom_boxplot() +
	  theme_minimal() +
	  labs(title = "Global expression levels per sample", x = "Sample ID", y = "Global expression levels")

# Part 3: Generate the Box Plot for Mitochondrial Contents
# Extract the mitochondrial content (percent.mt) and sample identities
mt_data <- data.frame(sample.id = so@meta.data$sample.id, percent.mt = so@meta.data$percent.mt)

# Generate a box plot for mitochondrial contents grouped by sample ID
box_plot_mt <- ggplot(mt_data, aes(x = sample.id, y = percent.mt)) +
	  geom_boxplot() +
	  theme_minimal() +
	  labs(title = "Mitochondrial content per sample", x = "Sample ID", y = "Mitochondrial content (%)")
...
ggsave(filename = file.path(wd.de.plots, paste0("heatmap_top10_markers.png")),
