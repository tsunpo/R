...
for (cell_type in cell_types) {
	  ...
	  result_df$color <- ifelse(result_df$log2FoldChange < -0.5, "blue",
	  																										ifelse(result_df$log2FoldChange > 0.5, "red", "grey"))
	  ...
	  pdf(file = file.path(wd.de.plots, paste0("volcano_DeSeq_~batch+pseudotime_", cell_type, ".pdf")), width = 5, height = 5)
	  print(volcano_plot)
	  dev.off()
}

save(results_list, file=file.path(wd.de.data, paste0("DESeq_~batch+pseudotime.RData")))
