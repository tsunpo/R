...
wd.meta     <- file.path(wd, BASE, "metadata", "Guo 2018")
...
save(so.integrated, expression_matrix, markers, expression_matrix_de, wss, file=file.path(wd.de.data, paste0("ssc_filtered_normalised_integrated_SCT_PCA_UMAP_resolution=0.5_", nfeatures, "_-12-15-17_annotated_clustering0_SSC_MAST_only.pos=T.RData")))
...
# > 1096 + 597 + 85 + 1762 + 312
# > 1096 + 597 + 85 + 1762 + 312

save(so.integrated, expression_matrix, markers, expression_matrix_de, wss, kmeans_result, k, gene_clusters, file=file.path(wd.de.data, paste0("ssc_filtered_normalised_integrated_SCT_PCA_UMAP_resolution=0.5_", nfeatures, "_-12-15-17_annotated_clustering_SSC_MAST_only.pos=T_k=5.RData")))
...
print(go_summary_df)
write.csv(go_summary_df, file=file.path(wd.de.data, "GO_BiologicalProcess_Cluster_Umbrella_Summary_Top3_only.pos=F.csv"), row.names = FALSE)
...
ggsave(filename = file.path(wd.de.plots, paste0("heatmap_", nfeatures, "_UMAP_resolution=0.5_MAST_kmeans_n=2544_pos=T_1,2,3,4,5.png")),
...
# Guo 2018
# -----------------------------------------------------------------------------
meta <- readTable(file.path(wd.meta, paste0("Fig.5b.txt")), header=T, rownames=T, sep="")
numbers <- readTable(file.path(wd.meta, paste0("overlaps_chrs.txt")), header=F, rownames=T, sep="")
...
for (i in unique(meta$GROUP)) {
	  # Get genes in the current cluster
	  genes_in_cluster <- rownames(subset(meta, GROUP == i))
	  ...
	  # Filter the top 3 terms based on the adjusted p-value
	  top_3_go_terms <- head(go_results_df, 3)
	  # Add the summary to the list
	  go_summary[[paste0("Cluster_", i)]] <- top_3_go_terms
}
...
# Print the GO summary table
print(go_summary_df)
write.csv(go_summary_df, file=file.path(wd.de.data, "GO_BiologicalProcess_Fig.5b.csv"), row.names = FALSE)
...
# Step 1: Create a factor with the desired order
#cluster_order <- factor(gene_clusters, levels = c(1, 2, 3, 4, 5, 6))

# Step 2: Subset the expression matrix to only include genes in your K-means clusters
#ordered_genes <- names(gene_clusters[order(cluster_order)])  # Order genes by cluster
ordered_genes <- intersect(rownames(meta), rownames(so.integrated))
expression_matrix_ordered <- expression_matrix[ordered_genes, ]
...
heatmap <- DoHeatmap(so.integrated, features = ordered_genes) + NoLegend() + theme(axis.text.y = element_blank())  # Remove gene names from the y-axis
ggsave(filename = file.path(wd.de.plots, paste0("heatmap_", nfeatures, "_UMAP_resolution=0.5_n=2047_Fig.5b.png")),
							plot = heatmap, width = 10, height = 20, dpi = 300)
