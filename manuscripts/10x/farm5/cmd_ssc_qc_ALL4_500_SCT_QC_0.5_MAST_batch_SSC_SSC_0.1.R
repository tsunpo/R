...
markers <- FindAllMarkers(so.integrated, test.use = "MAST", only.pos = FALSE, min.pct = 0.1, logfc.threshold = 0.25, latent.vars = c("nCount_RNA", "batch"))
#markers %>%
#	  group_by(cluster) %>%
#	  dplyr::filter(avg_log2FC > 1)

save(so.integrated, markers, file=file.path(wd.de.data, paste0("ssc_filtered_normalised_integrated_SCT_PCA_UMAP_resolution=0.5_", nfeatures, "_annotated_19-6-23_markers_MAST_SSC_SSC_0.1.RData")))
save(markers, file=file.path(wd.de.data, paste0("ssc_filtered_normalised_integrated_SCT_PCA_UMAP_resolution=0.5_", nfeatures, "_annotated_19-6-23_markers_markers_MAST_SSC_SSC_0.1.RData")))
