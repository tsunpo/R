...
for (pd_id in pd_ids) {
   new_row <- data.frame(
      ...
      mutations = file.path(wd.de.data, "nf-scomatic_germ-soma_30/out", pd_id, paste0(pd_id, "_trencadis_mutations_scomatic.tsv")),
      stringsAsFactors = FALSE
   )
...
write.table(
   samplesheet_new,
   file = file.path(wd.de.data, "nf-trencadis-seq_scomatic", "samplesheet_trencadis_scomatic.csv"),
   sep = ",",
   row.names = FALSE,
   quote = FALSE
)
...
# -----------------------------------------------------------------------------
for (pd_id in pd_ids) {
   lines <- readLines(file.path(wd.de.data, "nf-scomatic_germ-soma_30/out", pd_id, paste0(pd_id, ".calling.step2.pass.tsv")))
   header_line <- grep("^#CHROM", lines)
   header <- unlist(strsplit(sub("^#", "", lines[header_line]), "\t"))
   vcf <- read.table(
      file = file.path(wd.de.data, "nf-scomatic_germ-soma_30/out", pd_id, paste0(pd_id, ".calling.step2.pass.tsv")),
      skip = header_line,
      sep = "\t",
      col.names = header,
      stringsAsFactors = FALSE,
   ...
   write.table(
      mutations,
      file = file.path(wd.de.data, "nf-scomatic_germ-soma_30/out", pd_id, paste0(pd_id, "_trencadis_mutations_scomatic.tsv")),
      sep = "\t",
      col.names = TRUE,
      row.names = FALSE,
      quote = FALSE
