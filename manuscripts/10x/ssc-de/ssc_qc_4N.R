...
writeTable(normalised, file.path(wd.de.data, "ssc_filtered_normalised_4N.txt"), colnames=T, rownames=F, sep="\t")
save(filtered, normalised, samples0, samples0.filtered, so.list, ids, genes, file=file.path(wd.de.data, "ssc_filtered_normalised_4N.RData"))
...
save(filtered, normalised, samples0, samples0.filtered, so.merged, ids, ages, n2s, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_4N.RData"))
...
# -----------------------------------------------------------------------------
load(file=file.path(wd.de.data, "ssc_filtered_normalised_4N.RData"))
load(file=file.path(wd.de.data, "ssc_filtered_normalised_merged_4N.RData"))
...
pdf(file.path(wd.de.data, "ssc_filtered_normalised_merged_ElbowPlot_4N.pdf"))
...
write.table(prin_comp, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_4N.txt"),row.names=FALSE,col.names=FALSE,quote=FALSE,sep='\t')
save(filtered, normalised, samples0, samples0.filtered, so.merged, pct, cumu, component1, component2, prin_comp, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_4N.RData"))
...
load(file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_4N.RData"))
...
save(filtered, normalised, samples0, samples0.filtered, so.merged, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_UMAP_4N.RData"))

pdf(file=file.path(wd.de.data, "ssc_clustree_plot_4N.pdf"))
...
for (r in 1:length(resolutions)) {
	  ...
	  so.merged <- RunUMAP(so.merged, dims = 1:prin_comp, n.neighbors = 20, verbose = FALSE)
	  save(filtered, normalised, samples0, samples0.filtered, so.merged, file=file.path(wd.de.data, paste0("ssc_filtered_normalised_merged_PCA_UMAP_4N_resolution=", res, ".RData")))
}

for (r in 1:3) {
	  r <- 1
	  res <- resolutions[r]
	  load(file=file.path(wd.de.data, paste0("ssc_filtered_normalised_merged_PCA_UMAP_4N_resolution=", res, ".RData")))

	  pdf(file=file.path(wd.de.plots, paste0("4N_UMAP_dims=", prin_comp, "_resolution=", res, ".pdf")))
	  DimPlot(so.merged, label = TRUE)
	  dev.off()
}

##
pdf(file=file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_sampleID.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="sample.id")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_Age.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="age")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_2N.pdf"))
...
pdf(file = file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_feature_plot_SSC_differentiating.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()

feature_plot <- FeaturePlot(so.merged, reduction = "umap", features = c("SYCP3", "SPO11", "MLH3", "SPAG6", "CAMK4", "ZPBP", "CREM", "TNP1", "PRM2"),	ncol = 5)
pdf(file = file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_feature_plot_Meiosis_Spermatid structure proteins_Nuclear condensation.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()
...
pdf(file = file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_ST4_SSC_State0.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()

feature_plot <- FeaturePlot(so.merged, reduction = "umap", features = c("GFRA1", "NANOS3", "DMRT1", "KIT", "MKI67", "SOHLH2", "MAGE4", "REC8", "STRA8"),	ncol = 5)
pdf(file = file.path(wd.de.plots, "4N_UMAP_dims=15_resolution=0.25_ST4_SSC_Stage1+2+3.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()
