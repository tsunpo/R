...
writeTable(normalised, file.path(wd.de.data, "ssc_filtered_normalised.1_SCT_percent.mt_nCount_RNA.txt"), colnames=T, rownames=F, sep="\t")
filtered.1 <- filtered
...
genes.1 <- genes
save(filtered.1, normalised.1, samples0.1, samples0.filtered.1, so.list.1, ids.1, genes.1, file=file.path(wd.de.data, "ssc_filtered_normalised.1_SCT_percent.mt_nCount_RNA.RData"))
...
so.list.2 <- c(so)
ids.2 <- c("AMSBIO")
save(samples0.2, so.list.2, ids.2, file=file.path(wd.de.data, "ssc_filtered_normalised.2_SCT_percent.mt_nCount_RNA.RData"))
