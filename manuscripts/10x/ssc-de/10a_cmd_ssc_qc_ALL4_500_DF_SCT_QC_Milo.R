...
# Define your ordered cell types
cell_types <- c(
 "Stage 0", "Stage 0A", "Stage 0B", "Stage 1", "Stage 2", "Stage 3",
 "Leptotene", "Zygotene", "Pachytene", "Diplotene", "Meiotic division",
 "Early spermatid", "Late spermatid"
...
# Store results
milo_results_list <- list()

# Loop through cell types
for (ct in cell_types) {
 cat("Processing:", ct, "\n")
 
 # 1. Subset Seurat object to this cell type
 seurat_ct <- subset(so.integrated, subset = cell_type == ct)
 
 # Skip if too few cells
 if (ncol(seurat_ct) < 100) {
  message("Skipping ", ct, ": too few cells")
  next
 }
 
 # 2. Convert to SingleCellExperiment and Milo object
 sce_ct <- as.SingleCellExperiment(seurat_ct)
 milo_ct <- Milo(sce_ct)
 
 # 3. Build graph & neighborhoods
 milo_ct <- buildGraph(milo_ct, k = 30, d = 30)
 milo_ct <- makeNhoods(milo_ct, prop = 0.1, k = 30, d = 30)
 
 # 4. Count cells in each sample
 meta <- colData(milo_ct)
 milo_ct <- countCells(milo_ct, meta.data = meta, sample = "orig.ident")
 
 # 5. Prepare design matrix
 design_df <- unique(as.data.frame(meta)[, c("orig.ident", "age")])
 design_df$patient_id <- str_extract(design_df$orig.ident, "^[^_]+")
 design_df$age <- as.numeric(design_df$age)
 design_df$patient_id <- factor(design_df$patient_id)
 colnames(design_df)[1] <- "sample_id"
 
 # 6. Run Milo differential abundance test
 da_results <- tryCatch({
  testNhoods(milo_ct,
             design = ~ age + (1 | patient_id),
             design.df = design_df,
             model = "glmmTMB",
             sample_col = "sample_id")
 }, error = function(e) {
  message("Failed on ", ct, ": ", e$message)
  return(NULL)
 })
 
 # 7. Store results
 milo_results_list[[ct]] <- list(
  milo = milo_ct,
  da = da_results
 )
}
...
# -----------------------------------------------------------------------------
# Generalized linear mixed model (GLMM) on counts or proportions per cell type
# -----------------------------------------------------------------------------
# Summarize cell counts per sample
cell_counts <- so.integrated@meta.data %>%
 group_by(orig.ident, cell_type = cell_type, age) %>%
 summarise(n_cells = n()) %>%
 ungroup()
...
cell_counts <- left_join(cell_counts, total_cells, by = "orig.ident") %>%
 mutate(proportion = n_cells / total_cells,
        patient_id = str_extract(orig.ident, "^[^_]+"))

# Filter to just stem cells (or any cell type you're interested in)
stem_counts <- cell_counts %>% filter(cell_type == "Stem")

# Fit GLMM: does stem cell proportion increase with age?
model <- glmer(proportion ~ age + (1 | patient_id), data = stem_counts, family = binomial())
...
# Cells ~ age
# -----------------------------------------------------------------------------
# 1. Summarize absolute cell counts per sample
cell_counts <- so.integrated@meta.data %>%
 group_by(orig.ident, cell_type = YourCellTypeColumn, age) %>%
 summarise(n_cells = n(), .groups = "drop") %>%
 mutate(patient_id = str_extract(orig.ident, "^[^_]+"))

# 2. Filter for the cell type you're interested in
stem_counts <- cell_counts %>% filter(cell_type == "Stem")

# 3. Fit GLMM: test if stem cell count increases with age
model <- glmer(n_cells ~ age,
               data = stem_counts,
               family = poisson())
...
# -----------------------------------------------------------------------------
# Using Milo for GLMM with Random Effect
# -----------------------------------------------------------------------------
library(miloR)
library(Seurat)
library(stringr)
...
# 1. Extract patient_id from orig.ident (remove trailing _2N or _2O etc.)
meta$patient_id <- str_extract(meta$orig.ident, "^[^_]+")
# Add this back to the Seurat object
so.integrated@meta.data <- meta

# 2. Create Milo object
milo <- Milo(Seurat::as.SingleCellExperiment(so.integrated))

# 3. Build neighborhoods
milo <- buildGraph(milo, k = 30, d = 30)
milo <- makeNhoods(milo, prop = 0.1, k = 30, d = 30)

# 4. Count cells per sample
milo <- countCells(milo, meta.data = colData(milo), sample = "orig.ident")

# 5. Build design dataframe
design.df <- unique(meta[, c("orig.ident", "age", "patient_id")])
colnames(design.df)[1] <- "sample_id"  # Rename for consistency with Milo

# Make sure types are correct
design.df$patient_id <- as.factor(design.df$patient_id)
design.df$age <- as.numeric(design.df$age)

# 6. Run differential abundance test using GLMM
da_results <- testNhoods(
	  milo,
	  design = ~ age + (1 | patient_id),
	  design.df = design.df,
	  model = "glmmTMB",  # robust mixed model
	  sample_col = "sample_id"
)

# 7. Adjust p-values
da_results$FDR <- p.adjust(da_results$PValue, method = "BH")

# 8. Visualize DA neighborhoods
...
# Print the plot
png(file = file.path(wd.de.plots, "6-bins_barplot_age_group_cell_type_DimPlot_5.3x7_seurat_clusters.png"), width = 5.3, height = 7, units = "in", res = 300)
print(plot)
dev.off()
...
# Print the plot
png(file = file.path(wd.de.plots, "6-bins_barplot_age_group_cell_type_DimPlot_cell_counts_5.3x7_seurat_clusters.png"), width = 5.3, height = 7, units = "in", res = 300)
print(plot)
dev.off()
...
# Print the plot
pdf(file = file.path(wd.de.plots, "6-bins_barplot_correlation_results_age_median_unsorted_seurat_clusters.pdf"), width = 6, height = 6)
print(plot)
dev.off()
...
# Print the plot
pdf(file = file.path(wd.de.plots, "6-bins_barplot_correlation_results_age_median_unsorted_seurat_clusters_CLR.pdf"), width = 6, height = 6)
print(plot)
dev.off()
