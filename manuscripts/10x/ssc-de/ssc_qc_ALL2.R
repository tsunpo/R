...
wd.de.data  <- file.path(wd.de, "data_ALL2")
wd.de.plots <- file.path(wd.de, "plots_ALL2")
...
#36601 features across 60632 samples within 1 assay 
#Active assay: RNA (36601 features, 5000 variable features)
#29 layers present: counts.PD53621b_2N, counts.PD53623b_2N, counts.PD53623b_4N, counts.PD53624b_2N, counts.PD53625b_2N, counts.PD53626b_2N, counts.PD53621b_M, counts.PD53623b_M, counts.PD53624b_M, counts.PD53625b_M, counts.PD53626b_M, counts.PD40746e_M1, counts.PD40746e_M2, counts.SeuratProject, data.PD53621b_2N, data.PD53623b_2N, data.PD53623b_4N, data.PD53624b_2N, data.PD53625b_2N, data.PD53626b_2N, data.PD53621b_M, data.PD53623b_M, data.PD53624b_M, data.PD53625b_M, data.PD53626b_M, data.PD40746e_M1, data.PD40746e_M2, data.SeuratProject, scale.data.SeuratProject
...
save(samples0.filtered, so.merged, pct, cumu, component1, component2, prin_comp, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA.RData"))
...
so.merged <- FindClusters(so.merged, algorithm=3, resolution = 0.2)

# Optional: Run UMAP for visualization
so.merged <- RunUMAP(so.merged, dims = 1:prin_comp, n.neighbors = 20)
save(filtered, normalised, samples0, samples0.filtered, so.merged, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_UMAP_resolution=0.2.RData"))

##
pdf(file=file.path(wd.de.plots, "UMAP_dims=15_resolution=0.2.pdf"))
DimPlot(so.merged, label = TRUE)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=15_resolution=0.2_SampleID.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="sample.id")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=15_resolution=0.2_Age.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="age")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=15_resolution=0.2_2N.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="n2")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=15_resolution=0.2_Batch.pdf"))
...
pdf(file = file.path(wd.de.plots, "DotPlot_dims=15_resolution=0.2.pdf"), width = 12, height = 5)
print(dot_plot)
dev.off()

# Apply this custom order
# Note: Only change levels if 'Idents' are factor. If they are characters, convert them to factors first.
new_order <- c("14", "13", "11", "1", "9", "12", "6", "5", "10", "2", "4", "3", "7", "0", "8")
...
pdf(file = file.path(wd.de.plots, "DotPlot_dims=15_resolution=0.2_ordered.pdf"), width = 12, height = 5)
print(dot_plot)
dev.off()
...
cluster_to_celltype <- c('8' = 'Undiff. SPG', '0' = 'Undiff. SPG', 
																									'7' = 'Diff. SPG',
																								 '3' = 'Meiosis', '4' = 'Meiosis',
																									'2' = 'Spermatocyte', '10' = 'Spermatocyte',
																									'5' = 'Early spermatid (1)',
																									'6' = 'Early spermatid (2)',
																									'9' = 'Myoid & Leydig',
																									'1' = 'Sertoli',	'11' = 'Sertoli',
																									'13' = 'Endothelial', '14' = 'Macrophage')
...
pdf(file = file.path(wd.de.plots, "DotPlot_dims=15_resolution=0.2_ordered_annotated.pdf"), width = 12, height = 5)
print(dot_plot)
dev.off()

##
dim_plot <- DimPlot(so.merged, label = TRUE)
ggsave(file.path(wd.de.plots, "UMAP_dims=15_resolution=0.2_ordered_annotated.png"), plot = dim_plot, width = 10, height = 8, dpi = 300)
