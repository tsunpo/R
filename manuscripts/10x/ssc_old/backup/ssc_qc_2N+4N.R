...
so.merged <- FindNeighbors(so.merged, dims = 1:10)
so.merged <- FindClusters(so.merged, resolution = 0.25)

# Optional: Run UMAP for visualization
so.merged <- RunUMAP(so.merged, dims = 1:10)
save(filtered, normalised, samples0, samples0.filtered, so.merged, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_UMAP_dims=10_resolution=0.25_2N.RData"))

##
pdf(file=file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25.pdf"))
DimPlot(so.merged, label = TRUE)
dev.off()

pdf(file=file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25_sampleID.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="sample.id")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25_Age.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="age")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25_2N.pdf"))
...
pdf(file = file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25_feature_plot_Marcrophase_Endothelial.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()

feature_plot <- FeaturePlot(so.merged, reduction = "umap", features = c("DAZL", "MAGEA4", "UTF1", "ID4", "FGFR3", "KIT", "DMRT1", "DMRTB1", "STRA8"),	ncol = 5)
pdf(file = file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25_feature_plot_SSC_differentiating.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()

feature_plot <- FeaturePlot(so.merged, reduction = "umap", features = c("SYCP3", "SPO11", "MLH3", "SPAG6", "CAMK4", "ZPBP", "CREM", "TNP1", "PRM2"),	ncol = 5)
pdf(file = file.path(wd.de.plots, "2N_UMAP_dims=10_resolution=0.25_feature_plot_Meiosis_Spermatid structure proteins_Nuclear condensation.pdf"), width = 14, height = 5)
print(feature_plot)
dev.off()
