...
# Last Modified: 25/07/24
# -----------------------------------------------------------------------------
resolution.range <- seq(from = 0.45, to = 0.5, by = 0.05)
...
##
file.name <- paste0("SCT_", 5000, "_UMAP_dims=", prin_comp, "_resolution=0.5")
...
pdf(file = file.path(wd.de.plots, paste0("Di Persio_DotPlot_SCT_", 5000, "_SCT_dims=", prin_comp, "_resolution=0.5.pdf")), width = 14, height = 5)
print(dot_plot)
dev.off()

# Apply this custom order
# Note: Only change levels if 'Idents' are factor. If they are characters, convert them to factors first.
new_order <- c("6", "14", "3", "12", "10", "9", "13", "1", "8", "5", "7", "0", "2", "11", "4")
...
cluster_to_celltype <- c('4' = 'Stage 0', '11' = 'Stage 0', '2' = 'Stage 0',
																									'0' = 'Stage 0A',
																									'7' = 'Stage 0B', 
																									'5' = 'Stage 1', '8' = 'Stage 1', '1' = 'Stage 1', '13' = 'Stage 1',
																									'9' = 'Stage 2', '10' = 'Stage 2',
																									'12' = 'Stage 3', '3' = 'Stage 3', '14' = 'Stage 3')
...
pdf(file = file.path(wd.de.plots, paste0("Di Persio_DotPlot_SCT_", nfeatures, "_SCT_dims=", prin_comp, "_resolution=0.5_ordered_annotated.pdf")), width = 12, height = 5)
print(dot_plot)
dev.off()

##
dim_plot <- DimPlot(so.integrated, label = TRUE)
ggsave(file.path(wd.de.plots, paste0("Di Persio_SCT_", nfeatures, "_UMAP_dims=", prin_comp, "_resolution=0.5_ordered_annotated.png")), plot = dim_plot, width = 10, height = 8, dpi = 300)
...
load(file=file.path(wd.de.data, paste0("ssc_filtered_normalised_integrated_SCT_PCA_UMAP_resolution=0.5_", nfeatures, "_markers.RData")))
