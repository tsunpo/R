...
wd.de.plots <- file.path(wd.de, "plots/orig_um")
dir.create(wd.de.plots, showWarnings = FALSE)

load(file.path(wd.de.data, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_monocle3+phase_RNA.RData")))
...
Phase_mapping <- so.integrated.st$Phase[match(colnames(so.st), colnames(so.integrated.st))]
so.st$Phase <- Phase_mapping
...
na_summary
#   reduction NA_rows complete_rows total_rows prop_NA
# 1   spatial    3937          1415       5352  0.7356
# 2    dbscan    2443          2909       5352  0.4565
# 3       kde    3514          1838       5352  0.6566
...
pattern_counts
#                           pattern n_cells
# 1 spatial=NA | dbscan=NA | kde=NA    2022
# 2 spatial=NA | dbscan=OK | kde=NA    1492
# 3 spatial=OK | dbscan=OK | kde=OK    1415
# 4 spatial=NA | dbscan=NA | kde=OK     421
# 5 spatial=NA | dbscan=OK | kde=OK       2
...
# 1   spatial       5352    3937          1415       0.7356
# 2    dbscan       5352    2443          2909       0.4565
# 3       kde       5352    3514          1838       0.6566
...
spatial_data$Phase     <- so.st$Phase[spatial_data$cell]
...
for (ct in all_ct) {
	...
	dim_plot <- ggplot() +
		...
		geom_text_repel(data = cluster_centers,
							 aes(x = coord1, y = coord2, label = cell_type), size = 4, color = "black") +
	...
	filename <- paste0("Spatial_ST_", safe_cell_type, "_highlighted_orig_um.png")
...
# -----------------------------------------------------------------------------
# Plot cell-type-by-cell-type - Each cell type highlighted separately in a loop
# Colouring target cells by cell cycle Phase
# Last Modified: 03/12/25
# -----------------------------------------------------------------------------
library(ggplot2)
library(Seurat)
library(dplyr)
...
#spatial_data$Phase           <- so.st$Phase

spatial_coords <- colnames(spatial_data)[1:2]

cluster_centers <- spatial_data %>%
	group_by(cell_type) %>%
	summarise(
		coord1 = median(.data[[spatial_coords[1]]]),
		coord2 = median(.data[[spatial_coords[2]]])
	)

# =======================
# 1) Per cell type (as before)
# =======================
all_ct <- sort(unique(na.omit(spatial_data$cell_type)))
...
for (ct in all_ct) {
	...
	ct_phases     <- base::intersect(names(phase_colors), unique(as.character(target_cells$Phase)))
	ct_phase_cols <- phase_colors[ct_phases]
	...
	dim_plot <- ggplot() +
		...
		geom_point(
			data = target_cells,
			aes(
				x = .data[[spatial_coords[1]]],
				y = .data[[spatial_coords[2]]],
				color = Phase
			),
			size = 3, alpha = 1
		) +
		geom_text_repel(
			...
			aes(x = coord1, y = coord2, label = cell_type),
			size = 4, color = "black"
		) +
		scale_color_manual(
			values = ct_phase_cols,
			name   = "Cell cycle",
			drop   = FALSE
		) +
		coord_fixed() +
		labs(x = "X", y = "Y", title = ct) +
		theme_classic() +
		theme(
	...
	safe_cell_type <- gsub("[^A-Za-z0-9]", "_", ct)
	filename <- paste0("Spatial_ST_", safe_cell_type, "_Phase_highlighted_orig_um.png")
	ggsave(file.path(wd.de.plots, filename), plot = dim_plot,
...
# =======================
# 2) All cells shown; each phase highlighted in its own plot
# =======================
ordered_phases <- c("G2M", "G1", "S")

# Only keep phases that actually exist
available_phases <- base::intersect(ordered_phases, unique(as.character(spatial_data$Phase)))

for (ph in available_phases) {
	phase_cells <- spatial_data %>%
		dplyr::filter(Phase == .env$ph)
	
	phase_col <- phase_colors[ph]
	
	p_phase <- ggplot() +
		# background: all cells in grey / blank-ish
		geom_point(
			data = spatial_data,
			aes(
				x = .data[[spatial_coords[1]]],
			...
			color = "#D3D3D3", fill = "white", size = 2.5, alpha = 1, shape = 21
		) +
		# highlighted phase cells in colour
		geom_point(
			data = phase_cells,
			aes(
				x = .data[[spatial_coords[1]]],
		...
		# (optional) show cluster labels too
		geom_text_repel(
			data = cluster_centers,
			aes(x = coord1, y = coord2, label = cell_type),
			size = 4, color = "black"
		...
		scale_color_manual(
			values = setNames(phase_col, ph),  # correct mapping: Phase == ph → phase_col
			breaks = ph,
			labels = ph,
			name   = "Cell cycle",
			drop   = FALSE
		...
		theme(
			...
			legend.text      = element_text(size = 16),
			legend.title     = element_text(size = 16),
			legend.position      = c(0.05, 0.05),      # bottom-left
			legend.justification = c("left", "bottom"),
			legend.background    = element_rect(fill = "white", color = "grey80"),
			legend.margin        = margin(3, 6, 3, 6)
		) +
	...
	safe_phase <- gsub("[^A-Za-z0-9]", "_", ph)
	filename <- paste0("Spatial_ST_AllCells_", safe_phase, "_highlighted_orig_um.png")
	ggsave(
		file.path(wd.de.plots, filename),
		plot   = p_phase,
		width  = 6,
		height = 6,
		dpi    = 300
	)
	cat("Saved plot for phase", ph, "\n")
}
...
#phase_proportions.1 <- phase_proportions

# -----------------------------------------------------------------------------
# NND permutation test per cell type × phase
# Last Modified: 03/12/25
# -----------------------------------------------------------------------------
library(dplyr)

nnd_phase_test <- function(df, x_col = "d_1", y_col = "d_2", B = 1000) {
	# compute NN distance for each point
	nnd <- function(x, y) {
	...
	for (ct in unique(df$cell_type)) {
		...
		if (nrow(df_ct) < 3) next
		
		for (ph in c("G1", "S", "G2M")) {
			
			df_ph <- df_ct %>% filter(Phase == ph)
			if (nrow(df_ph) < 3) next  # need >2 cells in this phase
			
			# observed mean NND for this (cell_type, phase)
			obs_nnd <- mean(nnd(df_ph[[x_col]], df_ph[[y_col]]))
			
			# permutation distribution
			perm_vals <- numeric(B)
			for (b in seq_len(B)) {
				df_shuf <- df_ct %>%
					mutate(Phase = sample(Phase)) %>%
					filter(Phase == ph)
				perm_vals[b] <- mean(nnd(df_shuf[[x_col]], df_shuf[[y_col]]))
			}
			
			pval <- mean(perm_vals <= obs_nnd)
	...
	# effect size: positive delta = clustering (perm_mean > obs_nnd)
	out <- out %>%
		mutate(
			delta      = obs_nnd - perm_mean,
			p_bonf     = p.adjust(pval, method = "bonferroni")
		)  %>%
		select(cell_type, phase, obs_nnd, perm_mean, delta, pval, p_bonf)
	
	out
}

# run it
res_nnd <- nnd_phase_test(spatial_data, x_col = "d_1", y_col = "d_2", B = 500)
res_nnd

save(res_nnd, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_RNA_orig_um_res_nnd.RData")))
write.table(
	res_nnd,
	file = file.path(wd.de.plots, "res_nnd_orig_um.txt"),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
# -----------------------------------------------------------------------------
# Does the degree of clustering (obs_nnd, perm_mean, delta) for a given phase within a cell type relate 
# to how frequent that phase is in that cell type?
# -----------------------------------------------------------------------------
library(dplyr)
library(ggplot2)
...
phase_nnd <- res_nnd %>%
	left_join(
		phase_proportions %>% select(cell_type, phase, proportion),
		by = c("cell_type", "phase")
	)
phase_nnd

# (a) obs_nnd vs proportion
cor_results_obs <- phase_nnd %>%
	group_by(phase) %>%
	summarise(
		rho  = unname(cor.test(obs_nnd, proportion, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(obs_nnd, proportion, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
print(cor_results_obs)

# (b) perm_mean vs proportion
cor_results_perm <- phase_nnd %>%
	group_by(phase) %>%
	summarise(
		rho  = unname(cor.test(perm_mean, proportion, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(perm_mean, proportion, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
print(cor_results_perm)

# (c) delta vs proportion
cor_results_delta <- phase_nnd %>%
	group_by(phase) %>%
	summarise(
		rho  = unname(cor.test(delta, proportion, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(delta, proportion, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
print(cor_results_delta)

# -----------------------------------------------------------------------------
# Generic plotting helper for any metric (obs_nnd / perm_mean / delta)
# WITH cell type labels added next to each dot
# -----------------------------------------------------------------------------
save_phase_metric_plot <- function(df,
											  phase_text,
											  phase_label,
											  color_hex,
											  x_var = c("obs_nnd", "perm_mean", "delta"),
											  x_lab,
											  outdir,
											  prefix = "Spatial_ST_RNA_spearman_phase_vs") {
	...
	if (nrow(sub) < 3) {
		warning(sprintf("Not enough data to plot/correlate for phase %s and metric %s",
							 phase_text, x_var))
		return(invisible(NULL))
	}
	
	# Spearman correlation
	ct  <- stats::cor.test(sub[[x_var]], sub$proportion, method = "spearman", exact = FALSE)
	rho <- base::unname(ct$estimate)
	p   <- ct$p.value
	lab <- sprintf("rho = %.3f\np = %.3g", rho, p)
	
	x_pos <- max(sub[[x_var]],   na.rm = TRUE)
	y_pos <- max(sub$proportion, na.rm = TRUE)
	
	p_out <- ggplot2::ggplot(sub, ggplot2::aes(x = .data[[x_var]], y = proportion)) +
		ggplot2::geom_point(size = 4, alpha = 0.8, color = color_hex) +
		ggrepel::geom_text_repel(
			aes(label = cell_type),         # label each point by cell type
			size = 4,
			color = "black",
			max.overlaps = Inf
		) +
...
##
outdir <- file.path(wd.de.plots, "nnd_phase_cor")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

# OBS_NND vs proportion
save_phase_metric_plot(phase_nnd, "G1",  "G0/G1", phase_colors[["G1"]],
							  x_var = "obs_nnd",
							  x_lab = "Observed mean NND",
							  outdir = outdir)

save_phase_metric_plot(phase_nnd, "S",   "S",     phase_colors[["S"]],
							  x_var = "obs_nnd",
							  x_lab = "Observed mean NND",
							  outdir = outdir)

save_phase_metric_plot(phase_nnd, "G2M", "G2/M",  phase_colors[["G2M"]],
							  x_var = "obs_nnd",
							  x_lab = "Observed mean NND",
							  outdir = outdir)

# PERM_MEAN vs proportion
save_phase_metric_plot(phase_nnd, "G1",  "G0/G1", phase_colors[["G1"]],
							  x_var = "perm_mean",
							  x_lab = "Permutation mean NND",
							  outdir = outdir)
...
# DELTA vs proportion (probably the most interpretable)
save_phase_metric_plot(phase_nnd, "G1",  "G0/G1", phase_colors[["G1"]],
							  x_var = "delta",
							  x_lab = "Delta (obs_nnd - perm_mean)",
							  outdir = outdir)
...
phase_nnd <- res_nnd %>%
	left_join(
		phase_proportions %>% select(cell_type, phase, count),
		by = c("cell_type", "phase")
	)
phase_nnd
...
cor_results_obs <- phase_nnd %>%
	group_by(phase) %>%
	summarise(
		rho  = unname(cor.test(obs_nnd, count, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(obs_nnd, count, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
print(cor_results_obs)

# (b) perm_mean vs count
cor_results_perm <- phase_nnd %>%
	group_by(phase) %>%
	summarise(
		rho  = unname(cor.test(perm_mean, count, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(perm_mean, count, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
print(cor_results_perm)
...
cor_results_delta <- phase_nnd %>%
	group_by(phase) %>%
	summarise(
		rho  = unname(cor.test(delta, count, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(delta, count, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
print(cor_results_delta)
...
save_phase_metric_plot <- function(df,
											  ...
											  x_lab,
											  outdir,
											  prefix = "Spatial_ST_RNA_spearman_count_vs") {
	...
	# Spearman correlation
	ct  <- stats::cor.test(sub[[x_var]], sub$count, method = "spearman", exact = FALSE)
	rho <- base::unname(ct$estimate)
	p   <- ct$p.value
	lab <- sprintf("rho = %.3f\np = %.3g", rho, p)
	
	x_pos <- max(sub[[x_var]],   na.rm = TRUE)
	y_pos <- max(sub$count, na.rm = TRUE)
	
	p_out <- ggplot2::ggplot(sub, ggplot2::aes(x = .data[[x_var]], y = count)) +
		ggplot2::geom_point(size = 4, alpha = 0.8, color = color_hex) +
		ggrepel::geom_text_repel(
			aes(label = cell_type),         # label each point by cell type
			size = 4,
...
##
outdir <- file.path(wd.de.plots, "nnd_phase_cor_count")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)
...
# Drop cells with missing coordinates (cannot compute distances on NA)
meta_cells_only <- meta_cells_only %>%
	filter(!is.na(d_1), !is.na(d_2))
...
save(spatial_data, spatial_coords, cluster_centers, avg_dist, dist_map, meta_cells_only, phase_proportions, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_RNA_orig_um.RData")))
...
print(cor_results_obs)
#save(spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, phase_colors, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_RNA_orig_um.RData")))
...
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_RNA_spearman_orig_um_phase_proportions_ALL") {
	...
	ct  <- stats::cor.test(sub$dist_to_next, sub$proportion, method = "spearman", exact = FALSE)
	rho <- base::unname(ct$estimate)
	p   <- ct$p.value
	lab <- sprintf("rho = %.3f\np = %.3g", rho, p)
...
save_phase_plot(phase_dist, "G1",  "G0/G1", phase_colors[["G1"]],  file.path(wd.de.plots, "avg_dist"))
save_phase_plot(phase_dist, "S",   "S",     phase_colors[["S"]],   file.path(wd.de.plots, "avg_dist"))
save_phase_plot(phase_dist, "G2M", "G2/M",  phase_colors[["G2M"]], file.path(wd.de.plots, "avg_dist"))

write.table(
	dist_summary,
	file = file.path(wd.de.plots, "avg_dist", paste0("Spatial_ST_spearman_orig_um_phase_proportions_ALL.txt")),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
save(so.st, spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, phase_colors, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_RNA_orig_um.RData")))

# 1) Take only the meta cells and keep only those with coordinates
meta_cells_only <- subset(spatial_data, Phase == "G1")
...
# --- helper: average Euclidean distance FROM all cells in `from` TO all cells in `to`
avg_dist <- function(df, from, to) {
	df_from <- dplyr::filter(df, .data$cell_type == from) |> dplyr::select(d_1, d_2)
	df_to   <- dplyr::filter(df, .data$cell_type == to)   |> dplyr::select(d_1, d_2)
	if (nrow(df_from) == 0L || nrow(df_to) == 0L) return(NA_real_)
...
spatial_data$variant_flag <- ifelse(spatial_data$meta_flag == "yes", "11_119284966_T_A", NA)
...
for (ct in all_ct) {
	...
	legend_breaks <- if (have_variant) c(ct, "11_119284966_T_A") else ct
	legend_values <- c(setNames(pal[ct], ct), `11_119284966_T_A` = "red")
	override_list <- list(
	...
	safe_cell_type <- gsub("[^A-Za-z0-9]", "_", ct)
	filename <- paste0("Spatial_ST_", safe_cell_type, "_highlighted_origUM_11_119284966_T_A.png")
	ggsave(file.path(wd.de.plots, filename), plot = dim_plot, width = 6, height = 6, dpi = 300)
	cat("Saved plot for", ct, "\n")
}
# > meta_cells
...
gene <- "CBL"                  # target gene (case-insensitive handled)
...
# Save
outfile <- file.path(wd.de.plots, "Spatial_ST_META_ONLY_origcolors_short_labels.png")
...
save(meta_idx, spatial_data, spatial_coords, cluster_centers, meta_cells_only, phase_proportions, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_integrated_DF_SCT_PCA_UMAP_23_res=0.5_-C0_ordered_annotated_meta_cells_only_CBL.RData")))
...
write.table(
	dist_summary,
	file = file.path(wd.de.plots, "Spatial_ST_META_CBL.txt"),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
