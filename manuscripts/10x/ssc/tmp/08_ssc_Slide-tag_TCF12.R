...
# =============================================================================
CHR  <- "chr15"
POS  <- 56918476          # if your table has Start column
REF  <- "C"
ALT  <- "T"
GENE <- "TCF12"
...
barcodes <- read.table(
   file = file.path(wd.nf.data, "scomatics_PD53623b.tsv"),
   sep = "\t",
   header = T,
...
so <- qread(file.path(wd.rna, "ngs/slide-tag/pipeline_outputs/PD53623B_1/Positions/seurat.qs"))
so.integrated.st <- subset(so.integrated, sample.id == "PD53623b_ST2")
...
# --- inputs -------------------------------------------------------------------
path_matrix    <- file.path(wd.rna, "ngs/slide-tag/pipeline_outputs/PD53623B_1/Positions/matrix.csv.gz")
path_whitelist <- file.path(wd.rna, "ngs/slide-tag/pipeline_outputs/PD53623B_1/Positions/cb_whitelist.txt")
