...
wd.de.plots <- file.path(wd.de.plots, "CCF", "quantile mapping")
...
## ---- 1b) Quantile-mapped sensitivity s_i from coverage (Option B) ----
s_floor <- 0.10     # low-coverage floor (try 0.05–0.15)
s_max   <- 0.99     # near-perfect sensitivity ceiling
alpha   <- 0.80     # curvature: <1 concave (fast early gain), >1 convex

# use only reasonably powered stages for quantiles
use_idx <- is.finite(cov_stage$cov_frac) & (cov_stage$n_cells >= 5)
q10 <- as.numeric(stats::quantile(cov_stage$cov_frac[use_idx], 0.10, na.rm = TRUE))
q90 <- as.numeric(stats::quantile(cov_stage$cov_frac[use_idx], 0.90, na.rm = TRUE))

# if spread collapses (rare), fall back to anchor scaling
if (!is.finite(q10) || !is.finite(q90) || q90 <= q10) {
	message("Quantile spread collapsed; falling back to anchor scaling.")
	# anchor = best-covered, well-powered stage (prefer Pachytene)
	anchor <- cov_stage |>
	...
	# quantile mapping with curvature
	s_by_stage <- cov_stage |>
		dplyr::mutate(
			x = pmin(1, pmax(0, (cov_frac - q10)/(q90 - q10))),  # normalize to [0,1]
			s = s_floor + (s_max - s_floor) * x^alpha
		) |>
		dplyr::select(stage, s)
}
...
dplyr::left_join(
	cov_stage %>% dplyr::select(stage, n_cells, cov_frac),
	s_by_stage, by = "stage"
) %>% dplyr::arrange(dplyr::desc(cov_frac)) %>% print(n = Inf)
...
print(out, n = Inf)
readr::write_csv(out, file.path(wd.de.plots, paste0(GENE, "_prevalence_by_stage_misclass_corrected_S3.csv")))
