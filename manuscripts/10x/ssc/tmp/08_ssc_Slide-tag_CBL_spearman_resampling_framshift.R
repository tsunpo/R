...
# -----------------------------------------------------------------------------
# 
# Last Modified: 29/10/25
# -----------------------------------------------------------------------------
library(GenomicFeatures)
library(BSgenome.Hsapiens.UCSC.hg38)
library(Biostrings)

gtf <- "/lustre/scratch125/casm/staging/team294/ty2/SSC/ngs/pacbio_old/nf-trencadis-seq/data/gencode_gff3/GRCh38/gencode.v39.annotation.gtf"
tx  <- "ENST00000264033.6"
txdb <- makeTxDbFromGFF(gtf, format="gtf", circ_seqs=character())
bs   <- BSgenome.Hsapiens.UCSC.hg38

# CDS in transcript order
cds <- cdsBy(txdb, by="tx")[[tx]]
cds <- cds[order(start(cds))]   # + strand

# Helper to get sequence of concatenated ranges
seq_for <- function(gr) paste0(as.character(getSeq(bs, gr)), collapse="")

# WT protein
aa_wt <- translate(DNAString(seq_for(cds)))

# Coordinates you observe in mutant sashimi:
cryptic_acc <- 119284963  # <-- put the exact genomic coordinate here

# Build mutant CDS: left part ends at intron9 end (3' of exon9), right part starts at cryptic_acc
# Find exon 9 end and exon 10 block
exons <- exonsBy(txdb, by="tx")[[tx]][order(start(exonsBy(txdb, by="tx")[[tx]]))]
ex9_end <- end(exons[9])

cds_left <- cds[end(cds) <= ex9_end]
# Ensure last piece ends at ex9_end
end(cds_left[length(cds_left)]) <- ex9_end

cds_right <- cds[start(cds) >= cryptic_acc]
if (length(cds_right) > 0) start(cds_right[1]) <- cryptic_acc

cds_mut <- c(cds_left, cds_right)
aa_mut  <- translate(DNAString(seq_for(cds_mut)))

# Does mutant protein contain early stop?
grepl("\\*", as.character(aa_mut))
nchar(aa_wt); nchar(aa_mut)
