...
load(file=file.path(wd.de.data, "ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5.RData"))
...
so.integrated <- subset(so.integrated, subset = seurat_clusters != 11)
so.integrated <- subset(so.integrated, subset = seurat_clusters != 19)
...
write.table(qced, file.path(wd.de.data, paste0("ssc_filtered_normalised_DF_-C0C11C19.txt")), col.names=T, row.names=F, sep="\t")
...
save(prin_comp, samples.filtered, so.integrated, file=file.path(wd.de.data, paste0("ssc_filtered_normalised_DF_SCT_", nfeatures, "_", 25, "_", 100, "_integrated_PCA_UMAP_", prin_comp, "_0.5_-C0C11C19.RData")))

##
file.name <- paste0("UMAP_SCT_", nfeatures, "_", 25, "_", 100, "_", prin_comp, "_0.5_-C0C11C19")
