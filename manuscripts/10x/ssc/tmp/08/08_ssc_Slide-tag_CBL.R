...
cor_results_obs <- phase_dist %>%
	group_by(phase) %>%
	summarise(
		rho = unname(cor.test(dist_avg_to_next, proportion, method = "spearman", exact = FALSE)$estimate),
		pval = cor.test(dist_avg_to_next, proportion, method = "spearman", exact = FALSE)$p.value,
		.groups = "drop"
	)
...
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_META_CBL_RNA") {
