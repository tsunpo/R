...
print(k_n_by_stage)
readr::write_tsv(k_n_by_stage, file.path(wd.de.plots, "origUM", "k_n_by_stage.tsv"))
...
# Quick scale check: median NN among ALL callable cells (not just mutants)
# (useful to confirm numbers are on a tens-to-hundreds-of-µm scale)
{
	all_call <- calls2 %>%
		filter(callable) %>%
		select(CB) %>%
		inner_join(coords_df, by = "CB")
	if (nrow(all_call) >= 3) {
		W <- spatstat.geom::owin(poly = spatstat.geom::convexhull.xy(all_call$x, all_call$y)$bdry[[1]])
		pp <- spatstat.geom::ppp(all_call$x, all_call$y, window = W)
		md <- median(spatstat.geom::nndist(pp))
		message(sprintf("# sanity: median NN (all callable) = %.2f µm", md))
	}
}
...
stopifnot(exists("so.integrated"))
...
readr::write_tsv(stage_spatial, file.path(wd.de.plots, "origUM", "stage_spatial_summary_with_S.tsv"))
...
print(btw_results)
readr::write_tsv(btw_results, file.path(wd.de.plots, "origUM", "between_stage_correlations_with_S.tsv"))
...
readr::write_tsv(nnd_delta_by_stage,
					  file.path(wd.de.plots, "origUM", "nnd_delta_by_stage.tsv"))
