...
# -----------------------------------------------------------------------------
# To compute average Euclidean distances WITHIN cell types
# Last Modified: 24/10/25
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# Average within-stage Euclidean distances (plot_x, plot_y)
# Last Modified: 24/10/25
# -----------------------------------------------------------------------------
library(dplyr)

# --- define order (as you had)
...
# --- helper: average pairwise Euclidean distance within a stage
within_stage_avg_dist <- function(df_stage) {
	# df_stage must have columns: plot_x, plot_y
	if (nrow(df_stage) < 2L) return(NA_real_)
	coords <- as.matrix(dplyr::select(df_stage, plot_x, plot_y))
	dmat   <- as.matrix(stats::dist(coords, method = "euclidean"))  # symmetric, 0 diag
	# take upper triangle only (unique pairs)
	n <- nrow(dmat)
	if (n < 2L) return(NA_real_)
	mean(dmat[upper.tri(dmat)], na.rm = TRUE)
}

# --- compute per-stage within distances
within_dist_map <- meta_cells_only %>%
	dplyr::select(cell_type, plot_x, plot_y) %>%
	dplyr::group_by(cell_type) %>%
	dplyr::summarise(
		n_in_stage = dplyr::n(),
		within_euclid_mean = within_stage_avg_dist(dplyr::cur_data()),
		.groups = "drop"
	) %>%
	# keep only stages in your defined order
	dplyr::filter(!is.na(cell_type)) %>%
	dplyr::mutate(cell_type = factor(cell_type, levels = cell_type_order)) %>%
	dplyr::arrange(cell_type)

# --- join back to all cells (adds the same value to every cell of that stage)
meta_cells_only <- meta_cells_only %>%
	dplyr::left_join(within_dist_map, by = "cell_type")

# optional: look at the table
# print(within_dist_map)
...
dist_summary <- meta_cells_only %>%
	dplyr::select(cell_type, within_euclid_mean) %>%
	dplyr::distinct()
...
cor_results_obs <- phase_dist %>%
	group_by(phase) %>%
	summarise(
		rho = cor(within_euclid_mean, proportion, use = "complete.obs"),
		pval = cor.test(within_euclid_mean, proportion)$p.value,
		.groups = "drop"
	)

print(cor_results_obs)
save(meta_idx, spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, ct_order_master, short_labels, ct_levels, phase_colors, file=file.path(wd.de.plots, "origUM", paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA.RData")))
...
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_META_CBL_RNA_Within") {
	sub <- df %>% dplyr::filter(.data$phase == phase_text, is.finite(.data$within_euclid_mean))
	if (nrow(sub) < 3 || all(is.na(sub$within_euclid_mean))) {
		warning(sprintf("Not enough data to plot/correlate for phase %s", phase_text))
		return(invisible(NULL))
	}
	
	ct  <- stats::cor.test(sub$within_euclid_mean, sub$proportion)
	rho <- base::unname(ct$estimate)
	p   <- ct$p.value
	lab <- sprintf("rho = %.3f\np = %.3g", rho, p)
	
	x_pos <- max(sub$within_euclid_mean, na.rm = TRUE)
	y_pos <- max(sub$proportion,   na.rm = TRUE)
	
	p_out <- ggplot2::ggplot(sub, ggplot2::aes(x = .data$within_euclid_mean, y = .data$proportion)) +
...
write.table(
	dist_summary,
	file = file.path(wd.de.plots, "origUM", paste0("Spatial_ST_META_", GENE, "_within.txt")),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
save(so.st, meta_idx, spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, ct_order_master, short_labels, ct_levels, phase_colors, file=file.path(wd.de.plots, "origUM", paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_so.st_within.RData")))



# --- join with phase proportions
colnames(stage_spatial)[1] <- "cell_type"

phase_dist2 <- phase_proportions %>%
	dplyr::left_join(stage_spatial, by = "cell_type")

# Helper to make & save one phase plot to a specific directory with a clear filename
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_META_CBL_RNA_Within_nnd_mm") {
	sub <- df %>% dplyr::filter(.data$phase == phase_text, is.finite(.data$nnd_mm))
	if (nrow(sub) < 3 || all(is.na(sub$nnd_mm))) {
		warning(sprintf("Not enough data to plot/correlate for phase %s", phase_text))
		return(invisible(NULL))
	}
	
	ct  <- stats::cor.test(sub$nnd_mm, sub$proportion)
	rho <- base::unname(ct$estimate)
	p   <- ct$p.value
	lab <- sprintf("rho = %.3f\np = %.3g", rho, p)
	
	x_pos <- max(sub$nnd_mm, na.rm = TRUE)
	y_pos <- max(sub$proportion,   na.rm = TRUE)
	
	p_out <- ggplot2::ggplot(sub, ggplot2::aes(x = .data$nnd_mm, y = .data$proportion)) +
		ggplot2::geom_point(size = 4, alpha = 0.8, color = color_hex) +
		ggplot2::geom_smooth(method = "lm", se = FALSE, linewidth = 0.6, color = color_hex) +
		ggplot2::annotate("text", x = x_pos, y = y_pos, label = lab,
								hjust = 1.05, vjust = 1.4, size = 6) +
...
# Save each phase plot with different file names in the same directory
save_phase_plot(phase_dist2, "G1",  "G0/G1", phase_colors[["G1"]],  file.path(wd.de.plots, "origUM"))
save_phase_plot(phase_dist2, "S",   "S",     phase_colors[["S"]],   file.path(wd.de.plots, "origUM"))
save_phase_plot(phase_dist2, "G2M", "G2/M",  phase_colors[["G2M"]], file.path(wd.de.plots, "origUM"))
