...
ALT  <- "A"
GENE <- "CBL"
MUT <- paste0(gsub("chr", "", CHR), "_", POS, "_", REF)
...
barcodes.st <- subset(subset(subset(barcodes, CHROM == CHR), Start == POS), Base_observed == REF)  ## Callable WT!
...
for (ct in all_ct) {
	...
	legend_values <- c(
		setNames(pal[ct], ct),
		setNames("blue", MUT)   # <- use the value of MUT as the name
	)
	...
	filename <- paste0("Spatial_ST_", safe_cell_type, "_Callable-WT_highlighted_origUM_", MUT, ".png")
...
p_meta <- ggplot(meta_cells_only, aes(x = plot_x, y = plot_y, color = cell_type)) +
	...
	ggtitle("CBL callable WT (11:119284966, REF=T)\n")

print(p_meta)

# Save
outfile <- file.path(wd.de.plots, "origUM", "Spatial_ST_META_ONLY_origcolors_short_labels_title_Callalbe-WT.png")
...
save(meta_idx, spatial_data, spatial_coords, cluster_centers, avg_dist, dist_map, meta_cells_only, phase_proportions, file=file.path(wd.de.plots, "origUM", paste0("ssc_filtered_normalised_integrated_DF_SCT_PCA_UMAP_23_res=0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_Callable-WT.RData")))
...
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_META_CBL_RNA_Callable-WT") {
...
write.table(
	dist_summary,
	file = file.path(wd.de.plots, "origUM", paste0("Spatial_ST_META_", GENE, "_Callalble-WT.txt")),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
save(so.st, meta_idx, spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, ct_order_master, short_labels, ct_levels, phase_colors, file=file.path(wd.de.plots, "origUM", paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_so.st_Callable-WT.RData")))
...
#load(file=file.path(wd.de.plots, "origUM", paste0("ssc_filtered_normalised_integrated_DF_SCT_PCA_UMAP_23_res=0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_Callable-WT.RData")))
...
# write to disk
readr::write_tsv(perm_cor_long, file.path(wd.de.plots, "origUM", "perm_cor_by_phase_long_RNA_Callable-WT.tsv"))
readr::write_tsv(perm_cor_rho_wide, file.path(wd.de.plots, "origUM", "perm_cor_rho_wide_RNA_Callable-WT.tsv"))
readr::write_tsv(perm_cor_p_wide,   file.path(wd.de.plots, "origUM", "perm_cor_p_wide_RNA_Callable-WT.tsv"))
...
print(emp_dir)
readr::write_tsv(emp_dir, file.path(wd.de.plots, "origUM", "phase_empirical_pvalues_directional_RNA_Callable-WT.tsv"))

save(phase_colors, stage_labels, res_list, perm_cor_long, perm_cor_rho_wide, perm_cor_p_wide, phase_direction, emp_dir, file=file.path(wd.de.plots, "origUM", paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_perm_Callable-WT.RData")))
...
save_phase_plot_perm <- function(df, phase_text, phase_label, color_hex, outdir, emp_dir,
											prefix = "Spatial_ST_META_CBL_permutation_trunc_dec_RNA_Callable-WT") {
	sub <- df %>% dplyr::filter(.data$phase == phase_text, is.finite(.data$dist_to_next))
	if (nrow(sub) < 3 || all(is.na(sub$dist_to_next))) {
		warning(sprintf("Not enough data to plot/correlate for phase %s", phase_text))
		return(invisible(NULL))
