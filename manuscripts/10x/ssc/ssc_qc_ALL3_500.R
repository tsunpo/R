...
# Active assay: RNA (34685 features, 5000 variable features)
...
save(samples0.filtered, so.merged, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_UMAP_resolution=0.25.RData"))

##
pdf(file=file.path(wd.de.plots, "UMAP_dims=14_resolution=0.3.pdf"))
DimPlot(so.merged, label = TRUE)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=14_resolution=0.3_SampleID.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="sample.id")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=14_resolution=0.3_Age.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="age")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=14_resolution=0.25_2N.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="n2")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=14_resolution=0.3_Batch.pdf"))
...
length(unique(so.merged$seurat_clusters))
...
so.merged.ssc.filtered <- subset(so.merged.ssc, cells = rownames(metadata[metadata$sample.id %in% samples_to_keep, ]))

# Check the result
table(so.merged.ssc.filtered@meta.data$sample.id)
