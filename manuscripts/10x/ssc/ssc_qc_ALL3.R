...
# 36601 features across 64585 samples within 1 assay 
# Active assay: RNA (36601 features, 5000 variable features)
# 33 layers present: counts.PD53621b_2N, counts.PD53623b_2N, counts.PD53623b_4N, counts.PD53624b_2N, counts.PD53625b_2N, counts.PD53626b_2N, counts.PD53621b_M, counts.PD53623b_M, counts.PD53624b_M, counts.PD53625b_M, counts.PD53626b_M, counts.PD40746e_M1, counts.PD40746e_M2, counts.PD53623b_L1, counts.PD53623b_L2, counts.SeuratProject, data.PD53621b_2N, data.PD53623b_2N, data.PD53623b_4N, data.PD53624b_2N, data.PD53625b_2N, data.PD53626b_2N, data.PD53621b_M, data.PD53623b_M, data.PD53624b_M, data.PD53625b_M, data.PD53626b_M, data.PD40746e_M1, data.PD40746e_M2, data.PD53623b_L1, data.PD53623b_L2, data.SeuratProject, scale.data.SeuratProject
...
save(samples0.filtered, so.merged, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_UMAP.RData"))
...
save(samples0.filtered, so.merged, file=file.path(wd.de.data, "ssc_filtered_normalised_merged_PCA_UMAP_resolution=0.2.RData"))

##
pdf(file=file.path(wd.de.plots, "UMAP_dims=11_resolution=0.2.pdf"))
DimPlot(so.merged, label = TRUE)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=11_resolution=0.2_SampleID.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="sample.id")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=11_resolution=0.2_Age.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="age")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=11_resolution=0.2_2N.pdf"))
tplot = DimPlot(so.merged, reduction = "umap", group.by="n2")
tplot[[1]]$layers[[1]]$aes_params$alpha = 0.5
print(tplot)
dev.off()

pdf(file=file.path(wd.de.plots, "UMAP_dims=11_resolution=0.2_Batch.pdf"))
...
pdf(file = file.path(wd.de.plots, "DotPlot_dims=15_resolution=0.25.pdf"), width = 12, height = 5)
print(dot_plot)
dev.off()

# Apply this custom order
# Note: Only change levels if 'Idents' are factor. If they are characters, convert them to factors first.
new_order <- c("16", "12", "5", "10", "15", "8", "3", "9", "7", "11", "4", "13", "2", "6", "0", "14", "1")
...
cluster_to_celltype <- c('1' = 'Undiff. SPG', '14' = 'Undiff. SPG', '0' = 'Undiff. SPG', 
																									'6' = 'Diff. SPG',
																									'2' = 'Meiosis', '13' = 'Meiosis', '4' = 'Meiosis',
																									'11' = 'Spermatocyte', '7' = 'Spermatocyte', '9' = 'Spermatocyte', 
																									'9' = 'Early spermatid (1)',
																									'3' = 'Early spermatid (2)',
																									'8' = 'Late spermatid',
																									'10' = 'Myoid & Leydig',
																									'5' = 'Sertoli',	'12' = 'Sertoli',
																									'16' = 'Endothelial')
...
pdf(file = file.path(wd.de.plots, "DotPlot_dims=15_resolution=0.25_ordered_annotated.pdf"), width = 12, height = 5)
print(dot_plot)
dev.off()

##
dim_plot <- DimPlot(so.merged, label = TRUE)
ggsave(file.path(wd.de.plots, "UMAP_dims=15_resolution=0.25_ordered_annotated.png"), plot = dim_plot, width = 10, height = 8, dpi = 300)
