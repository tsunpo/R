...
wd.de.plots <- file.path(wd.de, "plots/dbscan")
...
p <- DimPlot(
   so, reduction = "dbscan",
   group.by = "cell_type",   # or "seurat_clusters", "Phase", etc.
   pt.size = 0.2
) +
...
dim_plot <- ggplot() +
	...
	# only one legend entry: the current ct color
	coord_fixed() +
	labs(x = "X", y = "Y", title = "PD53626b_ST1") +
	theme_classic() +
	theme(
...
filename <- paste0("Spatial_ST_highlighted_dbscan_9x9_12.png")
ggsave(file.path(wd.de.plots, filename), plot = dim_plot, width = 9, height = 9, dpi = 300)
...
for (ct in all_ct) {
	...
	ct_phases     <- intersect(names(phase_colors), unique(as.character(target_cells$Phase)))
	...
	safe_cell_type <- gsub("[^A-Za-z0-9]", "_", ct)
	filename <- paste0("Spatial_ST_", safe_cell_type, "_Phase_highlighted_dbscan.png")
...
available_phases <- intersect(ordered_phases, unique(as.character(spatial_data$Phase)))
...
for (ph in available_phases) {
	...
	filename <- paste0("Spatial_ST_AllCells_", safe_phase, "_highlighted_dbscan.png")
	ggsave(
...
# =======================
# 1) Create stacked bar chart using proportions
# =======================
# Define phase colors
...
# Save the plot
ggsave(file.path(wd.de.plots, "cell_cycle_proportions_by_cell_typ_6x14_RNA_PD53626b_ST1.png"), 
		 plot = bar_plot, width = 14, height = 6, dpi = 300)

# =======================
# 2) Create stacked bar chart using counts instead of proportions
# =======================
bar_plot <- ggplot(phase_proportions, aes(x = count, y = cell_type, fill = phase)) +
	geom_bar(stat = "identity", width = 0.7) +
	scale_fill_manual(
		values = phase_colors,
		breaks = c("G1", "S", "G2M"),
...
# Save the plot
ggsave(
	file.path(wd.de.plots, "cell_cycle_counts_by_cell_type_6x14_RNA_PD53626b_ST1.png"),
	plot = bar_plot,
	width = 14,
	height = 6,
	dpi = 300
...
# df = spatial_data with columns: plot_x, plot_y, cell_type, Phase

nnd_phase_test <- function(df, B = 1000) {
	...
	for (ct in unique(df$cell_type)) {
		...
		for (ph in c("G1", "S", "G2M")) {
			...
			# observed NND
			obs_nnd <- mean(nnd(df_ph$plot_x, df_ph$plot_y))
			...
			for (b in seq_len(B)) {
				df_shuf <- df_ct %>%
					...
					filter(Phase == ph)
				perm_vals[b] <- mean(nnd(df_shuf$plot_x, df_shuf$plot_y))
			}
	...
	# Add effect size (delta) and Bonferroni correction
	out <- out %>%
		mutate(
			delta  = obs_nnd - perm_mean,          # clustering effect size
...
# Run it
res_nnd <- nnd_phase_test(spatial_data)
print(res_nnd)

save(res_nnd, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_RNA_dbscan_res_nnd.RData")))
write.table(
	res_nnd,
	file = file.path(wd.de.plots, "res_nnd_dbscan.txt"),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
save_phase_metric_plot <- function(df,
											  ...
											  x_lab,
											  outdir,
											  prefix = "Spatial_ST_RNA_spearman_phase_vs_metric") {
	...
	# Safe filename per phase + metric
	fname <- file.path(
		outdir,
		sprintf("%s_%s_%s.png",
				  prefix,
...
save(spatial_data, spatial_coords, cluster_centers, avg_dist, dist_map, meta_cells_only, phase_proportions, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_RNA_dbscan.RData")))
...
print(cor_results_obs)
save(spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, phase_colors, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_RNA_dbscan.RData")))
...
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_META_CBL_RNA_spearman") {
...
write.table(
	dist_summary,
	file = file.path(wd.de.plots, "avg_dist", paste0("Spatial_ST_spearman.txt")),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
save(so.st, meta_idx, spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, ct_order_master, short_labels, ct_levels, phase_colors, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_so.st.RData")))

# -----------------------------------------------------------------------------
# To compute average Euclidean distances between cell types (fro each phase)
# Last Modified: 04/12/25
# -----------------------------------------------------------------------------
library(dplyr)

# 1) Take only the meta cells and keep only those with coordinates
meta_cells_only <- subset(spatial_data, Phase == "G2M")
...
# Helper to make & save one phase plot to a specific directory with a clear filename
save_phase_plot <- function(df, phase_text, phase_label, color_hex, outdir,
									 prefix = "Spatial_ST_RNA_spearman") {
	sub <- df %>% dplyr::filter(.data$phase == phase_text, is.finite(.data$dist_to_next))
	if (nrow(sub) < 3 || all(is.na(sub$dist_to_next))) {
		warning(sprintf("Not enough data to plot/correlate for phase %s", phase_text))
		return(invisible(NULL))
...
outdir <- file.path(wd.de.plots, "avg_dist_phase")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

# Save each phase plot with different file names in the same directory
#save_phase_plot(phase_dist, "G1",  "G0/G1", phase_colors[["G1"]],  file.path(wd.de.plots, "avg_dist_phase"))
#save_phase_plot(phase_dist, "S",   "S",     phase_colors[["S"]],   file.path(wd.de.plots, "avg_dist_phase"))
save_phase_plot(phase_dist, "G2M", "G2/M",  phase_colors[["G2M"]], file.path(wd.de.plots, "avg_dist_phase"))

write.table(
	dist_summary,
	file = file.path(wd.de.plots, "avg_dist_phase", paste0("Spatial_ST_spearman.txt")),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
