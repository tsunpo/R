...
if (!dir.exists(file.path(wd.de.plots, "avg_dist"))) dir.create(file.path(wd.de.plots, "avg_dist"), recursive = TRUE)
...
# Save
outfile <- file.path(wd.de.plots, "Spatial_ST_META_ONLY_origcolors_short_labels_title.png")
...
save(meta_idx, spatial_data, spatial_coords, cluster_centers, avg_dist, dist_map, meta_cells_only, phase_proportions, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA.RData")))
...
save(meta_idx, spatial_data, spatial_coords, cluster_centers, dist_map, meta_cells_only, phase_proportions, dist_summary, phase_dist, cor_results_obs, ct_order_master, short_labels, ct_levels, phase_colors, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA.RData")))
...
write.table(
	dist_summary,
	file = file.path(wd.de.plots, "avg_dist", paste0("Spatial_ST_META_", GENE, "_spearman.txt")),
	sep = "\t",
	quote = FALSE,
	row.names = FALSE,
	col.names = TRUE
...
# -----------------------------------------------------------------------------
# After running 08a_ssc_ST_CBL.R
# # Last Modified: 09/10/25
# -----------------------------------------------------------------------------
library(dplyr)

load(file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA.RData")))
...
for (b in seq_len(B)) {
	...
	cor_results <- phase_dist_perm %>%
		group_by(phase) %>%
		summarise(
			rho = unname(cor.test(dist_to_next.y, proportion, method = "spearman", exact = FALSE)$estimate),
			pval = cor.test(dist_to_next.y, proportion, method = "spearman", exact = FALSE)$p.value,
			.groups = "drop"
		)
...
perm_cor_long_with_obs <- perm_cor_long_with_obs %>%
	tidyr::replace_na(list(b = 0L))   # make sure b stays integer
...
# write to disk
readr::write_tsv(perm_cor_long,     file.path(wd.de.plots, "avg_dist", "perm_cor_by_phase_long_RNA_spearman.tsv"))
readr::write_tsv(perm_cor_rho_wide, file.path(wd.de.plots, "avg_dist", "perm_cor_rho_wide_RNA_spearman.tsv"))
readr::write_tsv(perm_cor_p_wide,   file.path(wd.de.plots, "avg_dist", "perm_cor_p_wide_RNA_spearman.tsv"))
...
print(emp_dir)
readr::write_tsv(emp_dir, file.path(wd.de.plots, "avg_dist", "phase_empirical_pvalues_directional_RNA_spearman.tsv"))

save(phase_colors, stage_labels, res_list, perm_cor_long, perm_cor_rho_wide, perm_cor_p_wide, phase_direction, emp_dir, file=file.path(wd.de.plots, paste0("ssc_filtered_normalised_DF_SCT_5000_25_100_integrated_PCA_UMAP_23_0.5_-C0_ordered_annotated_meta_cells_only_", GENE, "_RNA_perm_spearman.RData")))
...
save_phase_plot_perm <- function(df, phase_text, phase_label, color_hex, outdir, emp_dir,
											prefix = "Spatial_ST_META_CBL_permutation_trunc_dec_RNA_spearman") {
	sub <- df %>% dplyr::filter(.data$phase == phase_text, is.finite(.data$dist_to_next))
	if (nrow(sub) < 3 || all(is.na(sub$dist_to_next))) {
		warning(sprintf("Not enough data to plot/correlate for phase %s", phase_text))
		return(invisible(NULL))
...
# Save each phase plot with different file names in the same directory
save_phase_plot_perm(phase_dist, "G1",  "G0/G1", phase_colors[["G1"]],  file.path(wd.de.plots, "avg_dist"), emp_dir)
save_phase_plot_perm(phase_dist, "S",   "S",     phase_colors[["S"]],   file.path(wd.de.plots, "avg_dist"), emp_dir)
save_phase_plot_perm(phase_dist, "G2M", "G2/M",  phase_colors[["G2M"]], file.path(wd.de.plots, "avg_dist"), emp_dir)
