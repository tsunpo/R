...
samples <- samples[rownames(subset(samples, Cell_Type %in% c("Spermatogonia_PIvap_positive", "Spermatogonia_TA"))),]
...
trait <- samples$Cell_Type
trait.v <- c("Spermatogonia_PIvap_positive", "Spermatogonia_TA")
cols    <- c(orange, blue)
names(cols) <- trait.v  # This will map each color to the correct trait

# Plot PCA
pca_data <- as.data.frame(pca.de$x)
pca_data$trait <- trait

library(ggplot2)
ggplot(pca_data, aes(x = PC1, y = PC2, color = trait)) +
	  geom_point(size = 3) +
...
argv      <- data.frame(predictor="Cell_Type", predictor.wt="Spermatogonia_TA", test="Wilcoxon", test.fdr="Q", stringsAsFactors=F)
file.name <- paste0("DE_tpm-gene-median0_PIvap-vs-TA_wilcox_q")
file.main <- paste0("PIvap vs. TA in ", BASE)
...
# Add a column to classify each gene based on FDR and log2 fold change
de.tpm.gene$significance <- "Not Significant"
de.tpm.gene$significance[de.tpm.gene$FDR < fdr_threshold & de.tpm.gene$LOG2_FC > log2fc_threshold] <- "Upregulated"
de.tpm.gene$significance[de.tpm.gene$FDR < fdr_threshold & de.tpm.gene$LOG2_FC < -log2fc_threshold] <- "Downregulated"

# Create the volcano plot
volcano_plot <- ggplot(de.tpm.gene, aes(x = LOG2_FC, y = -log10(P), color = significance)) +
...
plot.de <- file.path(wd.de.plots, "volcanoplot_DE_SFB_median0_NEMO-vs-WT_p0.01_fc1_GENE")
