...
samples <- readTable(file.path(wd.wgs, "sclc_wgs_n101.list"), header=F, rownames=T, sep="")
samples.1 <- toTable(1, 2, length(samples), c("SAMPLE_ID", "RT"))
samples.1$SAMPLE_ID <- paste0(samples, "_T")

samples <- readTable(file.path(wd.wgs, "sclc_wgs_n92_N.list"), header=F, rownames=T, sep="")
samples.0 <- toTable(0, 2, length(samples), c("SAMPLE_ID", "RT"))
samples.0$SAMPLE_ID <- paste0(samples, "_N")
...
so <- sleuth_prep(s2c, target_mapping=t2g, aggregation_column="ens_gene", extra_bootstrap_summary=T, min_reads=50, min_prop=1.00)
# reading in kallisto results
# dropping unused factor levels
# ......................................................
# normalizing est_counts
# 67419 targets passed the filter
# normalizing tpm
# merging in metadata
# aggregating by column: ens_gene
# 28331 genes passed the filter
# summarizing bootstraps
# .............................................................................................
# 96338 targets passed the filter   ## min_reads=20, min_prop=1.00
# 33110 genes passed the filter
...
save(tpm.norm.filt, file=file.path(wd.de.data, paste0(base, "_kallisto_0.43.1_tpm.norm.filt_r50p100.RData")))
...
save(tpm.gene, file=file.path(wd.de.data, paste0(base, "_kallisto_0.43.1_tpm.gene_r50p100.RData")))
# > nrow(tpm.gene)
# [1] 26977
# > nrow(tpm.gene)
# [1] 31155   ## min_reads=20, min_prop=1.00
...
file.main <- file.path(wd.de.data, paste0(base, "_kallisto_0.43.1_tpm.gene_r50p100"))
load(paste0(file.main, ".RData"))
