...
# Manuscript   :
# Chapter      :
# Name         : manuscripts/expression/hcc-n-tpm-de.R
...
wd.de    <- file.path(wd.anlys, "expression/kallisto", paste0(base, "-n-tpm-de"))
wd.de.data  <- file.path(wd.de, "data")
wd.de.plots <- file.path(wd.de, "plots")

samples <- readTable(file.path(wd.rna, "hcc_rna_n130.txt"), header=T, rownames=T, sep="")
samples <- subset(samples, Specimen_Type == "Normal")$ICGC_Sample_ID
...
# 69458 targets passed the filter
# normalizing tpm
# merging in metadata
# aggregating by column: ens_gene
# 18863 genes passed the filter
