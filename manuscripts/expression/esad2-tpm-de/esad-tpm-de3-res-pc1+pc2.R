...
tpm.gene.res <- t(mapply(x = 1:nrow(tpm.gene), function(x) as.numeric(resid(lm(as.numeric(tpm.gene[x,]) ~ pc1 + pc2)))))
colnames(tpm.gene.res) <- rownames(samples)
rownames(tpm.gene.res) <- rownames(tpm.gene)
save(tpm.gene.res, file=file.path(wd.de.data, paste0(base, "_kallisto_0.43.1_tpm.gene.median0.res.pc1+pc2.RData")))

tpm.gene.log2.res <- t(mapply(x = 1:nrow(tpm.gene.log2), function(x) as.numeric(resid(lm(as.numeric(tpm.gene.log2[x,]) ~ pc1 + pc2)))))
colnames(tpm.gene.log2.res) <- rownames(samples)
rownames(tpm.gene.log2.res) <- rownames(tpm.gene.log2)
save(tpm.gene.log2.res, file=file.path(wd.de.data, paste0(base, "_kallisto_0.43.1_tpm.gene.median0.log2.res.pc1+pc2.RData")))
...
save(de.tpm.gene, samples, file=file.path(wd.de.data, "src_esad_tpm-gene-median0-residual-pc1+pc2_N-B-X_src_q_n68.RData"))
...
plot.de <- file.path(wd.de.plots, "volcanoplot_EAC_median0_residual_pc1_N-B-X_p1e-6")
