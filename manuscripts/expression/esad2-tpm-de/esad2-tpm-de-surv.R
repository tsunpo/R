...
colnames <- c("NR_intern", "CCG_ID", "PATIENT_ID2", "FILE_NAME", "FILE_NAME_2", "Responder", "Type", "FILE_NAME")
...
# -----------------------------------------------------------------------------
overlaps <- intersect(rownames(samples2), rownames(purities2))
samples2.surv  <- samples2[overlaps,]
purities2.surv <- purities2[overlaps,]

fit <- survfit(Surv(OS_month, OS_censor) ~ Survival, data=purities2.surv)
file.name <- file.path(wd.de.plots, "survfit_2_Survival")
plotSurvfit55(fit, file.name, "EAC 2 (n=57)", c("Long", "Short"), c(blue, red))

fit <- survfit(Surv(OS_month, OS_censor) ~ Response, data=subset(purities2.surv, Response != "Minor"))
file.name <- file.path(wd.de.plots, "survfit_2_Response_Complete-vs-Major")
plotSurvfit55(fit, file.name, "EAC 2", c("Complete", "Major"), c(green, red))

fit <- survfit(Surv(OS_month, OS_censor) ~ Response, data=subset(purities2.surv, Response != "Major"))
file.name <- file.path(wd.de.plots, "survfit_2_Response_Complete-vs-Minor")
plotSurvfit55(fit, file.name, "EAC 2", c("Complete", "Minor"), c(green, yellow))

fit <- survfit(Surv(OS_month, OS_censor) ~ Response, data=subset(purities2.surv, Response != "Complete"))
file.name <- file.path(wd.de.plots, "survfit_2_Response_Major-vs-Minor")
plotSurvfit55(fit, file.name, "EAC 2", c("Major", "Minor"), c(red, yellow))

###
##
fit <- survfit(Surv(OS_month, OS_censor) ~ Survival, data=purities2)
file.name <- file.path(wd.de.plots, "survfit_1+2_Survival")
plotSurvfit55(fit, file.name, "EAC 1+2 (n=92)", c("Long", "Short"), c(blue, red))

fit <- survfit(Surv(OS_month, OS_censor) ~ Response, data=subset(purities2, Response != "Minor"))
file.name <- file.path(wd.de.plots, "survfit_1+2_Response_Complete-vs-Major")
plotSurvfit55(fit, file.name, "EAC 1+2", c("Complete", "Major"), c(green, red))

fit <- survfit(Surv(OS_month, OS_censor) ~ Response, data=subset(purities2, Response != "Major"))
file.name <- file.path(wd.de.plots, "survfit_1+2_Response_Complete-vs-Minor")
plotSurvfit55(fit, file.name, "EAC 1+2", c("Complete", "Minor"), c(green, yellow))

fit <- survfit(Surv(OS_month, OS_censor) ~ Response, data=subset(purities2, Response != "Complete"))
file.name <- file.path(wd.de.plots, "survfit_1+2_Response_Major-vs-Minor")
plotSurvfit55(fit, file.name, "EAC 1+2", c("Major", "Minor"), c(red, yellow))

# -----------------------------------------------------------------------------
# Purities
# Last Modified: 13/04/22
# -----------------------------------------------------------------------------
purities2.surv.nona <- purities2.surv[!is.na(purities2.surv$Tumor_content_pathology),]
purities2.surv.nona <- purities2.surv.nona[!is.na(purities2.surv.nona$Estimated_purity_WES),]
 
x <- purities2.surv.nona$Tumor_content_pathology
y <- purities2.surv.nona$Estimated_purity_WES
file.name <- file.path(wd.de.plots, paste0("correlation_purity_EAC2_n47"))
plotCorrelation(file.name, "EAC 2 (n=47)", "Tumor_content_pathology", "Estimated_purity_WES", x, y, "topleft", col="black")

s <- rownames(subset(purities2.surv.nona, Survival == "Short"))
l <- rownames(subset(purities2.surv.nona, Survival == "Long"))

file.name <- file.path(wd.de.plots, "boxplot_purities_EAC2_Tumor_content_pathology")
ylab.txt <- "Tumor_content_pathology"
plotBox(file.name, purities2.surv.nona[s,]$Tumor_content_pathology, purities2.surv.nona[l,]$Tumor_content_pathology, "EAC 2 (n=47)", names=c("Short", "Long"), cols=c("white", "white"), ylab.txt)

file.name <- file.path(wd.de.plots, "boxplot_purities_EAC2_Estimated_purity_WES")
ylab.txt <- "Estimated_purity_WES"
plotBox(file.name, purities2.surv.nona[s,]$Estimated_purity_WES, purities2.surv.nona[l,]$Estimated_purity_WES, "EAC 2 (n=47)", names=c("Short", "Long"), cols=c("white", "white"), ylab.txt)

###
##
purities2.nona <- purities2[!is.na(purities2$Tumor_content_pathology),]
purities2.nona <- purities2.nona[!is.na(purities2.nona$Estimated_purity_WES),]

x <- purities2.nona$Tumor_content_pathology
y <- purities2.nona$Estimated_purity_WES
file.name <- file.path(wd.de.plots, paste0("correlation_purity_EAC1+2_n53"))
plotCorrelation(file.name, "EAC 1+2 (n=53)", "Tumor_content_pathology", "Estimated_purity_WES", x, y, "topleft", col="black")

s <- rownames(subset(purities2.nona, Survival == "Short"))
l <- rownames(subset(purities2.nona, Survival == "Long"))

file.name <- file.path(wd.de.plots, "boxplot_purities_EAC1+2_Tumor_content_pathology")
ylab.txt <- "Tumor_content_pathology"
plotBox(file.name, purities2.nona[s,]$Tumor_content_pathology, purities2.nona[l,]$Tumor_content_pathology, "EAC 1+2 (n=53)", names=c("Short", "Long"), cols=c("white", "white"), ylab.txt)

file.name <- file.path(wd.de.plots, "boxplot_purities_EAC1+2_Estimated_purity_WES")
ylab.txt <- "Estimated_purity_WES"
plotBox(file.name, purities2.nona[s,]$Estimated_purity_WES, purities2.nona[l,]$Estimated_purity_WES, "EAC 1+2 (n=53)", names=c("Short", "Long"), cols=c("white", "white"), ylab.txt)

# -----------------------------------------------------------------------------
# Wilcoxon rank sum test (non-parametric; n=45, 22 TR vs 23 UN)
# Last Modified: 22/08/20
# -----------------------------------------------------------------------------
samples2.surv$Survival <- purities2.surv$Survival
samples2.surv$Tumor_content_pathology <- purities2.surv$Tumor_content_pathology
...
# -----------------------------------------------------------------------------
samples2$Survival <- "NA"
samples2[rownames(subset(purities2.surv, Survival == "Long")),]$Survival  <- "Long"
samples2[rownames(subset(purities2.surv, Survival == "Short")),]$Survival <- "Short"

n <- which(samples2$Type == "N")
trait <- samples2[, "Survival"]
trait[n] <- "Normal"
trait.v <- c("Long", "Short", "Normal", "NA")
cols    <- c(blue, red, "lightgray", "lightgray")

###
##
test <- tpm.gene[, rownames(samples2)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.de <- getPCA(t(test))
file.main <- c("EAC 2 (n=97)", "Expressed genes (n=15,381)")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_EAC2_Survival_n97_ALL_normal", size=6, file.main, "bottomright", trait.v, cols, NULL, flip.x=1, flip.y=1, legend.title=NA)

test <- tpm.gene[genes.NBX.down.pe3, rownames(samples2)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.de <- getPCA(t(test))
file.main <- c("EAC 2 (n=97)", "14 Basal-like genes")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_EAC2_Survival_n97_14-basal-like-gene", size=6, file.main, "bottomright", trait.v, cols, NULL, flip.x=1, flip.y=1, legend.title=NA)

###
##
idx1 <- which(is.na(purities2.surv$Tumor_content_pathology))
idx2 <- which(purities2.surv$Tumor_content_pathology == 0)
idx <- c(idx1, idx2)
median(purities2.surv[-idx,]$Tumor_content_pathology)

samples2$Purity <- "NA"
samples2[rownames(subset(purities2.surv, Tumor_content_pathology >= 60)),]$Purity <- "Purity >= 60"
samples2[rownames(subset(purities2.surv, Tumor_content_pathology <  60)),]$Purity <- "Purity < 60"

n <- which(samples2$Type == "N")
trait <- samples2[, "Purity"]
trait[n] <- "Normal"
trait.v <- c("Purity < 60", "Purity >= 60", "Normal", "NA")
cols    <- c(blue, red, "lightgray", "lightgray")
...
pca.de <- getPCA(t(test))
file.main <- c("EAC 2 (n=97)", "Expressed genes (n=15,381)")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_EAC2_Purity_Tumor_content_pathology_n97_ALL_normal", size=6, file.main, "bottomright", trait.v, cols, NULL, flip.x=1, flip.y=1, legend.title=NA)

test <- tpm.gene[genes.NBX.down.pe3, rownames(samples2)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.de <- getPCA(t(test))
file.main <- c("EAC 2 (n=97)", "14 Basal-like genes")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_EAC2_Purity_Tumor_content_pathology_n97_14-basal-like-gene", size=6, file.main, "bottomright", trait.v, cols, NULL, flip.x=1, flip.y=1, legend.title=NA)

###
##
idx <- which(is.na(purities2.surv$Estimated_purity_WES))
median(purities2.surv[-idx,]$Estimated_purity_WES)

samples2$Purity2 <- "NA"
samples2[rownames(subset(purities2.surv, Estimated_purity_WES >= 35)),]$Purity2 <- "Purity > 35"
samples2[rownames(subset(purities2.surv, Estimated_purity_WES <  35)),]$Purity2 <- "Purity < 35"

n <- which(samples2$Type == "N")
trait <- samples2[, "Purity2"]
trait[n] <- "Normal"
trait.v <- c("Purity < 35", "Purity >= 35", "Normal", "NA")
cols    <- c(blue, red, "lightgray", "lightgray")
...
pca.de <- getPCA(t(test))
file.main <- c("EAC 2 (n=97)", "Expressed genes (n=15,381)")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_EAC2_Purity_Estimated_purity_WES_n97_ALL_normal", size=6, file.main, "bottomright", trait.v, cols, NULL, flip.x=1, flip.y=1, legend.title=NA)
