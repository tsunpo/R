...
# -----------------------------------------------------------------------------
colnames <- rownames(samples.surv.sclc)
...
for (s in 1:nrow(samples.surv.sclc)) {
   samples.sclc <- samples.surv.sclc[s,]
 
   mut.dupl <- readTable(file.path(wd.driver.data, "point_mutations", paste0(samples.sclc$SAMPLE_ID, "_mutcall_filtered_ens.vcf.gz")), header=T, rownames=F, sep="")
...
save(samples.surv.sclc, mut.gene, file=file.path(wd.driver.data, "sclc-driver-mut.RData"), version=2)
...
for (g in 1:nrow(pmr.mut.gene)) {
   ...
   samples.mut.mut <- subset(samples.surv.sclc, SAMPLE_ID %in% ids)
   samples.mut.wt  <- samples.surv.sclc[setdiff(rownames(samples.surv.sclc), rownames(samples.mut.mut)),]
...
file.name <- paste0("pmr_", base, "_mut.gene_n=", nrow(samples.surv.sclc), "_s>0_mut>0_g=", nrow(pmr.mut.gene), "_chisq_SORTING_G1-vs-S")
save(pmr.mut.gene, file=file.path(wd.driver.data, paste0(file.name, ".RData")))
writeTable(pmr.mut.gene, file.path(wd.driver.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")
...
file.name <- file.path(wd.driver.plots, paste0("PMR-SORTING_", base, "_mut.gene_n=", nrow(samples.nbl), "_s>10_mut>0_g=", nrow(pmr.mut.gene.s10), "_chisq_GRID2"))
plotPMR(file.name, "Proliferation mutational rate", "PMR", text.Log10.P, pmr.mut.gene.s10, c("ESR1", "CEP112", "ATRX", "GRID2"), size=5, xmax=47, ymax=6.5)

##
pmr.mut.gene.s20 <- subset(pmr.mut.gene, SAMPLE_FREQ >= 20)
pmr.mut.gene.s20$FDR <- testFDR(pmr.mut.gene.s20$P, "Q")
file.name <- paste0("pmr_", base, "_mut.gene_n=", nrow(samples.surv.sclc), "_s>20_mut>0_g=", nrow(pmr.mut.gene.s20), "_chisq_SORTING_G1-vs-S")
writeTable(pmr.mut.gene.s20, file.path(wd.driver.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")

file.name <- file.path(wd.driver.plots, paste0("PMR-SORTING_", base, "_mut.gene_n=", nrow(samples.surv.sclc), "_s>20_mut>0_g=", nrow(pmr.mut.gene.s20), "_chisq_NCAM2"))
plotPMR(file.name, "Proliferational mutation rate", "PMR", text.Log10.P, pmr.mut.gene.s20, c("ESR1", "NCAM2"), size=5, xmax=47, ymax=6.5)
