...
## Testing for SORTING
samples.sclc.tpm <- subset(samples.sclc.tpm, M2 == 1)
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_SCLC_tpm-gene_SORTING-vs-TPM_q_n35.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.sclc.tpm, file=file.path(wd.de.data, "SRC_SCLC_tpm-gene_SORTING-vs-TPM_q_n35.RData"))
nrow(src.tpm.gene)
# [1] 33181
...
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   plotSRC(genes[g], as.numeric(tpm.gene.log2[id,]), samples.sclc.tpm$COR, 1, pos="bottomright", col=blue, size=5)
}
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_SCLC_tpm-gene_CNA-vs-TPM_q_n35.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.sclc.tpm, file=file.path(wd.de.data, "SRC_SCLC_tpm-gene_CNA-vs-TPM_q_n35.RData"))
...
## Total
de <- getCannoli(wd.de.data, BASE, 35, NULL)
#plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_SCLC-M2_TPM-CNA-SORTING_P1E03")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
#file.de <- paste0(plot.de, ".pdf")
#file.main <- c(paste0("Proliferative ", BASE), "")
...
tpm.gene.cna <- tpm.gene[total, rownames(samples.sclc.tpm)]
expressed <- rownames(removeMedian0(tpm.gene.cna))

de <- getCannoli(wd.de.data, BASE, 35, expressed)
plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_SCLC-M2_TPM-CNA-SORTING_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("Proliferative Lung-SCLC"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "topleft", c("", ""), c(red, blue), c(red, blue), fold=0)

## Not expressed
#de <- getCannoli(wd.de.data, BASE, 35, setdiff(rownames(src.tpm.gene), expressed))
#plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_SCLC-M2_TPM-CNA-SORTING_P1E03_MEDIAN0-0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")\colnames <- c("GENE", "ADJ_1", "ADJ_2")
#file.de <- paste0(plot.de, ".pdf")
#file.main <- c(paste0("Proliferative ", BASE, " (not expressed)"), "")
...
writeTable(de.positive.sig, file.path(wd.de.data, "SRC_SCLC-M2_tpm-gene_SORTING-CNA-TPM_q_n35_pos3.txt"), colnames=T, rownames=T, sep="\t")
