...
wgds <- wgds[rownames(samples.sclc.tpm),]
wgds[which(wgds$wgd_predicted == 0), ]$wgd_predicted <- "non-WGD"
wgds[which(wgds$wgd_predicted == 1), ]$wgd_predicted <- "WGD"
summary(as.factor(wgds$wgd_predicted))
# non-WGD     WGD 
#      34      36 
samples.sclc.tpm <- samples.sclc.tpm[rownames(subset(wgds, wgd_predicted == "non-WGD")),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_SCLC-WGD_tpm-gene_SORTING-vs-TPM_q_n34.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.sclc.tpm, file=file.path(wd.de.data, "SRC_SCLC-WGD_tpm-gene_SORTING-vs-TPM_q_n34.RData"))
nrow(src.tpm.gene)
# [1] 33330
# [1] 33272
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_SCLC-WGD_tpm-gene_CNA-vs-TPM_q_n34.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.sclc.tpm, file=file.path(wd.de.data, "SRC_SCLC-WGD_tpm-gene_CNA-vs-TPM_q_n34.RData"))
...
## Total
de <- getCannoli(wd.de.data, BASE, 34, NULL)
plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_SCLC-WGD_TPM-CNA-SORTING_P1E03")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("Non-WGD ", BASE), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "topleft", c("", ""), c(red, blue), c(red, blue), fold=0)

## Expressed
de <- getCannoli(wd.de.data, BASE, 34, expressed)
plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_SCLC-WGD_TPM-CNA-SORTING_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("Non-WGD ", BASE, " (expressed)"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "topleft", c("", ""), c(red, blue), c(red, blue), fold=0)

## Not expressed
de <- getCannoli(wd.de.data, BASE, 34, setdiff(rownames(src.tpm.gene), expressed))
plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_SCLC-WGD_TPM-CNA-SORTING_P1E03_MEDIAN0-0")
...
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("Non-WGD ", BASE, " (not expressed)"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "topleft", c("", ""), c(red, blue), c(red, blue), fold=0)
