...
plotPCA <- function(x, y, pca, trait, wd.de.data, file.name, size, file.main, legend, trait.v, cols, samples, flip.x, flip.y, legend.title=NA) {
   ...
   text(scores["CLL26", x]*flip.x, scores["CLL26", y]*flip.y, "CLL26", col="black", adj=c(0, -0.5), cex=1.3)
   #text(scores["CLL28", x]*flip.x, scores["CLL28", y]*flip.y, "CLL28", col="black", adj=c(0, -0.5), cex=1.5)
   #text(scores["CLL33", x]*flip.x, scores["CLL33", y]*flip.y, "CLL33", col="black", adj=c(0, -0.5), cex=1.5)
   #text(scores["CLL35", x]*flip.x, scores["CLL35", y]*flip.y, "CLL35", col="black", adj=c(0, -0.5), cex=1.5)
   text(scores["CLL45", x]*flip.x, scores["CLL45", y]*flip.y, "CLL45", col="black", adj=c(0, -0.5), cex=1.3)
   text(scores["CLL46", x]*flip.x, scores["CLL46", y]*flip.y, "CLL46", col="black", adj=c(0, -0.5), cex=1.3)
   ...
   if (is.na(legend.title))
      legend(legend, trait.v, col=cols, pch=19, pt.cex=1.4, cex=1.1)   ##bty="n")
   else
      legend(legend, title=legend.title, trait.v, col=cols, pch=19, pt.cex=1.4, cex=1.1)
