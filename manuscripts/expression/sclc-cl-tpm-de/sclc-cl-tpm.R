...
#samples <- readTable(file.path(wd.rna, "sclc-cl_rna_n44.list0"), header=F, rownames=F, sep="")
#samples <- mapply(x = 1:length(samples), function(x) unlist(strsplit(samples[x], "_"))[1])
#samples <- unique(samples)
#writeTable(samples, file.path(wd.rna, "sclc-cl_rna_n44.list"), colnames=F, rownames=F, sep="")
samples <- readTable(file.path(wd.rna, "sclc-cl_rna_n45.list"), header=F, rownames=F, sep="")
