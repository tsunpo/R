...
samples <- readTable(file.path(wd.rna, "lusc_rna_n423.txt"), header=T, rownames=T, sep="\t")
pheno  <- readTable(file.path(wd.rna, "lusc_rna_n152.txt"), header=T, rownames=T, sep="\t")
samples <- cbind(samples[rownames(pheno),], pheno[rownames(pheno),])
writeTable(samples, file.path(wd.rna, "lusc_rna_n152.txt"), colnames=T, rownames=F, sep="\t")
...
writeTable(cbind(ensGene[rownames(tpm.gene.log2), c(1,7)], tpm.gene.log2), file.path(wd.de.data, "lusc_tpm-gene-log2_n152.txt"), colnames=T, rownames=F, sep="\t")
writeTable(cbind(ensGene[rownames(tpm.gene), c(1,7)], tpm.gene), file.path(wd.de.data, "lusc_tpm-gene_n152.txt"), colnames=T, rownames=F, sep="\t")
...
writeTable(tpm.gene[,rownames(samples)], file.path(wd.de.data, "LUSC_tpm-gene_n38.txt"), colnames=T, rownames=T, sep="\t")

tpm.gene.log2 <- log2(tpm.gene + 0.01)
tpm.gene.log2 <- tpm.gene.log2[,rownames(samples)]
writeTable(tpm.gene.log2, file.path(wd.de.data, "LUSC_tpm-gene-log2_n38.txt"), colnames=T, rownames=T, sep="\t")
