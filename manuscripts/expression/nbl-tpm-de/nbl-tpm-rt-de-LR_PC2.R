...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.LR$PC2, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.LR$PC2, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_PC2-vs-TPM_q_n17.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_PC2-vs-TPM_q_n17.RData"))
nrow(src.tpm.gene)
# [1] 30331
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, NULL, TEST="PC2")
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))
length(expressed)
# [1] 22886

de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, expressed, TEST="PC2")
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL-LR-TPM-CNA-PC2_P1E03_MEDIAN0")
...
writeRNKformatCNA(rbind(de.pos.pos, de.pos.neg), wd.de.gsea, "SRC_NBL-LR_tpm-gene-median0_PC2-CNA-TPM_q_n17_GAIN")   ## GSEA
writeRNKformatCNA(rbind(de.neg.pos, de.neg.neg), wd.de.gsea, "SRC_NBL-LR_tpm-gene-median0_PC2-CNA-TPM_q_n17_LOSS")   ## GSEA
...
tpm.gene.cna <- tpm.gene[total, rownames(samples.nbl.tpm.LR)]
expressed <- rownames(removeMedian0(tpm.gene.cna))
length(expressed)
# [1] 22543

de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, expressed)
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL-LR_TPM-CNA-PC2_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("LR (n=", 17, ")"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "bottomleft", c("", ""), c(red.lighter, blue.lighter), c(red, blue), fold=0, pos="bottomright")
