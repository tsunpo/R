...
## Testing for SORTING
samples.nbl.tpm.LR <- subset(samples.nbl.tpm, RISK == "low")
...
tpm.gene <- tpm.gene[, rownames(samples.nbl.tpm.LR)]   ## VERY VERY VERY IMPORTANT!!!
...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.LR$COR, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.LR$COR, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n17.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n17.RData"))
nrow(src.tpm.gene)
# [1] 22899
...
# [1] 34895    57
cna.gene.nona.tpm.src <- cna.gene.nona.tpm[, samples.nbl.tpm.LR$SAMPLE_ID]
tpm.gene.log2.cna <- tpm.gene.log2[rownames(cna.gene.nona.tpm.src), rownames(samples.nbl.tpm.LR)]
colnames(tpm.gene.log2.cna) <- samples.nbl.tpm.LR$SAMPLE_ID
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n17.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n17.RData"))
nrow(src.tpm.gene)
# [1] 30318
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, NULL)
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))
length(expressed)
# [1] 22886

de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, expressed)
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL-LR_TPM-CNA-SORTING_P1E03_MEDIAN0")
...
save(samples.nbl.tpm, samples.nbl.tpm.LR, expressed, file=file.path(wd.de.data, "2015", "samples.nbl.tpm_LR_n17.RData"))
...
writeRNKformatCNA(rbind(de.pos.pos, de.pos.neg), wd.de.gsea, "SRC_NBL-LR_tpm-gene-median0_SORTING-CNA-TPM_q_n17_GAIN")   ## GSEA
writeRNKformatCNA(rbind(de.neg.pos, de.neg.neg), wd.de.gsea, "SRC_NBL-LR_tpm-gene-median0_SORTING-CNA-TPM_q_n17_LOSS")   ## GSEA
...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.LR$purity2, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.LR$purity2, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_Purity-vs-TPM_q_n17.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.nbl.tpm.LR, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_Purity-vs-TPM_q_n17.RData"))
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, NULL, TEST="PC2", TEST2="Purity")
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))

de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 17, expressed, TEST="PC2", TEST2="Purity")
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL-LR_TPM-Purity-PC2_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("LR (n=", 17, ")"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "bottomleft", c("", ""), c(red.lighter, blue.lighter), c(red, blue), fold=0, pos="bottomright")
