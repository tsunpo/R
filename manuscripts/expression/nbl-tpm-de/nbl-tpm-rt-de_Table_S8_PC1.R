...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "ALL_PC1", "Table_S8", "gsea_report_for_na_pos_1661290075702.tsv"), header=T, rownames=F, sep="\t")
...
file.de <- file.path(wd.de.gsea, "ALL", "Table_S8_POS.pdf")
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "ALL_PC1", "Table_S8", "gsea_report_for_na_neg_1661290075702.tsv"), header=T, rownames=F, sep="\t")
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "LR", "Table_S8", "gsea_report_for_na_pos_1661290936014.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
gsea.pos <- gsea.pos[8:1, c("NAME", "NES")]
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) convert(gsea.pos$NAME[x]))
...
main.text <- c("Dong et al. gene sets NES                        ", "")
file.de <- file.path(wd.de.gsea, "LR", "Table_S8_POS.pdf")
pdf(file.de, height=2.7, width=4)
par(mar=c(2,10.5,2,3))   # increase y-axis margin.
barplot(gsea.pos$NES, main=main.text[1], las=1, horiz=T, xlim=c(0, 4), xaxt="n", names.arg=gsea.pos$NAME, col=red.lighter, xlab="")   #cex.names=0.8) cex.axis=1.1, cex.lab=1.15, cex.main=1.3
#abline(v=2, lty=5)
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "LR", "Table_S8", "gsea_report_for_na_neg_1661290936014.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[2:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))
...
main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "LR", "Table_S8_NEG.pdf")
pdf(file.de, height=0.85, width=4)
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "HR", "Table_S8", "gsea_report_for_na_pos_1661291304495.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
main.text <- c("Dong et al. gene sets NES                        ", "")
file.de <- file.path(wd.de.gsea, "HR", "Table_S8_POS.pdf")
pdf(file.de, height=2.8, width=4)
