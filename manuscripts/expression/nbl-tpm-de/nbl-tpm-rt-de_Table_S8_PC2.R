...
file.de <- file.path(wd.de.gsea, "ALL_PC2", "Table_S8_POS_2.pdf")
pdf(file.de, height=1.5, width=4)
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "LR_PC2", "Table_S8", "gsea_report_for_na_pos_1662300738941.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
file.de <- file.path(wd.de.gsea, "LR_PC2", "Table_S8_POS_2.pdf")
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "LR_PC2", "Table_S8", "gsea_report_for_na_neg_1662300738941.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "HR_PC2", "Table_S8", "gsea_report_for_na_pos_1662014999436.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
gsea.pos <- gsea.pos[3:1, c("NAME", "NES")]
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) convert(gsea.pos$NAME[x]))
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "HR_PC2", "Table_S8", "gsea_report_for_na_neg_1662014999436.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[7:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "ALL_PC2", "Table_S8_LOSS", "gsea_report_for_na_neg_1662755068636.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
file.de <- file.path(wd.de.gsea, "ALL_PC2", "Table_S8_POS_LOSS.pdf")
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "ALL_PC2", "Table_S8_LOSS", "gsea_report_for_na_pos_1662755068636.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[5:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))
...
main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "ALL_PC2", "Table_S8_NEG_LOSS.pdf")
pdf(file.de, height=1.6, width=4)
par(mar=c(2,1.1,0,9.1))   # increase y-axis margin.

posbar <- barplot(gsea.neg$NES * -1, main=main.text[1], las=1, horiz=T, xlim=c(-6, 0), xaxt="n", names.arg="", col="lightgray", xlab="")   #cex.names=0.8) cex.axis=1.1, cex.lab=1.15, cex.main=1.3
text(y=posbar, x=0, pos=4, labels=gsea.neg$NAME, xpd=NA)
#abline(v=-2, lty=5)
axis(side=1, at=seq(-6, 0, by=2), labels=F)
axis(side=1, at=seq(-6, 0, by=2), pos=0.5, tick=F)
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "LR_PC2", "Table_S8_LOSS", "gsea_report_for_na_neg_1662755113975.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
gsea.pos <- gsea.pos[1:1, c("NAME", "NES")]
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) convert(gsea.pos$NAME[x]))

#main.text <- c("Dong et al. gene sets NES                        ", "")
main.text <- c("Expression vs. CNA (rho < 0; 37.1%)                       ", "")
file.de <- file.path(wd.de.gsea, "LR_PC2", "Table_S8_POS_LOSS.pdf")
pdf(file.de, height=1, width=4)
par(mar=c(2,11,2,2.5))   # increase y-axis margin.
barplot(gsea.pos$NES * -1, main=main.text[1], las=1, horiz=T, xlim=c(0, 4), xaxt="n", names.arg=gsea.pos$NAME, col="lightgray", xlab="")   #cex.names=0.8) cex.axis=1.1, cex.lab=1.15, cex.main=1.3
#abline(v=2, lty=5)
axis(side=1, at=seq(0, 4, by=2), labels=F)
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "LR_PC2", "Table_S8_LOSS", "gsea_report_for_na_pos_1662755113975.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[7:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))
...
main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "LR_PC2", "Table_S8_NEG_LOSS.pdf")
pdf(file.de, height=2.1, width=4)
par(mar=c(2,4.3,0,9.2))   # increase y-axis margin.
...
# -----------------------------------------------------------------------------
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "HR_PC2", "Table_S8_LOSS", "gsea_report_for_na_pos_1662755097582.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[7:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))
...
file.de <- file.path(wd.de.gsea, "HR_PC2", "Table_S8_NEG_LOSS.pdf")
pdf(file.de, height=2.5, width=4)
par(mar=c(2,1.2,2,9))   # increase y-axis margin.
posbar <- barplot(gsea.neg$NES * -1, main=main.text[1], las=1, horiz=T, xlim=c(-6, 0), xaxt="n", names.arg="", col="lightgray", xlab="")   #cex.names=0.8) cex.axis=1.1, cex.lab=1.15, cex.main=1.3
text(y=posbar, x=0, pos=4, labels=gsea.neg$NAME, xpd=NA)
#abline(v=-2, lty=5)
axis(side=1, at=seq(-6, 0, by=2), labels=F)
