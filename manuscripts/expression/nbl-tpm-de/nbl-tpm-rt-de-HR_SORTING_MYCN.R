...
## Testing for SORTING
samples.nbl.tpm.HR.MYCN <- subset(samples.nbl.tpm, GROUP_ID3 == "MYCN")
...
tpm.gene <- tpm.gene[, rownames(samples.nbl.tpm.HR.MYCN)]   ## VERY VERY VERY IMPORTANT!!!
...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.HR.MYCN$purity2, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.HR.MYCN$purity2, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_Purity-vs-TPM_q_n9.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.nbl.tpm.HR.MYCN, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_Purity-vs-TPM_q_n9.RData"))
nrow(src.tpm.gene)
# [1] 22896
...
# [1] 34895    57
cna.gene.nona.tpm.src <- cna.gene.nona.tpm[, samples.nbl.tpm.HR.MYCN$SAMPLE_ID]
tpm.gene.log2.cna <- tpm.gene.log2[rownames(cna.gene.nona.tpm.src), rownames(samples.nbl.tpm.HR.MYCN)]
colnames(tpm.gene.log2.cna) <- samples.nbl.tpm.HR.MYCN$SAMPLE_ID
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n9.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.nbl.tpm.HR.MYCN, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n9.RData"))
nrow(src.tpm.gene)
# [1] 31606
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 9, NULL, TEST="SORTING", TEST2="Purity")
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))
length(expressed)
# [1] 22882

de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 9, expressed, TEST="SORTING", TEST2="Purity")
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL-HR-MYCN_TPM-Purity-SORTING_P1E03_MEDIAN0")
...
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 9, NULL, TEST="SORTING")
...
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 9, expressed, TEST="SORTING")
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL-HR-MYCN_TPM-CNA-SORTING_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("MYCN (n=", 9, ")"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "bottomleft", c("", ""), c(red.lighter, blue.lighter), c(red, blue), fold=0, pos="bottomright")
