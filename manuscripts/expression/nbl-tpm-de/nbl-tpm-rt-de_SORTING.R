...
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_NBL_LR-HR_n54", size=6, file.main, "topright", c("HR", "LR"), c("lightgray", blue), flip.x=-1, flip.y=-1)
...
file.main <- c("NB expression", "")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_NBL_LR-HR-MYC-TERT_n54_2", size=6, file.main, "topright", c("MYCN", "TERT", "HR", "LR"), c(red, yellow, "lightgray", blue), flip.x=-1, flip.y=-1)
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n54.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n54.RData"))
...
colnames <- c("GENE", "ADJ_1", "ADJ_2")
genes0 <- c("TERT", "MYC", "MYCN", "MYCL", "NTRK1", "MKI67")
genes <- toTable(NA, length(colnames), length(genes0), colnames)
genes$GENE <- genes0
#genes[2, 2] <- 1.03

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_median0_SORTING_NTRK1")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("S (n=25) vs. G1 (n=29)", "")
