...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm$PC1, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm$PC1, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_PC1-vs-TPM_q_n54.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015", "SRC_NBL_tpm-gene_PC1-vs-TPM_q_n54.RData"))
...
for (g in 1:length(genes)) {
   ...
   plotPC1(file.path(wd.de.plots, "2015"), genes[g], as.numeric(tpm.gene.log2[id,]), samples.nbl.tpm$PC1, 1, pos="bottomright", cols=c(red, blue), size=5)
}
...
de <- getCannoli(file.path(wd.de.data, "2015"), BASE, 54, expressed, TEST="PC1")
plot.de <- file.path(wd.de.plots, "2015", "cannoliplot_SRC_NBL_TPM-CNA-PC1_P1E03_MEDIAN0")
