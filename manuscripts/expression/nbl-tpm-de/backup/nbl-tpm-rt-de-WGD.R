...
save(pca.de, file=file.path(wd.de.data, paste0("PCA_NBL_n54.RData")))

samples.nbl.tpm$GROUP_ID <- "LR"
samples.nbl.tpm[rownames(subset(samples.nbl.tpm, RISK == "high")),]$GROUP_ID <- "HR"

samples.nbl.tpm$GROUP_ID2 <- "LR"
samples.nbl.tpm[rownames(subset(subset(samples.nbl.tpm, RISK == "high"), SORTING == "G1")),]$GROUP_ID2 <- "G1-HR"
samples.nbl.tpm[rownames(subset(subset(samples.nbl.tpm, RISK == "high"), SORTING == "S")),]$GROUP_ID2  <- "S-HR"

trait <- samples.nbl.tpm$GROUP_ID
file.main <- c("NB expression", "")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_NBL_LR-HR_n54", size=6, file.main, "topright", c("HR", "LR"), c(yellow, "skyblue"), flip.x=1, flip.y=1)

trait <- samples.nbl.tpm$GROUP_ID2
file.main <- c("NB expression", "")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_NBL_LR-HR-S-G1_n54_blue", size=6, file.main, "topright", c("S-HR", "G1-HR", "LR"), c(red, yellow, blue), flip.x=1, flip.y=1)

trait <- as.vector(samples.nbl.tpm$SORTING)
file.main <- c("NB expression", "")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_NBL_G1-S_n54", size=6, file.main, "topright", c("S", "G1"), c(red, blue), flip.x=1, flip.y=1)
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n53.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n53.RData"))
...
for (g in 1:length(genes)) {
   ...
   plotSRC(genes[g], as.numeric(tpm.gene.log2[id,]), samples.nbl.tpm$COR, 1, pos="bottomright", col=red, size=5)
}

genes <- c("MYC", "MYCN", "MYCL", "KIF1B", "SMG6")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   plotSRC(genes[g], as.numeric(tpm.gene.log2[id,]), samples.nbl.tpm$COR, 1, pos="bottomright", col=blue, size=5)
}
...
# [1] 34895    56
cna.gene.nona.tpm.src <- cna.gene.nona.tpm[, samples.nbl.tpm$SAMPLE_ID]
tpm.gene.log2.cna <- tpm.gene.log2[rownames(cna.gene.nona.tpm.src), rownames(samples.nbl.tpm)]
#colnames(tpm.gene.log2.cna) <- samples.nbl.tpm$SAMPLE_ID
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n53.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n53.RData"))
...
###
##
genes <- c("TERT", "MACF1", "MYC", "MYCN", "MYCL", "KIF1B", "SMG6")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   plotCNA(genes[g], as.numeric(tpm.gene.log2.cna[id,]), as.numeric(cna.gene.nona.tpm.src[id,]), 1, pos="bottomright", col=red, size=5)
}

genes <- c("KAL1", "HIST1H4I", "IGKV1-9", "FANCI")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   plotCNA(genes[g], as.numeric(tpm.gene.log2.cna[id,]), as.numeric(cna.gene.nona.tpm.src[id,]), 1, pos="bottomright", col=blue, size=5)
}
...
genes0 <- c("TERT", "MYC", "MYCN", "MYCL", "SMG6", "KIF1B")
...
## Expressed
de <- getCannoli(wd.de.data, BASE, 53, NULL)
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))

de <- getCannoli(wd.de.data, BASE, 53, expressed)
plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_NBL_TPM-CNA-SORTING_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("Neuroblastoma (n=", 53, ")"), "")
plotCannoli(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, "topright", c("", ""), c(red, blue), c(red, blue), fold=0, pos="bottomleft")
...
writeTable(rownames(de.negative.sig), file.path(wd.de.data, "SRC_NBL_tpm-gene_SORTING-CNA-TPM_q_n53_neg351.txt"), colnames=F, rownames=F, sep="")
...
# -----------------------------------------------------------------------------
load(file=file.path(wd.de.data, "SRC_NBL_tpm-gene-r5p47_CNA-vs-TPM_q_n54.RData"))
de1 <- src.tpm.gene
de1$Effect <- de1$RHO
load(file=file.path(wd.de.data, "SRC_NBL_tpm-gene-r5p47_SORTING_q_n54.RData"))
de2 <- src.tpm.gene
de2$Effect <- de2$RHO
...
plot.de <- file.path(wd.de.plots, "cannoliplot_SRC_NBL_r5p47_TPM-CNA-SORTING_TEST")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NB samples (n=54)", "")
plotCannoli(de1, de2, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, "topright", c("", ""), c(blue, red), c(blue, red), fold=0, pos="bottomleft")
...
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_RISK_wilcox_q_n54")
...
##
genes <- c("TERT", "ESR1", "MYC", "MYCL", "MYCN", "STRA6")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
...
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_SORTING_wilcox_q_n54")
file.main <- paste0("", BASE)
...
plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_SORTING_TERT")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("S (n=25) vs. G1 (n=29)", "")
plotVolcano(de.tpm.gene, 1, genes, file.de, file.main, xlab.text, ylab.text, "topright", c("S > G1", "S < G1"), c(adjustcolor.red, adjustcolor.blue), c(red, blue), fold=0)

##
genes <- c("TERT", "ESR1", "MYC", "MYCL", "MYCN", "STRA6")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   tpm.1 <- as.numeric(tpm.gene.log2[id, rownames(subset(samples.nbl.tpm, SORTING == "G1"))])
   tpm.2 <- as.numeric(tpm.gene.log2[id, rownames(subset(samples.nbl.tpm, SORTING == "S"))])

   file.name <- file.path(wd.de.plots, paste0("boxplot_TPM_SORTING_", genes[g]))
   plotBox(file.name, tpm.1, tpm.2, genes[g], names=c("G1", "S"), cols=c(blue, red), h=5, w=5)
}
...
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   tpm.1 <- as.numeric(sclc.tpm.gene.log2[id,])
   tpm.2 <- as.numeric(nbl.tpm.gene.log2[id,])  
 
   file.name <- file.path(wd.de.plots, paste0("boxplot-NBL-vs-SCLC_TPM_", genes[g]))
   plotBox(file.name, tpm.1, tpm.2, genes[g], names=c("SCLC", "NB"), cols=c("lightgray", "dimgray"), h=5, w=5)
}

icgc.tpm.gene.log2 <- tpm.gene.log2
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   tpm.1 <- as.numeric(icgc.tpm.gene.log2[id,])
   tpm.2 <- as.numeric(nbl.tpm.gene.log2[id,])  
 
   file.name <- file.path(wd.de.plots, paste0("boxplot-NBL-vs-PCAWG_TPM_", genes[g]))
   plotBox(file.name, tpm.1, tpm.2, genes[g], names=c("PCAWG", "NB"), cols=c("white", "dimgray"), h=5, w=5)
}

for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   tpm.1 <- as.numeric(icgc.tpm.gene.log2[id,])
   tpm.2 <- as.numeric(sclc.tpm.gene.log2[id,])  
 
   file.name <- file.path(wd.de.plots, paste0("boxplot-SCLC-vs-PCAWG_TPM_", genes[g]))
   plotBox(file.name, tpm.1, tpm.2, genes[g], names=c("PCAWG", "SCLC"), cols=c("white", "lightgray"), h=5, w=5)
}
...
xlab.text <- expression("S/G1 fold change " * "[log" * ""[2] * "]")
ylab.text <- expression("Significance " * "[-log" * ""[10] * "(" * italic("P") * ")]")

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_SORTING_p1e-3_NCAM2")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("G1 (n=29) vs. S (n=24)", "")
plotVolcano(de.tpm.gene, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, c("19 S > G1", "148 S < G1"))
...
##       Student's/t.test
## FDR : Q/BH
## DE  : 25 S-HR vs. 12 G1-HR
argv      <- data.frame(predictor="SORTING", predictor.wt="G1", test="Wilcoxon", test.fdr="Q", stringsAsFactors=F)
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_SORTING_wilcox_q_n37")
file.main <- paste0("", BASE)

samples.nbl.tpm.high <- subset(samples.nbl.tpm, RISK == "high")
tpm.gene.log2.high <- tpm.gene.log2[, rownames(samples.nbl.tpm.high)]
de <- differentialAnalysis(tpm.gene.log2.high, samples.nbl.tpm.high, argv$predictor, argv$predictor.wt, argv$test, argv$test.fdr)
...
genes <- c("TERT", "CEP112", "ESR1", "MYCL", "MYCN", "MYC", "STRA6")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   tpm.1 <- as.numeric(tpm.gene.log2[id, rownames(subset(samples.nbl.tpm, SORTING == "S"))])
   tpm.2 <- as.numeric(tpm.gene.log2[id, rownames(subset(samples.nbl.tpm, SORTING == "G1"))])
 
   file.name <- file.path(wd.de.plots, paste0("boxplot-sorting_TPM_", genes[g]))
   plotBox(file.name, tpm.1, tpm.2, genes[g], names=c("G1", "S"), cols=c(blue, red), h=5, w=5)
}
...
##       Student's/t.test
## FDR : Q/BH
## DE  : 24 High-risk NBL G1 vs. 12 High-risk NBL S
argv      <- data.frame(predictor="RISK", predictor.wt="low", test="Wilcoxon", test.fdr="Q", stringsAsFactors=F)
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_RISK_wilcox_q_n29")
file.main <- paste0("", BASE)

samples.nbl.tpm.G1 <- subset(samples.nbl.tpm, SORTING == "G1")
tpm.gene.log2.G1 <- tpm.gene.log2[, rownames(samples.nbl.tpm.G1)]
de <- differentialAnalysis(tpm.gene.log2.G1, samples.nbl.tpm.G1, argv$predictor, argv$predictor.wt, argv$test, argv$test.fdr)
...
colnames <- c("P", "FDR", "LR", "HR_G1", "HR_S", "FC_G1_L", "FC_S_L")
de <- toTable(0, length(colnames), nrow(tpm.gene.log2), colnames)
rownames(de) <- rownames(tpm.gene.log2)

## ANNOVA
de$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) testANOVA(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm$GROUP_ID2))

## Log2 fold change
de$LR <- median00(tpm.gene.log2, rownames(subset(samples.nbl.tpm, GROUP_ID2 == "LR")))
de$HR_G1 <- median00(tpm.gene.log2, rownames(subset(samples.nbl.tpm, GROUP_ID2 == "HR-G1")))
de$HR_S <- median00(tpm.gene.log2, rownames(subset(samples.nbl.tpm, GROUP_ID2 == "HR-S")))
de$FC_G1_L <- de$HR_G1 - de$LR
de$FC_S_L <- de$HR_S - de$LR
...
## Ensembl gene annotations
annot <- ensGene[,c("ensembl_gene_id", "external_gene_name", "chromosome_name", "strand", "start_position", "end_position", "gene_biotype")]
anova.tpm.gene <- cbind(annot[rownames(de),], de)   ## BE EXTRA CAREFUL!!

save(anova.tpm.gene, samples, file=file.path(wd.de.data, "ANOVA_NBL_tpm-gene-r5p47_q_n53.RData"))
writeTable(anova.tpm.gene, file.path(wd.de.data, "ANOVA_NBL_tpm-gene-r5p47_q_n53.txt"), colnames=T, rownames=F, sep="\t")
...
plotVolcano <- function(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, overlaps.up.pe3, overlaps.up.pe3.iz.e.cd, legend, cols, cols2) {
   ...
   de.up   <- subset(de.sig, LOG2_FC > 1.5)
   points(de.up$LOG2_FC, de.up$log10P, pch=16, col=cols[1], cex=1.7)
   de.down <- subset(de.sig, LOG2_FC < -1.5)
   ...
   legend("topright", legend=legend, col=cols2, pch=19, pt.cex=2, cex=1.6)
   dev.off()
}
...
# -----------------------------------------------------------------------------
risk.tpm.gene <- de.tpm.gene.nbl
risk.up.pe3.fc1   <- rownames(subset(subset(risk.tpm.gene, P <= 1E-3), LOG2_FC >= 1))
risk.down.pe3.fc1 <- rownames(subset(subset(risk.tpm.gene, P <= 1E-3), LOG2_FC <= -1))

rmr.mut.gene.s10[intersect(rownames(subset(rmr.mut.gene.s10, P < 1E-2)), c(risk.up.pe3, risk.down.pe3)),]

rmr.mut.gene.s10[intersect(rownames(subset(rmr.mut.gene.s10, P < 1E-2)), c(risk.up.pe3.fc1, risk.down.pe3.fc1)),]
risk.tpm.gene[intersect(rownames(subset(rmr.mut.gene.s10, P < 1E-2)), c(risk.up.pe3.fc1, risk.down.pe3.fc1)),]


risk.tpm.gene.G1 <- de.tpm.gene
risk.up.pe3.G1   <- rownames(subset(subset(risk.tpm.gene.G1, P <= 1E-3), LOG2_FC >= 0))
risk.down.pe3.G1 <- rownames(subset(subset(risk.tpm.gene.G1, P <= 1E-3), LOG2_FC < 0))

##
sorting.tpm.gene <- de.tpm.gene
sorting.up.pe3.fc0.58   <- rownames(subset(subset(sorting.tpm.gene, P <= 1E-3), LOG2_FC >= log2(1.5)))
sorting.down.pe3.fc0.58 <- rownames(subset(subset(sorting.tpm.gene, P <= 1E-3), LOG2_FC <= -log2(1.5)))

pmr.mut.gene.s10[intersect(rownames(subset(pmr.mut.gene.s10, P < 1E-2)), c(sorting.up.pe3.fc1, sorting.down.pe3.fc1)),]
sorting.tpm.gene[intersect(rownames(subset(pmr.mut.gene.s10, P < 1E-2)), c(sorting.up.pe3.fc1, sorting.down.pe3.fc1)),]

###
## 1st tier
xlab.text <- expression("HR/LR fold change " * "[log" * ""[2] * "]")
ylab.text <- expression("Significance " * "[-log" * ""[10] * "(" * italic("P") * ")]")

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_RISK_p1e-3_GRID2")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("LR (n=17) vs. HR (n=38)", "")
plotVolcano(risk.tpm.gene, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, NA, NA, c("282 Up", "702 Down"), cols=c(adjustcolor(yellow, alpha.f=0.5), adjustcolor("skyblue", alpha.f=0.5)), cols2=c(yellow, "skyblue"))

##
xlab.text <- expression("HR/LR fold change " * "[log" * ""[2] * "]")
ylab.text <- expression("Significance " * "[-log" * ""[10] * "(" * italic("P") * ")]")

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_RISK-G1_p1e-3_GRID2")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("LR (n=17) vs. HR-G1 (n=12)", "")
plotVolcano(risk.tpm.gene.G1, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, NA, NA, c("54 Up", "94 Down"), cols=c(adjustcolor(yellow, alpha.f=0.5), adjustcolor("skyblue", alpha.f=0.5)), cols2=c(yellow, "skyblue"))
...
plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_SORTING_p1e-3_ESR1")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NB proliferative genes", "")
plotVolcano(sorting.tpm.gene, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, NA, NA, c("390 Up", "1,036 Down"), cols=c(adjustcolor(red, alpha.f=0.5), adjustcolor(blue, alpha.f=0.5)), cols2=c(red, blue))
...
overlaps.up.pe3   <- rownames(subset(subset(de.tpm.gene, P <= 1E-3), LOG2_FC >= 0))
overlaps.down.pe3 <- rownames(subset(subset(de.tpm.gene, P <= 1E-3), LOG2_FC < 0))

## DE + SRC
overlaps.up.pe3.pe3   <- intersect(rownames(subset(subset(de.tpm.gene, P <= 1E-3), LOG2_FC >= 0)), rownames(subset(subset(src.tpm.gene, P <= 1E-3), LOG2_FC >= 0)))
overlaps.down.pe3.pe3 <- intersect(rownames(subset(subset(de.tpm.gene, P <= 1E-3), LOG2_FC < 0)),  rownames(subset(subset(src.tpm.gene, P <= 1E-3), LOG2_FC < 0)))
overlaps.pe3.pe3      <- c(overlaps.up.pe3.pe3, overlaps.down.pe3.pe3)
...
genes.iz.e <- c(genes.iz.e.cd$ensembl_gene_id, genes.iz.e.ho$ensembl_gene_id)



###
## 1st tier
xlab.text <- expression("High/Low-risk fold change " * "[log" * ""[2] * "]")
ylab.text <- expression("Significance " * "[-log" * ""[10] * "(" * italic("P") * ")]")

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_DE_p1e-3_GRID2")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NB high-risk genes", "")
plotVolcano(de.tpm.gene.nbl, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, NA, NA, "282 High-risk", cols=adjustcolor(yellow, alpha.f=0.5), cols2=yellow)

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_DE+DE_p1e-3_CEP112_H+P")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NB high-risk genes", "")
plotVolcano(de.tpm.gene.nbl, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, overlaps.up.pe3.pe3, NA, "144 High-risk and proliferative", c(adjustcolor.gray, adjustcolor(red, alpha.f=0.5)), red)

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_DE+DE_p1e-3_CEP112_H")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NB high-risk genes", "")
plotVolcano(de.tpm.gene.nbl, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, nbl, NA, "138 High risk-specific", c(adjustcolor.gray, adjustcolor(yellow, alpha.f=0.5)), yellow)

####
## 2nd tier
plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_DE+DE_p1e-3_CEP112")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NBL high-risk genes", "")
plotVolcano(de.tpm.gene.nbl, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, overlaps.up.pe3.pe3, NA, c("138 High-risk", "144 High-risk and proliferative"), c(adjustcolor(yellow, alpha.f=0.5), adjustcolor(red, alpha.f=0.5)), c(yellow, red))

nbl <- setdiff(overlaps.up.pe3, overlaps.up.pe3.pe3)
#writeTable(de.tpm.gene[overlaps.up.pe3.pe3,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_p1e3_n53_S_n154.txt"), colnames=T, rownames=F, sep="\t")
#writeTable(de.tpm.gene[nbl,],                 file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_p1e3_n53_H_n128.txt"), colnames=T, rownames=F, sep="\t")
#save(overlaps.up.pe3, overlaps.down.pe3, overlaps.up.pe3.pe3, overlaps.down.pe3.pe3, file=file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_p1e3_n53.RData"))

plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_DE+DE_p1e-3_CEP112")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NB high-risk genes", "")
plotVolcano(de.tpm.gene.nbl, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, overlaps.up.pe3.pe3, nbl, c("138 High-risk", "144 High-risk and proliferative"), c("white", adjustcolor(red, alpha.f=0.5), yellow), cols2=c(yellow, red))

####
## 3rd tier
plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_r5p47_DE+SRC+IZ_p1e-3_TERT")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("NBL high-risk genes", "")
plotVolcano(de.tpm.gene.nbl, 1E-3, genes, file.de, file.main, xlab.text, ylab.text, overlaps.up.pe3.pe3, intersect(overlaps.up.pe3, genes.iz.e), c("128 High-risk", "154 High-risk and proliferative", "21+22 Early IZ"), c(yellow, red, "black"))

# -----------------------------------------------------------------------------
# 
# Last Modified: 01/05/22
# -----------------------------------------------------------------------------
de.tpm.gene.nbl  <- de.tpm.gene
src.tpm.gene.nbl <- src.tpm.gene
load("/Users/tpyang/Work/uni-koeln/tyang2/ICGC/analysis/expression/icgc-tpm-de/data/de_icgc_tpm-gene-median0_SG1_wilcox_q_n904.RData")

xlab <- "High/Low-risk fold change"
ylab <- "Proliferative/Resting (ICGC)"
#cycle.icgc <- intersect(overlaps.up.pe3.pe3, rownames(subset(de.tpm.gene, LOG2_FC >= 0)))
cycle.icgc <- intersect(overlaps.up.pe3.pe3, rownames(de.tpm.gene))
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_S_g151"))
x <- de.tpm.gene.nbl[cycle.icgc,]$LOG2_FC
y <- de.tpm.gene[cycle.icgc,]$LOG2_FC
plotCorrelation(file.name, "151 High-risk, S genes", xlab, ylab, x, y, pos="bottomright", cols=c(red, red), size=5)

#nbl.icgc <- intersect(nbl, rownames(subset(de.tpm.gene, LOG2_FC >= 0)))
nbl.icgc <- intersect(nbl, rownames(de.tpm.gene))
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_H_g115"))
x <- de.tpm.gene.nbl[nbl.icgc,]$LOG2_FC
y <- de.tpm.gene[nbl.icgc,]$LOG2_FC
plotCorrelation(file.name, "115 High-risk genes", xlab, ylab, x, y, pos="bottomright", cols=c(yellow, yellow), size=5)

## CD genes
overlaps <- intersect(cycle.icgc, rownames(subset(tpm.gene.log2.m.rfd, TRC > 0)))
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_S-CD_g", 69))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "69 High-risk, S, CD", xlab, ylab, x, y, pos="bottomright", cols=c(red, red), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_S-CD_n69.txt"), colnames=T, rownames=F, sep="\t")

overlaps <- intersect(nbl.icgc, rownames(subset(tpm.gene.log2.m.rfd, TRC > 0)))
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_H-CD_g57"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "57 High-risk, CD", xlab, ylab, x, y, pos="bottomright", cols=c(yellow, yellow), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_H-CD_n57.txt"), colnames=T, rownames=F, sep="\t")

## HO genes
overlaps <- intersect(cycle.icgc, rownames(subset(tpm.gene.log2.m.rfd, TRC < 0)))
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_S-HO_g73"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "73 High-risk, S, HO", xlab, ylab, x, y, pos="bottomright", cols=c(red, red), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_S-HO_n73.txt"), colnames=T, rownames=F, sep="\t")

overlaps <- intersect(nbl.icgc, rownames(subset(tpm.gene.log2.m.rfd, TRC < 0)))
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_H-HO_g50"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "50 High-risk, HO", xlab, ylab, x, y, pos="bottomright", cols=c(yellow, yellow), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_H-HO_n50.txt"), colnames=T, rownames=F, sep="\t")
...
## Early genes
overlaps <- intersect(cycle.icgc, genes.iz.e)
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_S-IZ_g22"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "22 High-risk, S, early IZ", xlab, ylab, x, y, pos="bottomright", cols=c("dimgray", "black"), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_S-IZ_n22.txt"), colnames=T, rownames=F, sep="\t")

overlaps <- intersect(nbl.icgc, genes.iz.e)
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_H-IZ_g21"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "21 High-risk, early IZ", xlab, ylab, x, y, pos="bottomright", cols=c("dimgray", "black"), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_H-IZ_n21.txt"), colnames=T, rownames=F, sep="\t")

## Early CD genes
overlaps <- intersect(cycle.icgc, genes.iz.e.cd$ensembl_gene_id)
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_S-IZ-CD_g11"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "11 High-risk, S, early IZ, CD", xlab, ylab, x, y, pos="bottomright", cols=c("dimgray", "black"), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_S-IZ-CD_n11.txt"), colnames=T, rownames=F, sep="\t")

overlaps <- intersect(nbl.icgc, genes.iz.e.cd$ensembl_gene_id)
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_H-IZ-CD_g7"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "7 High-risk, early IZ, CD", xlab, ylab, x, y, pos="bottomright", cols=c("dimgray", "black"), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_H-IZ-CD_n7.txt"), colnames=T, rownames=F, sep="\t")

## Early HO genes
overlaps <- intersect(cycle.icgc, genes.iz.e.ho$ensembl_gene_id)
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_S-IZ-HO_g11"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "11 High-risk, S, early IZ, HO", xlab, ylab, x, y, pos="bottomright", cols=c("dimgray", "black"), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_S-IZ-HO_n11.txt"), colnames=T, rownames=F, sep="\t")

overlaps <- intersect(nbl.icgc, genes.iz.e.ho$ensembl_gene_id)
file.name <- file.path(wd.de.plots, paste0("Correlation_NBL-vs-ICGC_ALL_H-IZ-HO_g14"))
x <- de.tpm.gene.nbl[overlaps,]$LOG2_FC
y <- de.tpm.gene[overlaps,]$LOG2_FC
plotCorrelation(file.name, "14 High-risk, early IZ, HO", xlab, ylab, x, y, pos="bottomright", cols=c("dimgray", "black"), size=5)
writeTable(de.tpm.gene.nbl[overlaps,], file.path(wd.de.data, "NBL_tpm-gene-r5p47_DE+SRC_ALL_p1e3_n53_H-IZ-HO_n14.txt"), colnames=T, rownames=F, sep="\t")
...
## DE  : 
argv      <- data.frame(predictor="SORTING", predictor.wt="G1", test="Wilcoxon", test.fdr="Q", stringsAsFactors=F)
file.name <- paste0("de_", base, "_tpm-gene-r5p47_SORTING_wilcox_q_n53")
file.main <- paste0("", BASE)

de <- differentialAnalysis(tpm.gene.log2, samples.nbl.tpm, argv$predictor, argv$predictor.wt, argv$test, argv$test.fdr)
# Samples with MUT SORTING: 24
# Samples with  WT SORTING: 29
...
## 13/09/21
overlaps <- intersect(samples.rna$V2, samples.wgs$SAMPLE_ID)
overlaps.atrx <- intersect(overlaps, rownames(phenos.atrx))
samples.atrx  <- cbind(samples.rna[overlaps.atrx,], samples.wgs[overlaps.atrx,])
samples.atrx <- cbind(samples.atrx, phenos.atrx[overlaps.atrx,])
samples.atrx$M2 <- as.factor(samples.atrx$M2)
rownames(samples.atrx) <- samples.atrx$V1
...
plot.de <- file.path(wd.de.plots, "volcanoplot_NBL_median1_DE_p1e-3_TERT")
genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
