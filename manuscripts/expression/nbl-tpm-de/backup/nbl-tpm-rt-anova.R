...
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_RISK_wilcox_q_n53")
...
###
##
genes <- c("GRID2", "TERT", "CEP112", "ESR1", "MYCL", "MYCN", "MYC", "STRA6")
...
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_SORTING_wilcox_q_n53")
file.main <- paste0("", BASE)
...
genes <- c("TERT", "CEP112", "ESR1", "MYCL", "MYCN", "MYC", "STRA6", "GRID2")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
...
argv      <- data.frame(predictor="SORTING", predictor.wt="G1", test="Wilcoxon", test.fdr="Q", stringsAsFactors=F)
file.name <- paste0("DE_", "NBL", "_tpm-gene-r5p47_SORTING_wilcox_q_n36")
file.main <- paste0("", BASE)
...
writeTable(src.tpm.gene, file.path(wd.de.data, "SRC_NBL_tpm-gene-r5p47_SORTING_q_n53.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "SRC_NBL_tpm-gene-r5p47_SORTING_q_n53.RData"))
nrow(src.tpm.gene)
...
risk.up.pe3   <- rownames(subset(subset(risk.tpm.gene, P <= 1E-3), LOG2_FC >= 0))
risk.down.pe3 <- rownames(subset(subset(risk.tpm.gene, P <= 1E-3), LOG2_FC < 0))
...
sorting.up.pe3   <- rownames(subset(subset(sorting.tpm.gene, P <= 1E-3), LOG2_FC >= 0))
sorting.down.pe3 <- rownames(subset(subset(sorting.tpm.gene, P <= 1E-3), LOG2_FC < 0))
