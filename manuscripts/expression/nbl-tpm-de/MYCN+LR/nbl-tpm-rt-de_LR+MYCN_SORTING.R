...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.MYCN_LR$COR, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.MYCN_LR$COR, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n26.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n26.RData"))
...
for (g in 1:length(genes)) {
   ...
   plotSRC(file.path(wd.de.plots, "2015/MYCN+LR"), genes[g], as.numeric(tpm.gene.log2[id,]), samples.nbl.tpm.MYCN_LR$COR, 1, pos="bottomright", cols=c(red, blue), size=5)
}
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015/MYCN+LR"), BASE, 26, NULL, TEST="SORTING")
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))

de <- getCannoli(file.path(wd.de.data, "2015/MYCN+LR"), BASE, 26, expressed, TEST="SORTING")
plot.de <- file.path(wd.de.plots, "2015/MYCN+LR", "cannoliplot_SRC_NBL_TPM-CNA-SORTING_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
...
writeRNKformatCNA(rbind(de.pos.pos, de.pos.neg), wd.de.gsea, "/MYCN+LR/SORTING/SRC_NBL_tpm-gene-median0_SORTING-CNA-TPM_q_n26_GAIN")   ## GSEA
writeRNKformatCNA(rbind(de.neg.pos, de.neg.neg), wd.de.gsea, "/MYCN+LR/SORTING/SRC_NBL_tpm-gene-median0_SORTING-CNA-TPM_q_n26_LOSS")   ## GSEA
