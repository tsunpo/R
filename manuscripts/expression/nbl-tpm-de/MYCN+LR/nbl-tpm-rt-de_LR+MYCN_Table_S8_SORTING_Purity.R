...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "MYCN+LR/SORTING", "Table_S8", "gsea_report_for_na_pos_1661765507366.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
main.text <- c("Dong et al. gene sets NES                        ", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/SORTING", "Table_S8_POS.pdf")
pdf(file.de, height=1.75, width=4)
par(mar=c(2,11.5,2,2))   # increase y-axis margin.
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "MYCN+LR/SORTING", "Table_S8", "gsea_report_for_na_neg_1661765507366.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[6:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))

main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/SORTING", "Table_S8_NEG.pdf")
pdf(file.de, height=2.1, width=4)
par(mar=c(2.8,1.7,0,8.6))   # increase y-axis margin.
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "MYCN+LR/LR", "Table_S8", "gsea_report_for_na_pos_1661773242031.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
main.text <- c("Dong et al. gene sets NES                        ", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/LR", "Table_S8_POS.pdf")
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "MYCN+LR/LR", "Table_S8", "gsea_report_for_na_neg_1661773242031.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
...
main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/LR", "Table_S8_NEG.pdf")
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "MYCN+LR/MYCN", "Table_S8", "gsea_report_for_na_pos_1661774774868.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
main.text <- c("Dong et al. gene sets NES                        ", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/MYCN", "Table_S8_POS.pdf")
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "MYCN+LR/MYCN", "Table_S8", "gsea_report_for_na_neg_1661774774868.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
gsea.neg <- gsea.neg[9:1, c("NAME", "NES")]
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) convert(gsea.neg$NAME[x]))

main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/MYCN", "Table_S8_NEG.pdf")
...
# -----------------------------------------------------------------------------
## POS
gsea.pos <- readTable(file.path(wd.de.gsea, "MYCN+LR/TERT", "Table_S8", "gsea_report_for_na_pos_1662152024922.tsv"), header=T, rownames=F, sep="\t")
#gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) unlist(strsplit(gsea.pos$NAME[x], "_"))[2])
gsea.pos$NAME <- mapply(x = 1:nrow(gsea.pos), function(x) gsub("_", " ", gsea.pos$NAME[x]))
...
main.text <- c("Dong et al. gene sets NES                        ", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/TERT", "Table_S8_POS.pdf")
...
## NEG
gsea.neg <- readTable(file.path(wd.de.gsea, "MYCN+LR/TERT", "Table_S8", "gsea_report_for_na_neg_1662152024922.tsv"), header=T, rownames=F, sep="\t")
#gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) unlist(strsplit(gsea.neg$NAME[x], "HALLMARK_"))[2])
gsea.neg$NAME <- mapply(x = 1:nrow(gsea.neg), function(x) gsub("_", " ", gsea.neg$NAME[x]))
...
main.text <- c("", "")
file.de <- file.path(wd.de.gsea, "MYCN+LR/TERT", "Table_S8_NEG.pdf")
