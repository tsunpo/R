...
samples.nbl.tpm.MYCN_LR.MYCN <- subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "MYCN")
...
tpm.gene <- tpm.gene[, rownames(samples.nbl.tpm.MYCN_LR.MYCN)]   ## VERY VERY VERY IMPORTANT!!!
...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.MYCN_LR.MYCN$COR, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.MYCN_LR.MYCN$COR, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n9.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_SORTING-vs-TPM_q_n9.RData"))
...
dim(tpm.gene.log2)
# [1] 34908     9
cna.gene.nona.tpm.src <- cna.gene.nona.tpm[, samples.nbl.tpm.MYCN_LR.MYCN$SAMPLE_ID]
tpm.gene.log2.cna <- tpm.gene.log2[rownames(cna.gene.nona.tpm.src), rownames(samples.nbl.tpm.MYCN_LR.MYCN)]
colnames(tpm.gene.log2.cna) <- samples.nbl.tpm.MYCN_LR.MYCN$SAMPLE_ID
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n9.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.nbl.tpm.MYCN_LR, file=file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n9.RData"))
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015/MYCN+LR"), BASE, 9, NULL, TEST="SORTING")
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))

de <- getCannoli(file.path(wd.de.data, "2015/MYCN+LR"), BASE, 9, expressed, TEST="SORTING")
plot.de <- file.path(wd.de.plots, "2015/MYCN+LR", "cannoliplot_SRC_NBL_TPM-CNA-SORTING_P1E03_MEDIAN0_MYCN")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
file.main <- c(paste0("MYCN (n=", 9, ")"), "")
...
writeRNKformatCNA(rbind(de.pos.pos, de.pos.neg), wd.de.gsea, "/MYCN+LR/MYCN/SRC_NBL_tpm-gene-median0_SORTING-CNA-TPM_q_n9_GAIN")   ## GSEA
writeRNKformatCNA(rbind(de.neg.pos, de.neg.neg), wd.de.gsea, "/MYCN+LR/MYCN/SRC_NBL_tpm-gene-median0_SORTING-CNA-TPM_q_n9_LOSS")   ## GSEA
