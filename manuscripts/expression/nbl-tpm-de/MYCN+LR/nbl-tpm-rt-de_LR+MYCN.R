...
# -----------------------------------------------------------------------------
test <- tpm.gene[, rownames(samples.nbl.tpm.MYCN_LR)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.de <- getPCA(t(test))
save(pca.de, file=file.path(wd.de.data, "2015/MYCN+LR", paste0("PCA_NBL_n26_median0.RData")))

trait <- samples.nbl.tpm.MYCN_LR$GROUP_ID3
file.main <- c("LR + MYCN (n=26)", "")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "2015/MYCN+LR/PCA_NBL_LR-MYCN_n26", size=6, file.main, "topright", c("MYCN", "LR"), c(red, blue), flip.x=-1, flip.y=-1)

scores.de <- pcaScores(pca.de)
samples.nbl.tpm.MYCN_LR$PC1 <- scores.de$PC1 * -1
samples.nbl.tpm.MYCN_LR$PC2 <- scores.de$PC2 * -1

## PC1
x <- samples.nbl.tpm.MYCN_LR$PC1
y <- samples.nbl.tpm.MYCN_LR$COR
file.name <- file.path(wd.de.plots, "2015/MYCN+LR", paste0("correlation_PCA-NBL-TPM-MEDIAN0_PC1-vs-SORTING"))
plotCorrelation(file.name, "LR + MYCN (n=26)", paste0("PC", 1, " (", pcaProportionofVariance(pca.de, 1), "%)"), "S-phase cell fraction", x, y, size=5)

x <- samples.nbl.tpm.MYCN_LR$PC1
y <- samples.nbl.tpm.MYCN_LR$purity2
file.name <- file.path(wd.de.plots, "2015/MYCN+LR", paste0("correlation_PCA-NBL-TPM-MEDIAN0_PC1-vs-purity2"))
plotCorrelation(file.name, "LR + MYCN (n=26)", paste0("PC", 1, " (", pcaProportionofVariance(pca.de, 1), "%)"), "Purity", x, y, size=5)

## PC2
x <- samples.nbl.tpm.MYCN_LR$PC2
y <- samples.nbl.tpm.MYCN_LR$COR
file.name <- file.path(wd.de.plots, "2015/MYCN+LR", paste0("correlation_PCA-NBL-TPM-MEDIAN0_PC2-vs-SORTING"))
plotCorrelation(file.name, "LR + MYCN (n=26)", paste0("PC", 2, " (", pcaProportionofVariance(pca.de, 2), "%)"), "S-phase cell fraction", x, y, size=5)

x <- samples.nbl.tpm.MYCN_LR$PC2
y <- samples.nbl.tpm.MYCN_LR$purity2
file.name <- file.path(wd.de.plots, "2015/MYCN+LR", paste0("correlation_PCA-NBL-TPM-MEDIAN0_PC2-vs-purity2"))
plotCorrelation(file.name, "LR + MYCN (n=26)", paste0("PC", 2, " (", pcaProportionofVariance(pca.de, 2), "%)"), "Purity", x, y, size=5)

## SORTING
x <- samples.nbl.tpm.MYCN_LR$purity2
y <- samples.nbl.tpm.MYCN_LR$COR
file.name <- file.path(wd.de.plots, "2015/MYCN+LR", paste0("correlation_PCA-NBL-TPM-MEDIAN0_SORTING-vs-purity2"))
plotCorrelation(file.name, "LR + MYCN (n=26)", "Purity", "S-phase cell fraction", x, y, size=5)

# -----------------------------------------------------------------------------
# 
# Last Modified: 28/08/22
# -----------------------------------------------------------------------------
median(subset(samples.nbl.tpm.MYCN_LR, RISK == "high")$purity2)
median(subset(samples.nbl.tpm.MYCN_LR, RISK == "low")$purity2)
# [1] 0.912
# [1] 0.893

median(subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "MYCN")$purity2)
median(subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "LR")$purity2)
# [1] 0.912
# [1] 0.893

median(subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "MYCN")$COR)
median(subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "LR")$COR)
# [1] -0.6856597
# [1] -0.7443664

median(subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "MYCN")$PC2)
median(subset(samples.nbl.tpm.MYCN_LR, GROUP_ID3 == "LR")$PC2)
# [1] -3.271034
# [1] -16.78427
...
## SRC
src$RHO <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.MYCN_LR$PC2, method="spearman", exact=F)[[4]])
src$P   <- mapply(x = 1:nrow(tpm.gene.log2), function(x) cor.test(as.numeric(tpm.gene.log2[x,]), samples.nbl.tpm.MYCN_LR$PC2, method="spearman", exact=F)[[3]])
src <- src[!is.na(src$P),]
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_PC2-vs-TPM_q_n26.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples, file=file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_PC2-vs-TPM_q_n26.RData"))
...
for (g in 1:length(genes)) {
   ...
   plotPC2(file.path(wd.de.plots, "2015/MYCN+LR"), genes[g], as.numeric(tpm.gene.log2[id,]), samples.nbl.tpm.MYCN_LR$PC2, 1, pos="bottomright", cols=c(red, blue), size=5)
}
...
dim(tpm.gene.log2)
# [1] 34908    26
cna.gene.nona.tpm.src <- cna.gene.nona.tpm[, samples.nbl.tpm.MYCN_LR$SAMPLE_ID]
tpm.gene.log2.cna <- tpm.gene.log2[rownames(cna.gene.nona.tpm.src), rownames(samples.nbl.tpm.MYCN_LR)]
colnames(tpm.gene.log2.cna) <- samples.nbl.tpm.MYCN_LR$SAMPLE_ID
...
writeTable(src.tpm.gene, file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n26.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.nbl.tpm.MYCN_LR, file=file.path(wd.de.data, "2015/MYCN+LR", "SRC_NBL_tpm-gene_CNA-vs-TPM_q_n26.RData"))
...
###
##
genes <- c("TERT", "MYCN", "ESR1", "MKI67", "NTRK1")
for (g in 1:length(genes)) {
   id <- subset(ensGene, external_gene_name == genes[g])$ensembl_gene_id
   plotCNA(file.path(wd.de.plots, "2015/MYCN+LR"), genes[g], as.numeric(tpm.gene.log2.cna[id,]), as.numeric(cna.gene.nona.tpm.src[id,]), 1, pos="bottomright", col="black", size=5)
}
...
## Expressed
de <- getCannoli(file.path(wd.de.data, "2015/MYCN+LR"), BASE, 26, NULL, TEST="PC2")
load(file.path(wd, base, "analysis/expression/kallisto", paste0(base, "-tpm-de/data/", base, "_kallisto_0.43.1_tpm.gene.median0.RData")))
expressed <- intersect(rownames(de), rownames(tpm.gene))

de <- getCannoli(file.path(wd.de.data, "2015/MYCN+LR"), BASE, 26, expressed, TEST="PC2")
plot.de <- file.path(wd.de.plots, "2015/MYCN+LR", "cannoliplot_SRC_NBL_TPM-CNA-PC2_P1E03_MEDIAN0")
#genes <- readTable(paste0(plot.de, ".tab"), header=T, rownames=F, sep="\t")
file.de <- paste0(plot.de, ".pdf")
...
writeRNKformatCNA(rbind(de.pos.pos, de.pos.neg), wd.de.gsea, "/MYCN+LR/SRC_NBL_tpm-gene-median0_PC2-CNA-TPM_q_n54_GAIN")   ## GSEA
writeRNKformatCNA(rbind(de.neg.pos, de.neg.neg), wd.de.gsea, "/MYCN+LR/SRC_NBL_tpm-gene-median0_PC2-CNA-TPM_q_n54_LOSS")   ## GSEA
...
file.name <- paste0("DE_", "NBL", "_tpm-gene-median0_RISK_wilcox_q_n26")
file.main <- paste0("", BASE)

de <- differentialAnalysis(tpm.gene.log2, samples.nbl.tpm.MYCN_LR, argv$predictor, argv$predictor.wt, argv$test, argv$test.fdr)

## Ensembl gene annotations
annot <- ensGene[,c("ensembl_gene_id", "external_gene_name", "chromosome_name", "strand", "start_position", "end_position", "gene_biotype")]
de.tpm.gene <- cbind(annot[rownames(de),], de)

save(de.tpm.gene, file=file.path(wd.de.data, "2015/MYCN+LR", paste0(file.name, ".RData")))
writeTable(de.tpm.gene, file.path(wd.de.data, "2015/MYCN+LR", paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")
...
plot.de <- file.path(wd.de.plots, "/2015/MYCN+LR", "volcanoplot_NBL_r5p47_RISK_TERT")
file.de <- paste0(plot.de, ".pdf")
file.main <- c("LR (n=17) vs. MYCN (n=9)", "")
plotVolcano(de.tpm.gene, 1, genes, file.de, file.main, xlab.text, ylab.text, "bottomleft", c("LR < MYCN", "LR > MYCN"), c(adjustcolor.red, adjustcolor.blue), c(red, blue), fold=0, ymax=0)
...
plotVolcano <- function(de, pvalue, genes, file.de, file.main, xlab.text, ylab.text, legend, legends, cols, cols2, fold=1, ymax=0) {
   ...
   par(mar=c(5.1, 4.6, 4.1, 1.5), xpd=F)
