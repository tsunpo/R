...
wd.rna <- file.path(wd, "GTEx", "ngs/RNA", "WB")
wd.rna.raw <- file.path(wd.rna, "kallisto_hg19.ensembl_quant-b100--bias")

wd.anlys <- file.path(wd, "GTEx", "analysis")
wd.de    <- file.path(wd.anlys, "expression/kallisto", paste0("wb", "-tpm-de"))
wd.de.data  <- file.path(wd.de, "data")
wd.de.plots <- file.path(wd.de, "plots")

samples <- readTable(file.path(wd.rna, "gtex_wb_rna.list2"), header=F, rownames=F, sep="")
samples <- samples$V1
...
# 58752 targets passed the filter
# normalizing tpm
# merging in metadata
# aggregating by column: ens_gene
# 17643 genes passed the filter
...
# > nrow(tpm.gene)
# [1] 13919
# > 13919 - 16214   ## Different with line 105
...
tpm.gene.log2 <- getLog2andMedian(tpm.gene)
plotDensityCount(tpm.gene.log2$MEDIAN, nrow(tpm.gene.log2), file.path(wd.de.data, paste0(base, "_kallisto_0.43.1_tpm.gene_tpm0.pdf")), ymax=NULL)
