...
trait[which(trait == 0)] <- "UN"
trait[which(trait == 1)] <- "TR"
trait[which(trait == 2)] <- "N"

file.main <- c("ESAD samples (n=68)", "")
plotPCA(1, 2, pca.de, trait, wd.de.plots, "PCA_ESAD_UN-TR-N_n68", size=6.5, file.main, "topleft", c("blue", "red", "#01DF01"), NULL, flip.x=1, flip.y=1, legend.title=NA)
