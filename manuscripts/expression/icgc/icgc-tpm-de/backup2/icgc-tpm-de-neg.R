...
samples.neg <- subset(samples, histology_abbreviation %in% rownames(icgc.neg))
samples.neg$SG1 <- "G1"
samples.neg[which(samples.neg$COR > -0.6503083),]$SG1 <- "S"
samples.neg$SG1 <- as.factor(samples.neg$SG1)
nrow(samples.neg)
# [1] 1085

overlaps <- intersect(samples.neg$icgc_specimen_id, colnames(tpm.gene))
length(overlaps)
# [1] 368
samples.tpm <- samples.neg[overlaps,]
...
file.name <- paste0("de_", base, "_tpm-gene-median0_SG1_wilcox_q_RHO<0_n368")
...
# Samples with MUT SG1: 144
# Samples with  WT SG1: 224
...
file.name <- paste0("de_", base, "_tpm-gene-median0_SEX_wilcox_q_RHO<0_n368")
file.main <- paste0("", BASE)
...
writeTable(src.tpm.gene, file.path(wd.de.data, "src_tpm-gene-median0_icgc_q_RHO<0_n368.txt"), colnames=T, rownames=F, sep="\t")
save(src.tpm.gene, samples.tpm, file=file.path(wd.de.data, "src_tpm-gene-median0_icgc_q_RHO<0_n368.RData"))
nrow(src.tpm.gene)
# [1] 18502
