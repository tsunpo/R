...
nrow(samples.surv.neg)
# [1] 778

overlaps <- intersect(samples.surv.neg$icgc_specimen_id, colnames(tpm.gene))
length(overlaps)
# [1] 159
samples.tpm <- samples.surv.neg[overlaps,]
...
file.name <- paste0("de_", base, "_tpm-gene-median0_SG1_wilcox_q_RHO<0_n159")
...
# Samples with MUT SG1: 58
# Samples with  WT SG1: 101
...
neg.bad <- rownames(subset(subset(de.tpm.gene, P <= 1E-9), LOG2_FC > 0))
neg.good <- rownames(subset(subset(de.tpm.gene, P <= 1E-9), LOG2_FC < 0))

file.name <- paste0("de_", base, "_tpm-gene-median0_SG1+S_wilcox_q_RHO<0_n368_P1E-12_7genes")
writeTable(de.tpm.gene[intersect(neg.bad, pos.bad),], file.path(wd.de.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")

file.name <- paste0("de_", base, "_tpm-gene-median0_SG1+S_wilcox_q_RHO<0_n368_P1E-12_4genes_food")
writeTable(de.tpm.gene[intersect(neg.good, pos.good),], file.path(wd.de.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")
...
file.name <- paste0("de_", base, "_tpm-gene-median0_SEX_wilcox_q_RHO<0_n159")
file.main <- paste0("", BASE)
