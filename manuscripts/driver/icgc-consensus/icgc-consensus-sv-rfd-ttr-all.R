...
##
kbs <- seq(10, 10000, 10)
machines <- toTable(0, length(kbs), 1, paste0("TTR_", kbs, "kb"))
h <- 1

for (k in 1:length(kbs)) {
		 kb <- kbs[k]
		    
		 dels.9kb.ttr.all  <- subset(dels.nona.ttr.all, size < kb*1000)
		 dels.10kb.ttr.all <- subset(dels.nona.ttr.all, size >= kb*1000)

		 machines[h, k] <- testU(dels.9kb.ttr.all$RFD, dels.10kb.ttr.all$RFD)
}
...
file.name <- file.path(wd.rt.plots, paste0("correlation_LENGTH_P_", "ALL", "_TTR"))
plotOS(file.name, "Deletions in TTR", "Size [kb]", text.Log10.P, x, y, machines[h, min], kb, lwd=3)
...
ylim <- c(-1, 1.1)
file.name <- paste0("vioplot_", "ALL", "_DEL_RFD_TTR_", kb, "kb")
plotVio20(wd.rt.plots, file.name, dels.9kb.ttr.all$RFD, dels.10kb.ttr.all$RFD, main="Deletions in TTR", names=c(paste0("Del <", kb, "kb"), paste0("Del >", kb, "kb")), cols=c("black", "black"), cols2=c(orange, lightblue), ylim, ylab="RFD")
