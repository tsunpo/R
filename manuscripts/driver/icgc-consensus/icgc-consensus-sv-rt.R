...
for (h in 1:nrow(machines)) {
	  ...
	  #for (rt in 0:3) {	  
	  	  ...
	  	  for (k in 1:length(kbs)) {
		       ...
		       ##
		       machines[h, k] <- testU(dels.9kb$SPLINE, dels.10kb$SPLINE)
	     }
	     ...
	     file.name <- file.path(wd.rt.plots, paste0("correlation_LENGTH_P_", hist, "_RT"))
	     ...
	     ylim <- c(-1, 1.1)
	     file.name <- paste0("vioplot_", hist, "_rt", rt, "_DEL_RT_", kb, "kb")
	     plotVio20(wd.rt.plots, file.name, dels.9kb$SPLINE, dels.10kb$SPLINE, main=hist, names=c(paste0("Del <", kb, "kb"), paste0("Del >", kb, "kb")), cols="black", cols2=c(red, blue), ylim, ylab="SCLC-NL RT")
...
file.name <- "heatmap_table.sv.del.20.result.20.rt"
plotHeatmap(wd.rt.plots, file.name, pvals, c("white", red), height=3, width=10, text="", ylab="RT")
save(kbs, machines, pvals, sizes, file=file.path(wd.rt.data, "table.sv.del.20.result.20.rt.RData"), version=2)
