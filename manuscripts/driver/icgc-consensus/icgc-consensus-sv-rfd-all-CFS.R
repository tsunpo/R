...
for (rt in 0:3) {
	  ...
	  nrds <- nrds.RT.NRFD[0,]
	  for (f in 1:nrow(cfs)) {
		    overlaps <- getDELRTNRFD(cfs[f,], nrds.RT.NRFD, bed.gc)
...
file.name <- file.path(wd.rt.plots, paste0("NRFD_CFS_TTR-IZ-TZ_20kb_NEW2.pdf"))
plotReportNRFD(report.rfds, c("SCLC-NL", "SCLC", "NB", "CLL"), file.name, "Common fragile sites      ")
...
file.name <- file.path(wd.rt.plots, paste0("RT_CFS.pdf"))
plotReportRT(report.rts, c("SCLC-NL", "SCLC", "NB", "CLL"), file.name, "Common fragile sites      ")
