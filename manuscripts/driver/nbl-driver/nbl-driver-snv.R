...
samples.nbl.low <- subset(samples.nbl, RISK == "low")
# > nrow(samples.nbl.high)
# [1] 39
# > nrow(samples.nbl.low)
# [1] 17

for (s in 1:nrow(samples.nbl)) {
   sample <- samples.nbl[s,]$SAMPLE_ID
 
   muts <- read.peiflyne.mutcall.filtered.vcf(file.path(wd.vcf, sample, paste0(sample, "_ANALYSIS"), paste0(sample, "_mutcall_filtered.vcf")), pass=T, rs=F)
   muts.gene <- getSNVinEnsGene(muts, ensGene)
   
   writeTable(muts.gene, gzfile(file.path(wd.driver.data, "point_mutations", paste0(sample, "_mutcall_filtered_ens.vcf.gz"))), colnames=T, rownames=F, sep="\t")
}
...
for (s in 1:nrow(samples.nbl)) {
   ...
   mut.dupl <- readTable(file.path(wd.driver.data, "point_mutations", "2015", paste0(sample.nbl$SAMPLE_ID, "_mutcall_filtered_ens.vcf.gz")), header=T, rownames=F, sep="")
...
save(samples.nbl, mut.gene, file=file.path(wd.driver.data, "point_mutations", "2015", "nbl-driver-mut_n56.RData"), version=2)

###
## Proliferation rate (14/07/22)
x <- mapply(x = 1:ncol(mut.gene.2015), function(x) sum(mut.gene.2015[,x]))
y <- samples.nbl$COR
file.name <- file.path(wd.driver.plots, "2015", paste0("correlation_COR-vs-MUT_2015"))
plotCorrelation(file.name, "Peifer 2015", "N of gene SNVs", "Proliferative index", x, y, size=5)

x <- mapply(x = 1:ncol(mut.gene), function(x) sum(mut.gene[,x]))
y <- samples.nbl$COR
file.name <- file.path(wd.driver.plots, "2015", paste0("correlation_COR-vs-MUT_BWA-MEM"))
plotCorrelation(file.name, "BWA-MEM", "N of gene SNVs", "Proliferative index", x, y, size=5)

x <- mapply(x = 1:ncol(mut.gene), function(x) sum(mut.gene[,x]))
y <- mapply(x = 1:ncol(mut.gene.2015), function(x) sum(mut.gene.2015[,x]))
file.name <- file.path(wd.driver.plots, "2015", paste0("correlation_MUT_BWA-MEM"))
plotCorrelation(file.name, "N of gene SNVs", "BWA-MEM", "Peifer 2015", x, y, size=5)
...
for (g in 1:nrow(pmr.mut.gene)) {
   ...
   samples.mut <- subset(samples.nbl, SAMPLE_ID %in% ids)
   samples.wt  <- samples.nbl[setdiff(rownames(samples.nbl), rownames(samples.mut)),]
  
   mut.g1 <- rownames(subset(samples.mut, RISK == "low"))
   mut.s  <- rownames(subset(samples.mut, RISK == "high"))
   wt.g1  <- rownames(subset(samples.wt,  RISK == "low"))
   wt.s   <- rownames(subset(samples.wt,  RISK == "high"))
...
file.name <- paste0("PMR_", "NBL", "_2015_mut.gene_n=", nrow(samples.nbl), "_s>0_mut>0_g=", nrow(pmr.mut.gene), "_chisq_RISK_Low-vs-High")
...
file.name <- paste0("PMR_", "NBL", "_2015_mut.gene_n=", nrow(samples.nbl), "_s>10_mut>0_g=", nrow(pmr.mut.gene.s10), "_chisq_RISK_Low-vs-High")
...
file.name <- file.path(wd.driver.plots, paste0("RMR_", BASE, "_2015_mut.gene_n=", nrow(samples.nbl), "_s>10_mut>0_g=", nrow(rmr.mut.gene.s10), "_chisq_CDH12"))
plotPMR(file.name, "Risk mutation rate", "RMR", text.Log10.P, rmr.mut.gene.s10, c("GRID2", "CDH12", "CEP112", "ATRX", "PDE4DIP"), size=5, cols=c(adjustcolor.skyblue, adjustcolor.yellow), xmax=28, ymax=5)
...
for (g in 1:nrow(pmr.mut.gene)) {
   ...
   mut.g1 <- rownames(subset(samples.mut, SORTING == "G1"))
   mut.s  <- rownames(subset(samples.mut, SORTING == "S"))
   wt.g1  <- rownames(subset(samples.wt,  SORTING == "G1"))
   wt.s   <- rownames(subset(samples.wt,  SORTING == "S"))
...
file.name <- paste0("PMR_", "NBL", "_2015_mut.gene_n=", nrow(samples.nbl), "_s>0_mut>0_g=", nrow(pmr.mut.gene), "_chisq_SORTING_G1-vs-S")
save(pmr.mut.gene, file=file.path(wd.driver.data, paste0(file.name, ".RData")))
writeTable(pmr.mut.gene, file.path(wd.driver.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")
...
pmr.mut.gene.s10 <- subset(pmr.mut.gene, SAMPLE_FREQ >= 10)
pmr.mut.gene.s10$FDR <- testFDR(pmr.mut.gene.s10$P, "Q")
file.name <- paste0("PMR_", "NBL", "_2015_mut.gene_n=", nrow(samples.nbl), "_s>10_mut>0_g=", nrow(pmr.mut.gene.s10), "_chisq_SORTING_G1-vs-S")
writeTable(pmr.mut.gene.s10, file.path(wd.driver.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")

file.name <- file.path(wd.driver.plots, paste0("PMR-SORTING_", "NBL", "_2015_mut.gene_n=", nrow(samples.nbl), "_s>10_mut>0_g=", nrow(pmr.mut.gene.s10), "_chisq_ESR1"))
...
file.name <- file.path(wd.driver.plots, paste0("PMR-SORTING_", base, "_mut.gene_n=", nrow(samples.nbl.high), "_s>10_mut>0_g=", nrow(pmr.mut.gene.s10), "_chisq_ATRX"))
plotPMR(file.name, "Proliferative mutation rate", "PMR", text.Log10.P, pmr.mut.gene.s10, c("ATRX", "ESR1"), size=5, xmax=31.2, ymax=3.73, cols=c(adjustcolor.yellow, adjustcolor.red))
...
###
##
ids.LR    <- rownames(subset(samples.nbl, GROUP_ID2 == "LR"))
ids.HR.G1 <- rownames(subset(samples.nbl, GROUP_ID2 == "G1-HR"))
ids.HR.S  <- rownames(subset(samples.nbl, GROUP_ID2 == "S-HR"))
mut.dupls.LR <- subset(mut.dupls, SAMPLE_ID %in% ids.LR)
mut.dupls.HR.G1 <- subset(mut.dupls, SAMPLE_ID %in% ids.HR.G1)
mut.dupls.HR.S  <- subset(mut.dupls, SAMPLE_ID %in% ids.HR.S)
mut.dupls.HR.S$Y  <- 0.2
mut.dupls.HR.G1$Y <- 0.3
mut.dupls.LR$Y    <- 0.4

file.name <- file.path(wd.driver.plots, paste0("NBL_ESR1_LR-HR-G1-S_Ensembl"))
xlab.text <- paste0("Chromosome 6 position [Mb]")
#pdf(paste0(file.name, ".pdf"), height=6, width=47.2928/2)
pdf(paste0(file.name, ".pdf"), height=6, width=22)
par(mar=c(10,1,5,1))
plot(NULL, xlim=c(152000000/1E6, 152450754/1E6), ylim=c(0, 0.6), xlab=xlab.text, ylab="", yaxt="n", main="ESR1 mutations (Ensembl)", cex.axis=3.5, cex.lab=3.8, cex.main=4, mgp=c(8,3,0), axes=F)
axis(side=1, cex.axis=3.5, mgp=c(8,3.5,0), lwd=6, tck=-0.08)

ESR1 <- subset(subset(ensGene.transcript, ensembl_gene_id == "ENSG00000091831"), transcript_biotype == "protein_coding")[-c(1,6),]
for (t in 1:nrow(ESR1)) {
   rect(ESR1$transcript_start[t]/1E6, 0, ESR1$transcript_end[t]/1E6, 0.13)
}

points(mut.dupls.HR.S$POS/1E6,  mut.dupls.HR.S$Y,  col=red, pch=6, cex=5, lwd=4)
points(mut.dupls.HR.G1$POS/1E6, mut.dupls.HR.G1$Y, col=yellow, pch=6, cex=5, lwd=4)
points(mut.dupls.LR$POS/1E6,    mut.dupls.LR$Y,    col="skyblue", pch=6, cex=5, lwd=4)
dev.off()

###
##
file.name <- file.path(wd.driver.plots, paste0("NBL_ESR1_LR-HR-G1-S_RefSeq"))
xlab.text <- paste0("Chromosome 6 position [Mb]")
pdf(paste0(file.name, ".pdf"), height=6, width=22)
par(mar=c(10,1,5,1))
plot(NULL, xlim=c(152000000/1E6, 152450754/1E6), ylim=c(0, 0.6), xlab=xlab.text, ylab="", yaxt="n", main="ESR1 mutations (RefSeq)", cex.axis=3.5, cex.lab=3.8, cex.main=4, mgp=c(8,3,0), axes=F)
axis(side=1, cex.axis=3.5, mgp=c(8,3.5,0), lwd=6, tck=-0.08)

ESR1 <- subset(refGene, refseq_accession == "NM_000125")
for (t in 1:8) {
   rect(as.numeric(unlist(strsplit(ESR1$V8, ","))[t])/1E6, 0, as.numeric(unlist(strsplit(ESR1$V9, ","))[t])/1E6, 0.13)
}
