...
n2373 <- readTable(file.path(wd.driver.data, paste0("n2373.list")), header=F, rownames=F, sep="")
exits <- strsplit0(n2373, "_mutcall_filtered_ens.vcf.gz", 1)

patches <- setdiff(rownames(samples.vcf), exits)
samples.vcf <- samples.vcf[patches,]

# -----------------------------------------------------------------------------
# Step 1: Finding mutations locate within Ensembl genes
# Last Modified: 23/01/18
# -----------------------------------------------------------------------------
for (s in 1:nrow(samples.vcf)) {
