...
for (s in START:END) {
   ...
   vcf.gene <- getSNVEnsGeneTSS(vcf, ensGene)
 
   writeTable(vcf.gene, gzfile(file.path(wd.driver.data, "point_mutations_eqtl", paste0(sample.vcf$icgc_specimen_id, "_mutcall_filtered_ens.vcf.gz"))), colnames=T, rownames=F, sep="\t")
   rm(vcf.gene)
}
