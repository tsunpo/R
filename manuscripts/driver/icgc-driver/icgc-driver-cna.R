...
file.name <- paste0("de_", base, "_cna-gene_SG1_wilcox_q_n2443_cn.seg")
...
##
overlaps <- intersect(rownames(de.cna.gene), rownames(de.tpm.gene))
x <- -log10(de.tpm.gene[overlaps,]$P)
y <- -log10(de.cna.gene[overlaps,]$P)
file.name <- file.path(wd.driver.data, paste0("correlation_", base, "_cna-gene_vs_tpm-gene_SG1"))
plotCorrelation(file.name, "Differental CNA expression", "Differental expression", "Differental CNA", x, y, pos="topright", line=2.75)

for (tpm in 1:5) {
   genes.tpm <- rownames(tpm.gene[rownames(subset(tpm.gene.m, MEDIAN >= tpm)),])
 
   x <- -log10(de.tpm.gene[genes.tpm,]$P)
   y <- -log10(de.cna.gene[genes.tpm,]$P)
   file.name <- file.path(wd.driver.data, paste0("correlation_", base, "_cna-gene_vs_tpm-gene_SG1_tpm>", tpm))
   plotCorrelation(file.name, "Differental CNA expression", "Differental expression", "Differental CNA", x, y, pos="topright", line=2.75)
}

de.sig <- subset(de.cna.gene[rownames(subset(de.tpm.gene, P < 1E-21)),], P < 1E-15)
de.sig <- de.sig[order(de.sig$P),]
de.cna.gene.sig <- cbind(de.sig, de.tpm.gene[rownames(de.sig), c("P", "FDR", "SG1_WT", "SG1", "LOG2_FC")])

file.name <- paste0("de_", base, "_cna-gene_SG1_wilcox_q_n2443_cn.seg_tpm>1_sig")
save(de.cna.gene.sig,  file=file.path(wd.driver.data, paste0(file.name, ".RData")))
writeTable(de.cna.gene.sig, file.path(wd.driver.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")

## Consistent CNA < 0
overlaps <- rownames(subset(de.cna.gene[rownames(subset(de.tpm.gene, LOG2_FC < 0)),], LOG2_FC < 0))
x <- -log10(de.tpm.gene[overlaps,]$P)
y <- -log10(de.cna.gene[overlaps,]$P)
file.name <- file.path(wd.driver.data, paste0("correlation_", base, "_cna-gene_vs_tpm-gene_SG1_FC<0"))
plotCorrelation(file.name, "Differental CNA loss expression", "Differental expression", "Differental CNA", x, y, pos="topright", line=2.75)

de.cna.gene.sig <- cbind(de.cna.gene[overlaps,], de.tpm.gene[overlaps, c("P", "FDR", "SG1_WT", "SG1", "LOG2_FC")])
file.name <- paste0("de_", base, "_cna-gene_SG1_wilcox_q_n2443_cn.seg_tpm>1_FC<0")
save(de.cna.gene.sig,  file=file.path(wd.driver.data, paste0(file.name, ".RData")))
writeTable(de.cna.gene.sig, file.path(wd.driver.data, paste0(file.name, ".txt")), colnames=T, rownames=F, sep="\t")
...
y <- -log10(de.cna.gene[overlaps,]$P)
file.name <- file.path(wd.driver.data, paste0("correlation_", base, "_cna-gene_vs_tpm-gene_SG1_FC>0"))
plotCorrelation(file.name, "Differental CNA gain expression", "Differental expression", "Differental CNA", x, y, pos="topright", line=2.75)
...
de.sig <- subset(de.cna.gene[rownames(subset(de.tpm.gene, LOG2_FC < 0)),], LOG2_FC < 0)
de.sig <- de.sig[order(de.sig$P),]
de.cna.gene.sig <- cbind(de.sig, de.tpm.gene[rownames(de.sig), c("P", "FDR", "SG1_WT", "SG1", "LOG2_FC")])
