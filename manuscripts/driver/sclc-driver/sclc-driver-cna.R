...
# Last Modified: 22/03/22
# -----------------------------------------------------------------------------
for (s in 1:nrow(samples.sclc)) {
   sample <- samples.sclc[s,]$SAMPLE_ID
 
   colnames <- c("ensembl_gene_id", "CN")
   ensGene.cna <- toTable("", length(colnames), 0, colnames)
   segs <- read.peiflyne.cn.seg(file.path("/Users/tpyang/Work/uni-koeln/tyang2/SCLC/ngs/WGS/peiflyne", sample, paste0(sample, "_ANALYSIS"), paste0(sample, "_iCN.seg")))   ## See ReplicationTiming.R / Inner Class / PeifLyne File Reader
   ...
   writeTable(ensGene.cna, gzfile(file.path(wd.driver.data, "copy_number_alterations", paste0(sample, "_ens.iCN.seg.gz"))), colnames=T, rownames=F, sep="\t")
}
...
# -----------------------------------------------------------------------------
cna.gene <- getGeneCNV(samples.sclc)
cna.gene.nona <- cna.gene[removeMissing(cna.gene),]
save(samples.sclc, cna.gene, cna.gene.nona, file=file.path(wd.driver.data, "sclc-driver-cna_iCN.seg_NONA.RData"), version=2)
...
wgds <- wgds[rownames(samples.sclc),]
wgds[which(wgds$wgd_predicted == 0), ]$wgd_predicted <- "non-WGD"
wgds[which(wgds$wgd_predicted == 1), ]$wgd_predicted <- "WGD"

##
test <- cna.gene.nona[, rownames(samples.sclc)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.iCN <- getPCA(t(test))
save(pca.iCN, file=file.path(wd.driver.data, paste0("PCA-SCLC-iCN_n101_g57736.RData")))

trait <- wgds$wgd_predicted
file.main <- c("SCLC gene CNA (57,736 genes)", "")
plotPCA(1, 2, pca.iCN, trait, wd.driver.plots, "PCA-SCLC-iCN_WGD_n101_g57736", size=6, file.main, "topleft", c("WGD", "non-WGD"), c("black", "lightgray"), flip.x=1, flip.y=1)

scores.iCN <- pcaScores(pca.iCN)
x <- scores.iCN$PC1
y <- wgds$decision_value
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN_PC1-vs-WGD"))
plotCorrelation(file.name, "SCLC gene CNA (57,736 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "WGD", x, y, size=6)

x <- scores.iCN$PC1
y <- samples.sclc$COR
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN_PC1-vs-SORTING"))
plotCorrelation(file.name, "SCLC gene CNA (57,736 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), expression(italic("In silico") ~ "sorting"), x, y, size=6)

##
test <- cna.gene.nona.tpm[, rownames(samples.sclc)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.iCN <- getPCA(t(test))
save(pca.iCN, file=file.path(wd.driver.data, paste0("PCA-SCLC-iCN-TPM_n101_g34895.RData")))
...
##
test <- cna.gene.nona.tpm.median0[, rownames(samples.sclc)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.iCN <- getPCA(t(test))
save(pca.iCN, file=file.path(wd.driver.data, paste0("PCA-SCLC-iCN-TPM-MEDIAN0_n101_g23559.RData")))

trait <- wgds$wgd_predicted
file.main <- c("SCLC gene CNA", "")
plotPCA(1, 2, pca.iCN, trait, wd.driver.plots, "PCA-SCLC-iCN-TPM-MEDIAN0_WGD_n101_g23559", size=5, file.main, "topleft", c("WGD", "non-WGD"), c("black", "lightgray"), flip.x=1, flip.y=-1)

scores.iCN <- pcaScores(pca.iCN)
x <- scores.iCN[overlaps,]$PC1
y <- wgds[overlaps,]$decision_value
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-MEDIAN0_PC1-vs-WGD"))
plotCorrelation(file.name, "SCLC gene CNA", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "WGD", x, y, size=5)

x <- scores.iCN[overlaps,]$PC1
y <- samples.sclc[overlaps,]$COR
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-MEDIAN0_PC1-vs-SORTING"))
plotCorrelation(file.name, "SCLC gene CNA", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "Proliferation rate", x, y, size=5)

x <- scores.iCN[overlaps,]$PC1
y <- samples.wgs[overlaps,]$purity
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-MEDIAN0_PC1-vs-purity"))
plotCorrelation(file.name, "SCLC gene CNA", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "Purity", x, y, size=5)

x <- scores.iCN[overlaps,]$PC1
y <- samples.wgs[overlaps,]$ploidy
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-MEDIAN0_PC1-vs-ploidy"))
plotCorrelation(file.name, "SCLC gene CNA", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "Ploidy", x, y, size=5)
...
gene.sclc   <- c(rownames(tpm.gene.log2.m.rfd.ctr.iz),   rownames(tpm.gene.log2.m.rfd.ctr.tz),   rownames(tpm.gene.log2.m.rfd.ttr))
gene.sclc.e <- c(rownames(tpm.gene.log2.m.rfd.ctr.iz.e), rownames(tpm.gene.log2.m.rfd.ctr.tz.e), rownames(tpm.gene.log2.m.rfd.ttr.e))
gene.sclc.l <- c(rownames(tpm.gene.log2.m.rfd.ctr.iz.l), rownames(tpm.gene.log2.m.rfd.ctr.tz.l), rownames(tpm.gene.log2.m.rfd.ttr.l))
length(gene.sclc)
# [1] 31612
length(gene.sclc.e)
# [1] 23534
...
gene.sclc.e.cna <- intersect(rownames(cna.gene.nona.tpm), gene.sclc.e)
gene.sclc.l.cna <- intersect(rownames(cna.gene.nona.tpm), gene.sclc.l)
length(gene.sclc.e.cna)
# [1] 23534
length(gene.sclc.l.cna)
# [1] 8078

##
test <- cna.gene.nona.tpm[gene.sclc.e, rownames(samples.sclc)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.iCN <- getPCA(t(test))
#save(pca.iCN, file=file.path(wd.driver.data, paste0("PCA-SCLC-iCN-TPM-RT-EARLY_n101_g23534.RData")))

trait <- wgds$wgd_predicted
file.main <- c("SCLC gene CNA (23,534 genes)", "")
plotPCA(1, 2, pca.iCN, trait, wd.driver.plots, "PCA-SCLC-iCN-TPM-RT-EARLY_WGD_n101_g23534", size=6, file.main, "topleft", c("WGD", "non-WGD"), c("black", "lightgray"), flip.x=1, flip.y=1)

scores.iCN <- pcaScores(pca.iCN)
x <- scores.iCN$PC1
y <- wgds$decision_value
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-RT-EARLY_PC1-vs-WGD"))
plotCorrelation(file.name, "SCLC gene CNA (23,534 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "WGD", x, y, size=6)

x <- scores.iCN$PC1
y <- samples.sclc$COR
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-RT-EARLY_PC1-vs-SORTING"))
plotCorrelation(file.name, "SCLC gene CNA (23,534 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), expression(italic("In silico") ~ "sorting"), x, y, size=6)

##
test <- cna.gene.nona.tpm[gene.sclc.l, rownames(samples.sclc)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.iCN <- getPCA(t(test))
#save(pca.iCN, file=file.path(wd.driver.data, paste0("PCA-SCLC-iCN-TPM-RT-LATE_n101_g8078.RData")))

trait <- wgds$wgd_predicted
file.main <- c("SCLC gene CNA (8,078 genes)", "")
plotPCA(1, 2, pca.iCN, trait, wd.driver.plots, "PCA-SCLC-iCN-TPM-RT-LATE_WGD_n101_g8078", size=6, file.main, "topleft", c("WGD", "non-WGD"), c("black", "lightgray"), flip.x=1, flip.y=1)

scores.iCN <- pcaScores(pca.iCN)
x <- scores.iCN$PC1
y <- wgds$decision_value
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-RT-LATE_PC1-vs-WGD"))
plotCorrelation(file.name, "SCLC gene CNA (8,078 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "WGD", x, y, size=6)

x <- scores.iCN$PC1
y <- samples.sclc$COR
file.name <- file.path(wd.driver.plots, paste0("correlation_PCA-SCLC-iCN-TPM-RT-LATE_PC1-vs-SORTING"))
plotCorrelation(file.name, "SCLC gene CNA (8,078 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), expression(italic("In silico") ~ "sorting"), x, y, size=6)
...
## WGD
scores.iCN <- pcaScores(pca.iCN)
overlaps <- intersect(rownames(scores.iCN), rownames(samples.sclc))

x <- scores.iCN[overlaps,]$PC1
y <- wgds[overlaps,]$decision_value
file.name <- file.path(wd.driver.plots, paste0("correlation_CNA-GENE-PC1-vs-WGD"))
plotCorrelation(file.name, "SCLC gene CNA (57,736 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), "WGD", x, y, size=6)

x <- scores.iCN[overlaps,]$PC1
y <- samples.sclc[overlaps,]$COR
file.name <- file.path(wd.driver.plots, paste0("correlation_CNA-GENE-PC1-vs-SORTING"))
plotCorrelation(file.name, "SCLC gene CNA (57,736 genes)", paste0("PC", 1, " (", pcaProportionofVariance(pca.iCN, 1), "%)"), expression(italic("In silico") ~ "sorting"), x, y, size=6)
...
plotCorrelation(file.name, "SCLC samples", "PC2 (4.1%)", "WGD", x, y, size=6)

y <- wgds$decision_value
x <- samples.sclc$COR
file.name <- file.path(wd.driver.plots, paste0("correlation_WGD_vs_SORTING"))
plotCorrelation(file.name, "SCLC samples", expression(italic("In silico") ~ "sorting"), "WGD", x, y, size=6)

##
trait <- wgds$wgd_predicted
file.main <- c("SCLC gene CNA (34,895 genes)", "")
plotPCA(1, 2, pca.iCN, trait, wd.driver.plots, "PCA-SCLC-iCNTPM_WGD_n101_g34895", size=6, file.main, "topright", c("WGD", "non-WGD"), c("black", "lightgray"), flip.x=1, flip.y=1)
...
save(pca.de, file=file.path(wd.driver.data, paste0("PCA_SCLC_iCN_n101_g57736.RData")))

#samples.sclc$M2 <- samples.sclc$M2 + 1
trait <- as.vector(samples.sclc$M2)
file.main <- c("SCLC gene CNA (57,736 genes)", "")
plotPCA(2, 3, pca.de, trait, wd.driver.plots, "PCA_SCLC_M2-M1_n101_g57736", size=6, file.main, "topright", c(2, 1), c(red, blue), flip.x=1, flip.y=1)

##
test <- cna.gene.nona[, rownames(samples.surv.sclc)]   ## BUG FIX 13/02/17: Perform PCA using normalised data (NOT log2-transformed)
pca.de <- getPCA(t(test))
save(pca.de, file=file.path(wd.driver.data, paste0("PCA_SCLC_iCN_n92_g57736.RData")))

trait <- as.vector(samples.surv.sclc$SORTING)
file.main <- c("SCLC gene CNA (57,736 genes)", "")
plotPCA(1, 2, pca.de, trait, wd.driver.plots, "PCA_SCLC_G1-S_n92_g57736", size=6, file.main, "topright", c("S", "G1"), c(red, blue), flip.x=1, flip.y=1)

##
#samples.sclc$M2 <- samples.sclc$M2 + 1
trait <- as.vector(samples.surv.sclc$STAGE)
file.main <- c("SCLC gene CNA (57,736 genes)", "")
plotPCA(1, 2, pca.de, trait, wd.driver.plots, "PCA_SCLC_STAGE_n92_g57736", size=6, file.main, "topright", c("III-IV", "I-II", "NA"), c(yellow, "skyblue", "lightgray"), flip.x=1, flip.y=1)
...
plotCorrelation(file.name, "SCLC samples (57,736 genes)", "PC1 (53.1%)", expression(italic("In silico") ~ "sorting"), x, y, size=6)

y <- samples.sclc$COR
x <- scores$PC2
file.name <- file.path(wd.driver.plots, paste0("correlation_SORTING_vs_PC2_g57736"))
plotCorrelation(file.name, "SCLC samples (57,736 genes)", "PC2 (3.9%)", expression(italic("In silico") ~ "sorting"), x, y, size=6)

y <- samples.sclc$COR
x <- scores$PC3
file.name <- file.path(wd.driver.plots, paste0("correlation_SORTING_vs_PC3_g57736"))
plotCorrelation(file.name, "SCLC samples (57,736 genes)", "PC3 (3.6%)", expression(italic("In silico") ~ "sorting"), x, y, size=6)
